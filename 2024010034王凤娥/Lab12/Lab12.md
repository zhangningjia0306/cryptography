...
| :--- | :------- |
| 访问的网站 |3n7er.space|
| 证书主题（Subject） |C=US, O=Let's Encrypt, CN=E7 |
| 证书颁发者（Issuer） |C=US, O=Let's Encrypt, CN=E7|
| 证书有效期（起始-结束） | May 22 12:16:05 2026 GMT- Aug 20 12:16:04 2026 GMT|
| 公钥算法 | id-ecPublicKey(P-256 / prime256v1)|
...
| 证书颁发者（Issuer） |C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Certificate Authority, CN=My Root CA(自签名) |
| 主题和颁发者是否相同 |是 |
| 证书有效期（天数） | 3650|
| 公钥长度（位） | 4096|
| Basic Constraints 中 CA 字段的值 | TRUE|
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |	e6:c8:63:5c:74:7f:7f:b3:f7:e0:b9:c7:67:51:17:ae:6e:c3:58:c1:22:cd:71:06:e6:09:bd:0b:27:f0:9f:38|
| SHA-1 指纹（完整） | a2:c5:2d:cc:c9:e3:2f:ff:e4:09:8f:32:10:d3:2c:c7:a3:ab:66:0f|
...
| 浏览器默认是否信任 |是 |否 |
| 证书签发者是否为第三方 CA |是 |否 |
| 适合的使用场景 | 公开互联网网站、面向公众的服务|企业内部网络、测试环境、内网服务、开发测试|
...
> 答：数字证书本质是由可信第三方CA签发的、将实体身份（如域名/组织）与公钥绑定的电子凭证（X.509标准格式）。它解决的是公钥分发中的"中间人攻击"问题——在开放网络中，攻击者可以替换你收到的公钥为自己伪造的公钥，仅靠数字签名无法证明"这个公钥确实属于Alice"。
单独的数字签名只能验证数据完整性和声称的签名者，但无法解决"你拿到的那个公钥本身是否可信、属于谁"这个信任锚点问题。数字证书通过CA的签名背书，让验证方用本地已信任的CA公钥来验证证书签名，从而确认证书内公钥的真实归属。
...
> 答：自签名证书：由自身私钥生成并签名（Subject = Issuer），没有上级CA为其背书。
浏览器警告原因：浏览器在系统信任库中找不到该自签名证书的签发者（即它自己），无法建立信任链，因此判定为不可信并弹出警告（NET::ERR_CERT_AUTHORITY_INVALID）。这不等于加密不安全，而是身份未被第三方验证，存在中间人攻击风险。
适用场景：本地开发/测试环境（localhost）、内网设备（路由器/NAS）、企业内部服务（配合手动导入私有根CA到信任库后使用）、作为私有PKI的根CA本身。
...
> 答：为什么需要中间CA：根CA私钥极其敏感，通常离线保存在HSM（硬件安全模块）中。如果根CA直接签发海量终端实体证书，根私钥频繁使用增加泄露风险；一旦根CA私钥泄露，所有由其签发的证书瞬间失效需全局更新信任库，代价极大。
中间CA的作用：根CA只用于签发少量中间CA证书，中间CA负责日常签发终端实体证书。中间CA被攻破时只需吊销该中间CA并由根CA重新签发新中间CA，不影响其他分支和根CA本身的信任状态。这也便于按业务/地域/用途划分不同签发策略。
...
> 答：根CA是自签名的，我们信任它是因为操作系统和浏览器厂商（Microsoft、Apple、Mozilla、Google等）在出厂时预装了一份经严格审核的"受信任根证书存储（Root Store）"，里面包含约100多个经WebTrust审计的全球权威CA根证书。
信任决策流程：浏览器收到服务器证书→逐级向上验证签名→找到锚点（根CA）→检查该根CA是否在本地信任库中→在则信任。信任来源于预装且经审计的第三方根证书列表，而非密码学本身。
...
> 答：BasicConstraints: CA:TRUE, pathLenConstraint:<n>中的 pathlen 限制该 CA 证书之下允许的非自签名中间CA证书层数（不含终端实体证书）。
pathlen:0：该中间CA只能签发终端实体证书，不能再签发下级CA证书。——实验中你的中间CA设为此值，符合最佳实践。
pathlen:1：该中间CA可以签发终端实体证书，还可以签发一级下级中间CA（但该下级中间CA通常被限制为 pathlen:0）。
这是权限最小化的重要安全控制，防止中间CA被滥用创建无限深度的子CA。
...
> 答：过期后果：浏览器提示 NET::ERR_CERT_DATE_INVALID，默认阻止访问（可手动跳过但不推荐），HTTPS连接被视为不安全。
有效期限制的原因：① 密钥可能随时间被逐步破解或泄露，短周期限制暴露窗口；② CA可能在证书有效期内吊销域名所有权（如域名转让），有效期限制旧证书的有效范围；③ 加密算法随着计算能力提升逐渐变弱，定期更换促进算法升级。
Let's Encrypt 90天原因：短有效期倒逼自动化——鼓励使用ACME协议自动申请和续期，减少人为疏忽导致长期证书过期或私钥长期暴露的风险；同时缩短被泄露/误签发证书的生效窗口，降低CT日志和吊销系统压力。
...
> 答：需要吊销的场景：私钥泄露、域名/组织停止使用、CA发现误签发、员工离职后需撤销其客户端证书等。
CRL（证书吊销列表）：CA定期发布已被吊销的序列号列表。缺点：列表随时间增大、存在更新延迟（缓存期内无法感知最新吊销）、客户端需下载完整列表。
OCSP（在线证书状态协议）：客户端向CA的OCSP Responder实时查询某张证书的状态（good/revoked/unknown）。优点：实时、无需下载大列表；缺点：CA需承受高并发查询压力、泄露用户访问隐私（CA可知你在查哪个站点）、网络故障时无法判断。
现代浏览器多用 OCSP Stapling——由服务器定期代客户端获取OCSP响应并随TLS握手发给浏览器，兼顾隐私与性能。
...
> 答：Subject Alternative Name（SAN）
为什么需要SAN：传统X.509用Subject中的CN（Common Name）字段存域名，但CN只能填一个值、格式不严格、不是专为域名设计。RFC 6125及现代浏览器（Chrome 58+、Firefox、Safari）已忽略CN做主机名匹配，强制检查SAN扩展。
SAN vs CN：SAN是X.509 v3扩展字段（subjectAltName），可包含多个DNS名称、IP地址、邮箱、URI等结构化条目；CN是Subject DN中的一个RDN组件，只含一个字符串且已被废弃用于域名验证。现在正确做法是CN随意填（或同SAN第一个值），域名必须放在SAN的DNS条目中。
...
> 答：计算方法：对DER编码的整个证书二进制数据进行哈希运算得到。SHA-256指纹 = SHA256(certificate_der_bytes)，SHA-1指纹同理。命令行：openssl x509 -in cert.pem -noout -fingerprint -sha256。
证书钉扎（Certificate Pinning）：客户端（App/浏览器）预先硬编码某域名期望的证书公钥哈希或指纹，TLS握手时比对服务器证书的指纹/公钥哈希是否匹配。即使系统信任库中某CA被攻破签发了伪造证书，因指纹不匹配，连接仍会被拒绝。常用于移动App、内网高安全系统等，防止因CA被入侵导致的MITM。
...
> 答：通配符证书
*.example.com匹配：www.example.com、api.example.com、mail.example.com等单层子域。
不匹配：example.com（裸域，无前缀）、a.b.example.com（二级子域，*只匹配一个标签）、*.a.example.com（需另外签发）。
局限性：不覆盖裸域（需同时在SAN中加 DNS:example.com）；不能多级匹配；多数CA不签发 *.com等TLD级通配符；不支持部分通配如 prefix*.example.com。
...
> 答：证书透明度（CT）
Certificate Transparency（CT）​ 是一套公开、仅追加（append-only）、由Merkle树实现的TLS证书签发日志系统。CA签发公开信任证书时必须先将证书提交到CT Log，获得Signed Certificate Timestamp（SCT）后才返回给用户。
解决的问题：防止CA被入侵后秘密签发伪造证书（如历史DigiNotar事件中google.com假证书长期未被发现）。CT让所有公开签发的证书全局可见、可审计，域名所有者可监控CT日志发现未授权的证书签发，浏览器也可要求SCT才信任证书。
...
> 答：Let's Encrypt 的工作原理
免费签发流程（ACME协议）：
用户在服务器运行ACME客户端（如Certbot），生成密钥对并提交CSR。
Let's Encrypt CA通过ACME挑战验证域名控制权：
HTTP-01：客户端在服务器上放置约定文件（/.well-known/acme-challenge/），LE验证HTTP访问是否可达→确认你对域名的Web服务有控制权。
DNS-01：客户端在域名DNS记录中添加指定TXT记录（_acme-challenge.example.com），LE查询DNS验证→适用于通配符证书。
验证通过后，LE用中间CA（R3，由ISRG Root X1签发）签署服务器证书并返回。
90天+自动续期：短有效期促使使用自动化续期脚本（Certbot cron/systemd timer每月自动renew），减少人为失误和密钥暴露窗口；一旦域名转移或不再续期，旧证书自然短期失效。
3n7er.space的链：ISRG Root X1[](@replace=10011)（自签根） → R3/Sectigo E1（中间CA） → 3n7er.space（终端实体）
...
> 答：浏览器信任 Let's Encrypt 签发的 3n7er.space而不信任本实验私有 PKI 签发的服务器证书，根本原因是信任锚（Trust Anchor）不同：浏览器/操作系统出厂预装了 ISRG Root X1 等公开根 CA，Let's Encrypt 的证书链最终能回溯到这个预装根证书；而实验自建的私有根 CA 不在系统信任库中，证书链无法锚定到任何已知可信根，浏览器因此报"证书不受信任（NET::ERR_CERT_AUTHORITY_INVALID）"。
两者相同点与不同点如下：
证书格式（相同）：均为标准 X.509 v3 格式，DER/PEM 编码，含序列号、Subject、Issuer、有效期、公钥、签名算法、SAN/EKU 等扩展，格式无区别。
证书链结构（基本相同）：均遵循根 CA → 中间 CA → 终端实体证书的三级链式结构，中间 CA 有 BasicConstraints CA:TRUE，终端实体证书 CA:FALSE，链验证逻辑一致。
信任根（不同——关键区别）：Let's Encrypt 的信任根是 ISRG Root X1，已通过审计预装在 OS/浏览器信任库中，浏览器自动认可；私有 PKI 的信任根是自建 My Root CA，未加入任何客户端信任库，除非手动导入否则不被承认。
