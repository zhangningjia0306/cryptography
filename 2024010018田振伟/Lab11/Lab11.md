...
| 签名公钥模数 N 的前 16 位十六进制 |00:c4:7e:3b:a1:2f:5d:8c |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护。发送者：田振伟，学号：2024010018 |
...
> 答：身份认证：只有签名私钥持有者能生成合法签名，任何人可用对应公钥核验发送方身份。实验中用signature_public_key.pem验证成功，证明密文由对应私钥持有者发送。
完整性：签名绑定数据哈希，数据任意修改哈希都会改变，验签直接失败。实验篡改密文后输出Verification failure，验证完整性保护。
不可否认性：私钥仅发送者独有，他人无法伪造，发送方无法否认自己签署过该密文。攻击者无签名私钥，无法伪造有效签名，发送者不能抵赖发送行为。
...
> 答：效率限制：RSA 非对称大数运算速度极慢，直接对大文件签名不现实；SHA-256 快速生成 32 字节短摘要，仅对摘要签名，运算速度大幅提升。
输入长度约束：RSA 单次可处理的数据长度受 2048 位模数限制，无法直接处理任意长度文件；哈希统一输出固定长度摘要，适配算法输入要求。
标准化统一：无论原始数据大小，哈希输出长度固定，签名接口统一，算法实现、校验逻辑简洁通用。
...
> 答：RSA 密钥模数单位为 bit，1 字节 = 8bit，2048÷8=256，因此 2048 位 RSA 密钥生成的签名固定 256 字节。
若使用 4096 位 RSA 密钥，4096÷8=512，签名长度为 512 字节。
...
> 答：抗原像性：给定哈希值，无法反向推导出原始密文 / 消息。攻击者拿到签名哈希，不能还原原始密文。
抗第二原像性：已知文件 m1，找不到另一不同文件 m2 拥有相同哈希。攻击者不能构造替换密文复用原有签名。
抗碰撞性：不存在两份完全不同的文件拥有相同哈希值。防止攻击者提前构造两份不同文件共用一份签名。
任意一条不满足，攻击者都能构造替换文件绕过签名校验，破坏完整性。
...
> 答：2017 年谷歌公开 SHA-1 碰撞攻击，可人为制造两份内容完全不同、但 SHA-1 哈希一致的文件，哈希抗碰撞特性失效。
攻击危害：攻击者制作正常文件 A 与恶意文件 B，二者 SHA-1 相同。用户对 A 签名后，攻击者可将文件替换为恶意 B，验签依然通过，伪造合法签名。因此现代数字签名统一使用 SHA-256，淘汰 SHA-1。
...
> 答：签名密钥对
签名私钥：仅发送者持有，用于对密文哈希生成数字签名；
签名公钥：公开分发，所有人用于校验签名合法性。
加密密钥对
加密公钥：公开，发送方用来加密明文；
加密私钥：接收者保密，仅用于解密密文。
攻击者拿到公钥的权限
能做：使用公钥验证已有签名、使用加密公钥加密消息；
不能做：伪造签名、解密任何密文、推导对应的私钥。
...
> 答：加密目标：保障机密性，解决消息不被第三方偷看；使用接收方公钥加密，只有对方私钥能解密。
数字签名目标：身份 + 完整性 + 不可否认，证明数据来源、未被篡改、发送方不能抵赖；使用自身私钥签名，所有人用公钥核验。
不能用加密替代签名：加密只能隐藏内容，无法证明发送人身份；加密后的密文可被中间人截获替换，无法检测篡改，缺少完整性与不可否认保障。
...
> 答：不可否认：签名生成操作唯一对应私钥持有人，第三方可独立核验，发送方无法否认自己签署、发送过该消息。
MAC 基于对称密钥，通信双方共用同一密钥，无法区分是谁生成校验码，任何人都可伪造，无法追责；
RSA 数字签名是非对称体系，签名私钥仅持有者独有，公钥全网公开，第三方可中立验证，具备法律层面不可否认能力。
...
> 答：软件发布：厂商用私钥对安装包签名，用户通过官方公钥验签；防止安装包被黑客篡改植入病毒，确认软件为官方原版。
邮件防钓鱼：S/MIME 邮件签名；确认邮件发送人真实身份，拦截伪造老板、客服的钓鱼邮件。
Git 代码提交签名：开发者私钥签名 commit 记录；防止他人冒用账号提交恶意代码，溯源代码真实作者。
...
> 答：单纯数字签名无法自带可信时间，签名仅绑定文件与私钥，不具备权威时间证明，攻击者可回放旧签名伪造时间。
解决方案：使用 TSA 可信时间戳服务。流程：文件签名后，将签名摘要发送第三方权威时间服务器，TSA 用自身私钥对摘要 + 标准 UTC 时间生成时间戳；核验时同时校验签名与时间戳，由权威机构证明签名真实生成时间。
...
> 答：1024 位 RSA：算力提升后已可被暴力分解，安全性不足，现代规范禁止使用。
2048 位 RSA：本实验选用标准，安全与性能平衡；短期数十年内无法暴力破解，运算速度适中，教学、普通业务场景通用。
4096 位 RSA：安全等级更高，可抵御未来更强算力，但模幂运算计算量翻倍，签名、加密、解密耗时显著变长，仅用于高密级金融、政务场景，普通实验无需使用。
...
> 答：多层签名具备实际意义，即签名链 / 证书链；每一级签名者对上一级完整数据（含上一级签名）再次签名，逐级背书信任。
适用场景：
CA 数字证书链：根 CA 签名二级 CA 证书，二级 CA 再颁发用户证书，多级签名构建信任体系；
多级审批文件：员工签名文件，部门负责人二次签名确认审批，分层权责；
区块链区块：每区块包含上一区块哈希与签名，形成链式多重校验。
多重签名可分层验证身份、划分权责、构建逐级可信链路。
