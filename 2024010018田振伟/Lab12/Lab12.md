...
| 证书颁发者（Issuer） |subject=CN=3n7er.space  issuer=C=US, O=Let's Encrypt, CN=E7 |
| 证书有效期（起始-结束） |2026-05-22 12:16:05 GMT - 2026-08-20 12:16:04 GMT |
| 公钥算法 | Public Key Algorithm: id-ecPublicKey |
| 签名算法 | Signature Algorithm: ecdsa-with-SHA384|
| 证书链层级数 |3 |
| 根 CA 名称 | O=Let's Encrypt, CN=E7 |
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |BB:E1:F2:1A:E1:55:7F:B6:DD:A3:68:E5:52:0A:13:0F:1A:6D:A1:FF:9D:FB:8F:56:15:BA:46:A1:4F:9F:AE:97 |
| SHA-1 指纹（完整） |A6:20:00:51:01:1A:07:4E:6E:55:68:38:FF:D4:09:C9:D7:8E:C8:C0 |
...
| 浏览器默认是否信任 |是 | 否|
| 证书签发者是否为第三方 CA |是 |否 |
| 适合的使用场景 |公网对外网站、互联网公开 HTTPS 服务 |企业内网、本地开发测试、内部系统 |
...
> 答：本质：数字证书是由可信第三方 CA 签发、绑定实体身份与公钥的电子凭证，包含主体信息、公钥、有效期、CA 数字签名，遵循 X.509 标准。
> 解决的核心问题：公钥分发时的中间人劫持问题，证明 “这个公钥确实属于对应网站 / 个人”，防止攻击者伪造公钥窃听、篡改通信。
> 仅数字签名不足的原因：数字签名只能证明 “数据未被篡改、签名者持有对应私钥”，但无法验证公钥归属；攻击者可伪造公钥并对伪造公钥做签名，没有证书则无法辨别公钥真假，中间人攻击依然可行。
...
> 答：自签名证书：证书签发者 (Issuer) 与使用者 (Subject) 为同一实体，用自身私钥签名自身公钥，无第三方担保。
> 浏览器告警原因：操作系统或浏览器信任库中不存在该自签名根证书，无法验证证书合法性，无法排除中间人伪造风险，判定通信不安全。
> 适用场景：内网开发测试、企业内部系统、离线私有环境；公网对外网站禁止使用。
...
> 答：证书链作用：构建分层信任体系（根 CA→中间 CA→业务证书），分层分担签发压力、隔离安全风险，实现信任规模化管理。
> 根 CA 不直接签发终端证书的原因：
> (1)安全风险：根 CA 私钥是整个信任体系的最高信任源，极少离线保存，不能频繁联网签发证书，泄露则全网信任崩塌；
> (2)规模限制：全球海量网站，单一根 CA 无法承载全部签发业务；
> (3)权限隔离：中间 CA 可分配给不同子机构，单独管控域名签发权限，故障仅影响局部，不会全局瘫痪。
...
> 答：根证书自签名却可信的原因：各大操作系统、浏览器厂商人工审核、预植入可信根 CA 证书到本地信任库，人为建立初始信任锚点。
> 系统或浏览器信任判定逻辑：
> (1)厂商安全团队审核 CA 机构资质、安全规范；
> (2)通过审核的根证书内置到系统信任列表；
> (3)访问网站时，证书链能追溯到列表内根 CA，则判定可信；不在列表则弹出安全警告。
...
> 答：pathlen:0含义：该中间 CA不能再签发下级 CA 证书，仅能签发终端服务器 / 用户证书，限制证书链长度最多 2 层（根→中间→终端）。
> pathlen:1区别：允许该 CA 再签发一层下级中间 CA，完整证书链最长为 3 层（根→一级中间→二级中间→终端）；pathlen 数值代表向下还能嵌套的 CA 层级上限。
> 设计目的：限制证书链深度，缩小私钥泄露后的风险扩散范围，管控 PKI 层级结构。
...
> 答：过期后果：浏览器拦截网站，弹出安全风险警告，HTTPS 通信直接阻断，用户无法正常访问站点。
> 设置有效期的原因：
> (1)密钥存在泄露风险，定期强制更换公钥 / 私钥，缩小泄露窗口期；
> (2)网站域名、企业主体信息会变更，到期重签同步更新信息；
> (3)缩减证书吊销列表 (CRL)、OCSP 数据体积，提升校验效率。
> Let’s Encrypt 仅 90 天的原因：
> (1)缩短生命周期，大幅降低私钥被盗、证书被盗用的风险窗口；
> (2)强制站点部署自动续期脚本，淘汰手动运维、长期不维护的老旧站点；
> (3)配合 ACME 自动化协议，低成本实现自动续期，无人工成本压力。
...
> 答：需要吊销证书场景：网站私钥泄露、域名所有权变更、企业注销、证书信息错误、CA 机构判定证书存在欺诈风险。
> CRL 与 OCSP 区别：
> CRL（证书吊销列表）工作方式为：CA 定期发布完整吊销证书清单，客户端完整下载。列表随吊销数量膨胀，下载流量大,存在更新间隔，吊销信息非实时,大列表下载慢、缓存过期风险。
> OCSP（在线证书状态协议）工作方式为：客户端单条在线查询单张证书状态。单次仅查询目标证书，流量极小，实时查询，状态即时同步，依赖 OCSP 服务器，断网无法查询。
...
> 答：现代证书必备 SAN 扩展原因：早期仅依靠 CN 字段绑定域名，标准限制 CN 仅支持 1 个域名；SAN 扩展允许一张证书绑定多个域名、通配符域名，大幅降低多站点证书运维成本。
>SAN 与 CN 区别：
>CN：仅兼容老旧浏览器，标准规定不再作为域名校验依据，仅作展示；
>SAN：现代浏览器唯一校验域名的扩展字段，支持多条普通域名、通配符域名、IP，是合法多域名承载标准。
...
> 答：指纹计算方式：对完整证书二进制原文做 SHA1/SHA256 哈希运算，输出十六进制指纹串，唯一标识单张证书。
>证书钉扎（Certificate Pinning）：客户端 / 服务端预先配置信任证书指纹，仅接受指纹匹配的证书；即使信任根 CA 被劫持、恶意证书签发，指纹不匹配则直接阻断连接，抵御中间人攻击。
>用途：人工核对证书真伪、钉扎校验、日志标识证书、快速比对两张证书是否完全一致。
...
>*.example.com可匹配域名：www.example.com、mail.example.com、blog.example.com等一级子域名。
>局限性：
>(1)仅匹配一级子域名，无法匹配多级子域名（如api.www.example.com不匹配）；
>(2)不匹配裸域名example.com，需额外在 SAN 中单独添加；
>(3)私钥泄露会导致该域名下全部子域名同时失效，风险范围更大；
>(4)部分 CA 对通配符证书有额外域名所有权校验限制。
---
...
> 答：定义：一套公开、可审计的证书日志系统，所有公网可信 CA 签发的 HTTPS 证书必须提交至公开 CT 日志服务器。
>解决的核心问题：
>(1)防止 CA 违规、私自给他人域名签发恶意证书；
>(2)域名所有者可检索日志，发现针对自身域名的非法证书；
>(3)浏览器校验证书必须存在 CT 日志记录，拦截未备案的欺诈证书；
>(4)安全研究员可全局审计所有签发记录，及时发现 CA 安全漏洞。
...
> 答：免费签发原理：非盈利机构 ISRG 运营，依托 ACME 自动化协议，全程无人工审核、零人工成本，依靠捐赠维持服务，面向全网开放免费域名证书。
>ACME 域名控制权验证方式：
>(1)HTTP-01：在网站根目录放置指定验证文件，ACME 服务器访问文件验证域名归属；
>(2)DNS-01：域名 DNS 添加指定 TXT 解析记录，校验 DNS 所有权，支持通配符证书；
>(3)短期 90 天 + 自动续期原因：同第 6 题，缩短泄露风险窗口；自动化脚本消除手动续期负担，强制站点实现自动化运维。
>结合3n7er.space证书链：站点证书由中间 CA E7 签发，根证书 ISRG Root X1 预装在浏览器信任库，整条链路符合 CT 日志规范，浏览器默认信任。
...
> 答：浏览器信任差异原因
>3n7er.space证书：签发根 CA 为 ISRG Root X1，预装在 Windows、Chrome、Firefox 等系统信任库，存在可信锚点，浏览器自动信任；
>实验自建私有证书：根 CA 为本地自行生成，未导入系统信任列表，浏览器无信任锚，判定证书不可信，弹出告警。
>相同点
>证书格式：均遵循 X.509 v3 标准，包含 Subject、Issuer、有效期、公钥、SAN、Basic Constraints 等标准扩展；
>证书链结构：统一采用分层三级结构：根 CA→中间 CA→终端服务器证书，依靠 pathlen 管控层级。
>不同点
>信任根：公网 PKI 根为厂商预装全球可信 CA；私有 PKI 根为本地自签名证书，仅本机手动导入后才信任；
>签发主体：公网由合规第三方商业 / 公益 CA 签发；私有 PKI 由个人 / 企业自建 CA 签发；
>使用范围：公网证书适用于全网公开站点；私有证书仅内网、测试环境可用；
>吊销 / CT 机制：公网强制接入 CT 日志、支持 OCSP/CRL 吊销；私有 PKI 无强制 CT，吊销需自行搭建 CRL 服务。
