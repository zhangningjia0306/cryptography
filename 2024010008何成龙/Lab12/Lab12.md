...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | Fingerprint=XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX|
| SHA-1 指纹（完整） |Fingerprint=XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX |

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |ISRG Root X1（全球预装公网根 CA）|自建 My Root CA（本地私有根 CA） |
| 浏览器默认是否信任 |是，操作系统预装根证书| 否，必须手动导入根证书到系统信任库|
| 证书签发者是否为第三方 CA |是，非营利公共可信 CA Let’s Encrypt | 否，本地自建内部 CA，无全球公信力|
| 适合的使用场景 | 公网对外网站、面向全网互联网服务| 企业内网、本地开发测试、局域网内部系统、离线设备|
...
> 答：数字证书本质：遵循 X.509 v3 标准，由可信第三方 CA 私钥签名的绑定数据包，结构 = 实体身份信息 + 实体公钥 + CA 数字签名。
核心解决问题：公钥可信性难题。单纯数字签名仅能校验消息完整性、发送方不可抵赖，无法证明公钥真正属于目标实体。
仅数字签名的缺陷：中间人攻击者可同步替换目标公钥、消息、签名，客户端无法识别伪造公钥；数字证书通过权威 CA 对公钥与身份绑定签名，建立层级信任链，杜绝公钥劫持中间人攻击。
...
> 答：自签名证书：证书颁发者 (Issuer) 与持有者 (Subject) 完全一致，实体使用自身私钥完成签名，无第三方可信 CA 背书。
浏览器警告原因：不存在系统预装的可信信任锚，无法密码学验证公钥归属；中间人可随意伪造任意自签名证书，存在高风险中间人劫持漏洞。
适用场景：本地开发调试、企业内网管理系统、局域网离线设备；禁止用于面向公网普通用户的对外服务。
...
> 答：多层证书链基于安全隔离、运维成本、权限管控三大设计目标：
根 CA 私钥长期离线冷存储，极少启用；若直接签发海量网站证书会频繁动用根私钥，极大提升密钥泄露风险，中间 CA 分担日常证书签发工作。
风险隔离：若单台中间 CA 私钥泄露，仅需吊销该中间 CA，不会破坏全局根 CA 信任体系；根 CA 一旦泄露，全网所有依赖该根的服务全部失效。
业务分工：不同中间 CA 可划分业务线（域名证书、代码签名、设备证书），实现最小权限分离。
pathlen 路径长度限制约束链条最大深度，防止攻击者无限延伸伪造多级 CA，缩小攻击面。
...
> 答：对根 CA 的信任不属于密码学验证，来源于厂商人工商业安全审核：操作系统、浏览器厂商严格审计 CA 机构的运维规范、密钥安全机制、应急处置流程，审核通过后将其根证书预装至系统全局信任库。
准入标准：CA 机构必须符合国际 WebTrust 审计规范；若 CA 出现违规签发伪造证书、密钥泄露等安全事故，会被各大厂商批量从信任列表移除（典型案例 DigiNotar）。
客户端校验逻辑：遍历证书链向上追溯，若链条终点存在于预装信任根列表，则整条证书链判定为可信。
...
> 答：pathlen:0：该 CA 仅允许签发终端实体证书（服务器 / 客户端证书），禁止签发任何下级 CA 证书，限制证书链最长为两层（根 CA→中间 CA→业务证书）。
若改为pathlen:1：允许该中间 CA 再签发一层次级中间 CA，证书链最长可延伸三层（根→一级中间→二级中间→业务证书）。
安全作用：最小权限约束，防止中间 CA 私钥泄露后，攻击者搭建多层伪造 CA 批量劫持域名。
...
> 答：过期后果：浏览器、客户端直接阻断 HTTPS 连接，弹出证书过期高危安全警告，用户无法正常访问网站。
设置有效期的意义：
公私钥长期存储存在暴力破解、泄露风险，有效期强制定期轮换密钥，缩短风险窗口；
企业注销、域名转让、业务下线后，过期证书自动失效，降低长期遗留安全隐患；
简化证书吊销管理，过期证书天然失效，无需永久维护吊销列表。
Let’s Encrypt 90 天短周期原因：
依托 ACME 自动化协议可脚本无感自动续期，无人工运维成本；
大幅缩短密钥泄露可被滥用的时间窗口；
快速清理废弃域名证书，减少无效证书留存风险。
...
> 答：需要吊销证书场景：服务器私钥泄露、域名所有权变更、企业注销、证书误签发、服务器遭受入侵控制。
CRL（证书吊销列表）：CA 定期生成完整吊销证书清单文件，客户端定时下载本地批量校验；缺点：文件体积大、更新存在延迟、带宽开销高。
OCSP（在线证书状态协议）：客户端向 CA 服务器单条查询单张证书实时状态；优点实时性高、流量开销小；缺点依赖网络连通，存在 OCSP 中间人劫持隐患。
...
> 答：SAN (Subject Alternative Name) 作用：单张证书可绑定多个独立域名，支持多域名、通配符域名；现代浏览器仅依靠 SAN 完成 TLS 域名匹配校验，是强制标准扩展。
核心区别：
CN（通用名称）：早期单域名标识字段，现代浏览器完全忽略 CN，不再用于域名合法性校验；
SAN：X.509 v3 强制扩展，存储证书全部合法域名，HTTPS 域名匹配仅校验 SAN 列表；
无 SAN 扩展的证书会被浏览器判定为域名不匹配，直接拦截访问。
...
> 答：指纹计算流程：将完整证书原始 DER 二进制编码进行哈希运算（SHA1/SHA256），再格式化为十六进制分隔字符串，每张合法证书指纹唯一。
证书钉扎 (Pinning)：客户端 / APP 本地预先内置合法服务器证书指纹，建立 TLS 连接时比对服务端返回证书指纹；即使攻击者持有其他合法 CA 签发的伪造证书，指纹不匹配会直接断开连接，抵御 CA 被攻陷后的大规模中间人攻击。
...
> 答：可匹配域名：仅一级子域名，如api.example.com、blog.example.com、admin.example.com。
局限性：
无法匹配多级子域名（dev.api.example.com不支持）；
通配符仅作用于域名最左侧分段，不能单独匹配根域名example.com；
安全风险：私钥一旦泄露，该域名下所有一级子域名全部面临劫持风险。
...
> 答：CT：一套公开、可审计的全局证书日志系统，CA 签发任意域名证书后必须写入公共 CT 日志，日志全网开放检索。
解决问题：防御恶意 CA 违规签发伪造域名证书（如 DigiNotar 安全事件）；任何人可检索对应域名名下所有已签发证书，快速发现非法伪造证书；现代浏览器强制校验证书存在 CT 日志记录，无记录则拦截访问。
...
Let’s Encrypt 免费模式：非营利公益机构，依靠科技企业赞助覆盖运营成本；ACME 全自动化协议消除人工审核、纸质流程，实现零人工成本签发证书。
ACME 域名验证三种方式：
HTTP-01：域名根目录放置指定验证文件，CA 公网访问校验域名所有权；
DNS-01：域名 TXT 解析记录写入随机验证串，通过 DNS 解析完成所有权校验；
TLS-ALPN-01：特殊自定义 TLS 握手完成域名所有权校验。
短有效期 + 自动续期原因：自动化脚本消除人工续期负担；缩短密钥风险暴露周期；及时清理废弃域名证书；依托 ACME 定时脚本实现后台无感自动更新证书。
...
> 答：信任差异根本原因
Let’s Encrypt 根证书 ISRG Root X1 预装在全部操作系统、浏览器信任库；自建私有根 CA 未经过厂商安全审核，不会默认内置，浏览器无可信信任锚，判定证书不安全。
相同点
格式标准统一：均遵循 X.509 v3 规范 PEM 证书，包含版本、序列号、有效期、SAN、密钥用途全套标准字段；
链条结构一致：三层信任链（根 CA→中间 CA→终端服务器证书），逐级签名完成密码学验证；
密码学逻辑一致：使用 SHA 哈希 + 非对称加密完成证书签名，用于 TLS 握手公钥加密。
不同点
信任根：公网证书根为厂商预装全球可信 CA；私有证书根为自建自签名 CA，无外部公信力；
签发主体：公网由合规第三方商业 / 公益 CA 签发；私有为本地自建 CA，无国际安全审计；
默认信任状态：公网证书开箱即用自动信任；私有证书必须手动导入根证书才能被客户端信任；
适用范围：公网面向全球互联网用户；私有仅适用于局域网、本地测试环境。
