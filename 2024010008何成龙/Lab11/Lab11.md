...
| :--- | :------- |
| 签名 RSA 密钥长度（位） | 2048|
| 签名私钥 `signature_private_key.pem` 文件大小（字节） |1700 |
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | 460|
| 签名公钥指数 e 的值 | 65537（0x10001）|
| 签名公钥模数 N 的前 16 位十六进制 | 00:bf:6d:98:2d:d9:e6:57|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） | 1700|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：邱龙菲，学号：2024010007|
| 密文文件 encrypted_message.bin 大小（字节） | 256|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） | 9b15d8a1cc410b57df05e4bf7ba39c4b6ee757bff292458fa775971de06a5b|
...
> 答：三大保障：完整性、身份认证、不可否认
完整性
含义：消息在传输过程中没有被篡改、修改。
实验体现：一旦密文被改动，SHA256 哈希值就会变化，签名校验直接返回Verification failure，无法通过验证。
身份认证（防伪造）
含义：能够确认消息确实来自发送方，别人无法冒充。
实验体现：只有发送方持有签名私钥，只有用该私钥才能生成合法签名；接收方用公钥核验，冒充者没有私钥，造不出合法签名。
不可否认
含义：发送方事后无法抵赖自己发送过这条消息。
实验体现：签名由发送方独有的私钥生成，私钥只有本人保管，第三方可以核验签名归属，发送方不能否认。
...
> 答：1.RSA 明文长度有限制
2048 位 RSA 最多只能加密 245 字节数据，如果密文很长，无法直接签名；哈希把任意长度文件压缩成固定 256 比特摘要，解决长度超限问题。
2.提升运算速度
RSA 大数运算很慢。对几百字节的简短摘要签名，远比对几 MB 大文件直接签名效率高得多。
3.防止签名重复攻击，保障安全性
哈希具有雪崩效应，原文改动哪怕一个字符，摘要完全改变，避免攻击者利用原文微小变化伪造有效签名。
...
> 答：2048÷8=256 字节
所以本次实验里signature.bin大小为 256 字节。
若使用 4096 位 RSA 密钥：
4096÷8=512 字节
签名文件长度将变为 512 字节。
...
> 答：抗原像性（单向性）
含义：给定哈希值，无法在合理时间内反推出原始明文。
安全缺陷：若不满足，攻击者可以从摘要还原原文，伪造带合法签名的虚假消息。
抗第二原像性（弱无碰撞）
含义：给定原文 M，找不到另一条不同原文 M'，使得两者哈希值相等。
安全缺陷：攻击者可以构造另一条不同内容的文档，替换原文后签名依然有效，篡改内容不破坏签名。
抗碰撞性（强无碰撞）
含义：无法找到两条完全不同的消息 M1、M2，使得哈希值相同。
安全缺陷：攻击者预先制造两条哈希一致的文档，一条正常签名，另一条用来实施诈骗，实现签名移植攻击
...
> 答：淘汰原因：SHA-1 已经被破解，不再具备强抗碰撞性，存在公开可行的碰撞算法，无法保障数字签名防篡改，因此密码标准不再推荐用于签名场景。
攻击影响：攻击者可以构造两份内容完全不同但 SHA-1 哈希一致的文件。一份让用户签署合法签名，另一份恶意文件可以直接复用该签名，实现 “签名移植”。
攻击举例：
攻击者制作两份 PDF，A 是普通合同，B 是高额欠条，两份文件 SHA-1 哈希值完全一样。用户对 A 文件完成数字签名，攻击者把签名剥离出来附加到 B 欠条上，接收方校验时哈希匹配，签名就会被判定为合法。
...
签名公钥：接收方用来验证签名真伪。
加密密钥对
加密公钥：用来加密明文，本次实验用它加密消息。
加密私钥：用来解密密文，只有接收方持有。
攻击者拿到公钥的权限：
✅ 可以做：核验签名、加密消息；
❌ 不能做：伪造合法签名、解密密文、推导私钥。
公钥只用于核验与加密，无法逆向算出私钥，因此不会破坏机密性与不可否认性。
...
> 答：消息加密：使用接收方公钥加密，目标是保证机密性，防止第三方偷看内容。
数字签名：使用发送方私钥对摘要签名，目标是保证完整性、身份认证、不可否认，防止内容被篡改、防止冒充发送者。
二者解决的问题
加密：解决 “消息不能被外人看见”。
签名：解决 “消息没被改、确实是此人发的、事后不能抵赖”。
不能用加密代替签名的原因
加密是公钥操作，任何人都可以发起加密，无法证明发送者身份，也无法实现不可否认；加密只能保密，不能防篡改，因此无法替代数字签名
...
> 答：不可否认性：消息发出后，发送方无法否认自己发送过这条信息，第三方可以仲裁证据。
数字签名可以实现：
签名依靠非对称密钥，只有发送方独自持有私钥，签名只能由本人生成，公钥全网公开可核验，第三方能够判定签名归属，因此具备不可否认性。
MAC 无法实现：
MAC 采用对称密钥，通信双方共用同一把密钥，收发双方都能生成和篡改验证码。一旦出现纠纷，无法证明是发送方还是接收方伪造，没有中立仲裁依据，所以不支持不可否认。
...
> 答：软件安装包签名
开发商用私钥给安装包签名。用户下载后用开发者公钥校验，防止文件被木马篡改，保证软件来源正规、文件未被劫持。
电子合同、电子签章
对 PDF 合同进行数字签名，确保合同文本不能被事后修改，并且能够锁定签署人身份，满足法律上的防抵赖要求。
代码提交（Git 代码签名）
程序员用私钥对代码提交记录签名，服务器使用公钥核验，避免账号被盗后他人冒用身份提交恶意代码。
...
> 答：数字签名本身无法证明签名的具体时间，签名只保证内容与签署人，不带可信时间。
解决方案：接入可信时间戳服务（TSA）。
流程：把签名文件发给权威时间戳服务器，服务器加盖官方标准时间并对整体再次签名。
接收方核验时，不仅校验文件签名，同时校验 TSA 权威时间，证明签名真实发生在该时刻，无法事后篡改签署日期，杜绝倒签文件。
...
> 答：本实验选用 2048 位 RSA：兼顾安全性与运行速度，是当前工程环境下的主流标准配置。
1024 位密钥：已经可以被暴力分解，不再安全，被密码标准淘汰，不适合正式业务使用。
4096 位密钥：安全性更高，抗破解能力更强，但大数模幂运算速度会大幅下降，CPU 开销成倍上升。
权衡结论：普通业务优先使用 2048 位；金融、政务等高安全场景可以选用 4096 位；1024 位已禁止用于正式签名与加密。
...
> 答：这种多重签名是有实际意义的，形成了签名链。
适用场景：
多级审批：员工签署文件 → 部门负责人二次签署 → 公司法务终审签署，每一级都对前一级签名进行背书，形成审批链路。
证书信任链：用户证书由二级 CA 签名，二级 CA 又由根 CA 签名，层层背书，构建信任体系。
项目多层审核：研发提交代码签名，运维再对提交记录二次签名，保证每一环都可追溯、不可篡改。
