...
| 证书有效期（起始-结束） |Apr 10 03:12:45 2025 GMT - Jul 09 03:12:44 2025 GMT |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） | C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Security Lab, CN=My Root CA 2026|
| 证书颁发者（Issuer） | 	C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Security Lab, CN=My Root CA 2026|
| 主题和颁发者是否相同 | 是|
| 证书有效期（天数） | 	3650|
| 公钥长度（位） |4096 |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） | C=CN, ST=Yunnan, L=Yuxi, O=My Intermediate CA, OU=Issuer Dept, CN=My Intermediate CA E7|
| 证书颁发者（Issuer） |	C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Security Lab, CN=My Root CA 2026 |
| pathlen 值 | 0|
| 证书有效期（天数） | 1825|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） |C=CN, ST=Yunnan, L=Yuxi, O=YXOR TEAM, OU=Web Server, CN=www.myserver.test |
| 证书颁发者（Issuer） | C=CN, ST=Yunnan, L=Yuxi, O=My Intermediate CA, OU=Issuer Dept, CN=My Intermediate CA E7|
| CA 字段的值 | CA:FALSE|
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | 	95:5F:60:0C:C5:9A:88:F9:F1:48:AC:C5:1E:B6:53:F1:7E:95:E0:14:BF:AF:8C:96:AD:25:18:A4:17:86:60:F2|
| SHA-1 指纹（完整） | 	自行执行 -sha1 命令复制输出填入|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |操作系统 / 浏览器内置全球可信根 CA（ISRG Root X1） | （ISRG Root X1）	自建离线私有根 CA（My Root CA 2026）|
| 浏览器默认是否信任 |	是，系统预装根证书，开箱信任 |	否，需要手动导入根证书到系统信任库才会信任 |
| 证书签发者是否为第三方 CA | 是，Let’s Encrypt 公开第三方可信 CA| 否，自建企业内部 CA，无第三方背书|
| 适合的使用场景 |公网对外网站、面向互联网所有用户的公开服务 |企业内网、内部系统、测试环境、设备内网通信、不对外公开的业务 |
...
> 答：数字证书本质是由可信第三方 CA 对实体公钥、身份信息进行签名的电子凭证，将公钥与实体身份绑定。
解决的核心问题：公钥分发时的中间人攻击，解决 “如何确认公钥属于目标实体” 的身份信任问题。
仅数字签名不足的原因：数字签名只能证明数据未被篡改、签名者持有对应私钥，但无法验证签名者身份，无法确认公钥归属；证书引入权威 CA 背书，建立可信身份链条，解决公钥身份确权问题。
...
> 答：
自签名证书：实体使用自身私钥给自己签发证书，签发者与使用者为同一主体，无第三方 CA 背书。
浏览器告警原因：浏览器内置可信根列表中不存在该自签名根，无法验证身份合法性，存在中间人劫持风险，因此标记不安全。
适用场景：企业内网、本地开发测试、封闭内网设备通信、仅内部人员访问且可手动导入根证书的环境，不用于公网对外服务
---
...
> 答：1.安全分层隔离：根 CA 离线严格保管（最高信任源），不直接暴露在业务网络；中间 CA 承担日常签发终端证书工作，即使中间 CA 泄露，可单独吊销，不会动摇根信任根基，降低根私钥泄露风险。
2.分级管理、权限拆分：大型机构可按业务 / 地区划分多条中间 CA，分别管理网站、设备、员工证书，精细化管控证书签发权限。
3.扩展性更强：若根 CA 直接签发海量终端证书，证书管理、吊销、更新工作量巨大；分层架构分散签发压力，便于大规模运维。
若根 CA 直接签发所有终端证书，根私钥高频在线使用，泄露后整个信任体系完全崩溃，且无法分层管控、运维成本极高。
...
> 答：1.信任根 CA 的原因：根 CA 是整个信任体系的源头，自签名代表它没有上级证书，其信任不依赖其他证书背书。我们信任它并非依靠密码学校验，而是依靠人工可信渠道预先分发。
2.系统与浏览器信任逻辑：操作系统、主流浏览器厂商会严格审核 CA 机构资质、安全规范、审计记录，通过后将该根证书内置到系统 / 浏览器自带的可信根证书库。用户打开网站时，程序会遍历内置根库，若证书链能匹配库内根证书，则判定可信；未入库的自签名根一律告警。
...
> 答：1.pathlen:0含义：该中间 CA 最多再签发0 层下级 CA，只能签发终端实体证书（网站、设备证书），不能再创建二级中间 CA。
2.pathlen:1区别：允许向下签发 1 层中间 CA，二级中间 CA 无法再签发 CA 证书，证书链最多三层：根 CA → 中间 CA (pathlen=1) → 二级中间 CA → 终端证书。
3.核心作用：约束证书链最大深度，限制 CA 分层数量，防止无限嵌套中间 CA 带来的安全风险
...
> 答：1.证书过期后果：浏览器直接拦截网站，弹出安全告警，禁止 HTTPS 访问；无法完成身份校验，加密通道建立失败。
2.设置有效期的原因：
① 私钥存在泄露风险，短期轮换证书可缩小泄露后的风险窗口；
② 域名、企业主体信息会变更，到期强制更新保证信息准确；
③ 配合证书吊销机制，过期证书无需长期维护吊销列表。
3.Let’s Encrypt 仅 90 天有效期原因：
① 缩短风险周期，私钥泄露影响范围更小；
② 推动自动化续期（ACME 协议），降低人工运维成本；
③ 减少长期证书被盗用、冒用的安全隐患；
④ 快速淘汰废弃域名证书，提升全网证书安全性
...
CRL 与 OCSP 区别
CRL（证书吊销列表）
CA 定期生成完整的吊销证书清单，客户端下载完整列表本地校验；缺点是文件随吊销证书增多体积巨大、存在更新延迟，占用带宽。
OCSP（在线证书状态协议）
客户端向 OCSP 服务器单条查询目标证书实时状态，只获取本条证书结果；优点轻量化、实时性强；缺点会产生额外网络请求，存在隐私泄露风险。
---
...
> 答：现代证书必须使用 SAN 的原因
早期仅依靠 CN 绑定域名存在浏览器兼容缺陷，现代浏览器已不再信任仅靠 CN 匹配域名的证书，强制读取 SAN 扩展；同时一张证书可绑定多个域名，实现多站点共用单张证书，降低运维成本。
SAN 与 CN 区别
1.CN（通用名）：旧标准字段，仅能填写一条域名，现已不被浏览器优先校验；
2.SAN（主题备用名称）：标准扩展字段，支持填写多条 DNS 域名、IP 地址等，是当前域名匹配的唯一有效字段。
...
> 答：证书指纹计算方式
对完整 DER 编码证书原文做哈希运算（SHA256/SHA1），输出分段十六进制字符串，唯一标识该证书，证书微小改动都会导致指纹完全变化。
证书钉扎（Certificate Pinning）
将服务器证书 / 公钥的指纹预先硬编码到客户端程序、网站配置中；客户端连接时校验服务端证书指纹与内置指纹是否一致。
作用：抵御中间人劫持，即使中间人伪造可信 CA 证书，指纹不匹配也会直接拦截连接，强化身份校验安全。
...
> 答：可匹配域名（以 *.example.com 为例）
只能匹配一级子域名：
a.example.com、www.example.com、api.example.com 等。
局限性
无法匹配根域名 example.com；
不支持多级子域名，不能匹配 a.b.example.com；
安全风险：私钥泄露后，该域名下所有一级子域名全部失效；
一张证书仅支持单一层级通配，无法跨多级域名
...
> 答：定义
CT 是一套公开日志系统，所有 CA 签发的公网证书必须写入公开 CT 日志，浏览器会校验证书是否存在 CT 日志凭证（SCT）。任何人可查询日志，查看某域名下所有已签发证书。
解决的核心问题
防止 CA 违规、秘密签发伪造域名证书（中间人攻击常用手段）；域名所有者可随时检索日志，发现未授权的非法证书；
给浏览器提供校验依据，强制证书必须公开可查，杜绝隐蔽违规证书；
监管机构、域名管理员可实时审计所有证书签发记录，快速发现泄露、劫持风险。
...
> 答：一、免费签发的底层逻辑（以 3n7er.space 证书链为例）
信任链条：根证书 ISRG Root X1（系统预装可信根）→ 中间 CA R3 → 终端域名证书 3n7er.space，整条链路被主流浏览器信任；
运营模式：由非营利机构 ISRG 运营，资金依靠企业捐赠，无商业收费诉求；
自动化签发：基于 ACME 协议全流程自动化，无需人工审核，极大降低运营成本，因此可以免费对外签发域名证书。
二、ACME 域名所有权验证方式
HTTP-01：客户端在网站根目录放置指定随机验证文件，ACME 服务器通过http://域名/.well-known/acme-challenge/访问文件，匹配内容即证明拥有网站服务器控制权；仅适用于 80 端口可公网访问的 HTTP 站点。
DNS-01：在域名 DNS 解析添加_acme-challenge前缀 TXT 记录，CA 查询 TXT 记录校验，可用于通配符域名、无公网 80 端口的内网服务。
三、短期 90 天有效期 + 自动续期的原因
缩短风险窗口：若私钥泄露、域名被盗，90 天短证书会快速失效，降低攻击窗口期；
适配 ACME 自动化：工具可脚本自动续期，无需人工操作，短有效期不会增加运维负担；
淘汰废弃域名：长期未续期的证书自动过期，减少无效证书堆积，提升全网证书安全；
倒逼自动化部署：强制站点接入自动续期工具，避免长期静态证书带来的安全隐患。
...
> 答：一、信任差异原因
浏览器默认信任 3n7er.space 证书：
Let’s Encrypt 根证书 ISRG Root X1 被操作系统、浏览器厂商严格审核后，内置在系统可信根证书库，证书链可逐级校验到可信根，无安全告警。
不信任实验自建证书：
自建根 CA 为自签名证书，未录入系统信任库，浏览器无法完成可信链校验，判定存在中间人劫持风险，弹出安全警告。
二、相同点
证书格式：均遵循 X.509 v3 标准，包含公钥、主体、颁发者、有效期、SAN、Basic Constraints 等标准扩展字段；
证书链结构：都采用根 CA→中间 CA→终端网站证书三级分层架构，通过 pathlen 约束证书签发层级；
密码算法：均使用 RSA 非对称加密、SHA256 哈希签名，支持 serverAuth 服务器身份用途。
三、不同点
信任根：
真实 PKI：根为全球公认第三方可信根，系统预装；
私有 PKI：自建离线根，需手动导入系统才会被信任；
签发主体：
真实 PKI：合规第三方公益 CA，受浏览器、监管审计；
私有 PKI：企业 / 个人自建，无第三方权威背书；
使用场景：
真实 PKI：公网对外开放网站；
私有 PKI：内网、测试环境、内部业务系统；
证书有效期：
真实 PKI：90 天短期；
实验私有证书：自定义长期（根 10 年、中间 5 年、网站 1 年）；
合规要求：
真实 PKI：强制 CT 日志、吊销机制、域名所有权校验；
私有 PKI：无强制公网合规约束，流程自定义。
