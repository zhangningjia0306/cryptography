...
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | 460字节|
| 签名公钥指数 e 的值 |65537 |
| 签名公钥模数 N 的前 16 位十六进制 | e4:27:1b:47:ab:2f:7e:de|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） | 1.7K|
| 加密公钥 `encryption_public_key.pem` 文件大小（字节） |460字节 |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：陆永林，学号：2024010026|
| 密文文件 encrypted_message.bin 大小（字节） | 256 字节|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） | b6e033a753b09eeef1ccc15fa0242ac8ab1afb9486abbed7cc7949367858aa7|
| 密文 SHA-256 哈希值长度（十六进制字符数） |64 个十六进制字符 |
...
> 答：
(1) 身份认证（Authentication）
含义：证明消息确实来自持有签名私钥的发送者，而不是其他人冒充的。接收者可以确信消息的来源是可信的。
实验实现：只有持有 signature_private_key.pem 私钥的人才能生成有效的签名。验证时，使用对应的 signature_public_key.pem 公钥进行验证。在实验中，Verified OK 的结果证明密文确实来自持有签名私钥的发送者。
(2) 完整性保护（Integrity）
含义：确保消息在传输过程中未被篡改，接收方收到的内容与发送方发送的内容完全一致。
实验实现：在任务四中，我们对原始密文验证签名得到 Verified OK；但当我们向密文文件末尾追加一个字符 'x' 后，签名验证失败（Verification failure）。这证明哪怕密文只发生 1 个字节的改动，签名也会失效，从而保护了消息的完整性。
(3)不可否认性（Non-repudiation）
含义：发送者无法否认自己发送过该消息，因为只有他持有生成签名所需的私钥。
实验实现：签名是用 signature_private_key.pem 生成的，这个私钥只有发送者持有。如果第三方拿到签名和公钥可以验证签名的有效性，但无法伪造签名。因此发送者不能抵赖说自己没有发送过这个消息。在法律场景中，数字签名可作为证据使用。
...
> 答：
> (1) 效率问题
RSA 非对称加密算法的运算速度远低于哈希函数。直接对一个 1GB 的文件进行 RSA 签名在实践上不可行，而哈希函数可以快速将任意长度的数据压缩为固定长度（如 SHA-256 输出 256 位 = 32 字节）的摘要，然后只需要对这个摘要进行签名。
在实验中，计算密文的 SHA-256 哈希只需要瞬间完成，而如果要直接对密文进行 RSA 签名，虽然密文只有 256 字节，但对于更大文件，这种差异会更加明显。
> 
> (2) 输入长度限制
RSA 运算有数据长度限制，一次只能处理不超过模数长度的数据块。对于 2048 位的 RSA 密钥，最多只能直接处理 256 字节的数据（且还要扣除填充空间）。
先计算哈希值可以将任意大小的数据压缩为固定长度（32 字节），完全满足 RSA 的输入要求。哈希函数起到了"数据压缩器"的作用。
> 
> (3) 标准化与安全性
哈希值为签名算法提供了统一的接口——无论消息是 1KB 还是 1GB，签名的输入都是固定长度的哈希值。
哈希函数的性质（抗原像性、抗碰撞性）保证了签名无法被伪造或复用。如果直接签名，可能会受到长度扩展攻击等安全问题的影响。
此外，哈希函数的标准化使得签名算法可以独立于消息格式，提高了系统的兼容性和安全性。
> 
---
...
> 答：
>(1)RSA 签名运算公式为：σ = H(m)^d mod N，其结果是一个小于 N 的非负整数。这个结果以二进制形式存储时，其长度正好等于 N 的字节长度（因为结果的范围是 0 到 N-1）。
2048 位 RSA 密钥：N 的位长度为 2048 位，即 2048/8 = 256 字节
实验验证：signature.bin 文件大小确实是 256 字节，与理论完全一致
>
> (2)4096 位 RSA 密钥：
N 的位长度为 4096 位，即 4096/8 = 512 字节
如果使用 4096 位的 RSA 密钥，签名长度将是 512 字节
...
> 答：
>(1)抗原像性（Preimage Resistance）
含义：给定一个哈希值 h，在计算上无法找到一个消息 m 使得 H(m) = h。即"单向性"。
安全问题：如果哈希函数不满足抗原像性，攻击者知道签名值 σ = H(m)^d mod N 后，可以反推出哈希值 H(m)，然后尝试构造一个消息 m' 使得 H(m') = H(m)。这样攻击者就能用原来的签名去验证另一个消息，从而伪造有效的签名。
> 
> (2) 抗第二原像性（Second Preimage Resistance）
含义：给定消息 m1，在计算上无法找到另一个不同的消息 m2（m2 ≠ m1）使得 H(m1) = H(m2)。
安全问题：如果不满足这个性质，攻击者拿到一个合法的消息-签名对 (m1, σ)，可以找到另一个消息 m2 使得 H(m2) = H(m1)。由于签名只依赖于哈希值，σ 对 m2 也同样有效，攻击者就可以用 m1 的签名来冒充 m2 被签名。
> 
>(3) 抗碰撞性（Collision Resistance）
含义：在计算上无法找到任意两个不同的消息 m1 和 m2（m1 ≠ m2）使得 H(m1) = H(m2)。
安全问题：如果不满足这个性质，攻击者可以事先准备两个内容不同但哈希值相同的消息——一个是看起来无害的版本（让签名者签署），另一个是恶意的版本。签名者签了无害版本后，攻击者可以将签名替换到恶意版本上，由于两个版本哈希值相同，签名验证会通过。这正是 SHA-1 被攻击所暴露的问题。
>
>
...
> 答：
>为什么 SHA-1 不再被推荐用于数字签名：
SHA-1 的输出长度为 160 位（20 字节），理论上需要 2^80 次运算才能找到碰撞。但随着计算能力的提升和密码分析技术的发展，这个安全边际已经不够。2017 年，Google 和 CWI Amsterdam 联合公开了名为 "SHAttered" 的实际碰撞攻击：两个内容不同但 SHA-1 哈希值完全相同的 PDF 文件。
>
> 对数字签名的影响：
>攻击者可以利用这个漏洞进行伪造签名攻击。具体例子：
攻击者准备两个文件：
文件 A：一份看起来正常的合同，内容是"我同意支付 100 元"，愿意让对方签名
文件 B：一份恶意的合同，内容是"我同意支付 10000 元"
攻击者精心构造两个文件（通过填充垃圾数据），使得它们的 SHA-1 哈希值完全相同（利用碰撞攻击）。
攻击者让签名者（受害者）对文件 A 进行数字签名。
攻击者将文件 A 替换为文件 B，由于两个文件的 SHA-1 哈希值相同，签名对文件 B 同样有效，验证通过。
结果：签名者在不知情的情况下签署了恶意合同，造成重大损失。
>
---
...
> 答：
>签名私钥:	生成数字签名,	只有持有私钥的发送者才能对消息进行签名，这是身份认证和不可否认性的基础
> 
>签名公钥:	验证数字签名	任何人都可以用公钥来验证签名是否有效，从而确认消息的来源和完整性
>
>加密公钥:	加密消息	发送方使用接收方的公钥对消息进行加密，只有持有对应私钥的人才能解密
>
>加密私钥:解密密文	接收方用私钥解密密文，恢复原始消息，保证消息的机密性
>
>攻击者可以：
用公钥加密消息（向持有私钥的人发送加密消息）
用签名公钥验证签名是否有效
分析公钥的数学结构（模数 N 和指数 e）
>
>不能做的：
不能用公钥解密加密过的消息（需要私钥）
不能用公钥伪造数字签名（需要私钥进行签名运算）
不能从公钥推导出私钥（在计算上不可行，基于大整数分解的困难性）
---
...
> 答：
>(1)使用的密钥	数字签名发送方用私钥签名，消息加密接收方用公钥验证	发送方用接收方公钥加密，接收方用私钥解密
> (2)解决的问题	数字签名身份认证、完整性保护、不可否认性	消息加密消息机密性（保密性）
>(3)目的	数字签名证明"谁发的"和"内容没改"	消息加密防止"谁看到了"
>(4)能否否认	数字签名发送方无法否认（不可否认性）	消息加密不涉及身份问题
>(5)密钥持有者	数字签名私钥唯一属于发送者	消息加密私钥唯一属于接收者
>签名用于证明身份和完整性，加密用于保护机密性，两者各司其职，不能互相替代。 在实际应用中，通常将两者结合使用
...
> 答：
>不可否认性（Non-repudiation）是指：发送方无法否认自己发送过某条消息。也就是说，一旦发送方对消息进行了数字签名，接收方可以出示这份签名作为法律上有效的证据，证明该消息确实来自发送方。
>
> 数字签名能提供不可否认性的原因：
私钥的唯一性：签名私钥只有发送方持有，是发送方的独有身份凭证。
签名的不可伪造性：在密码学上，只有持有私钥的人才能生成有效签名。如果其他人能伪造签名，说明要么私钥泄露，要么密码学系统被攻破。
可公开验证：第三方（如法院）可以用发送方的公钥验证签名，判断签名是否有效，无需访问私钥。
>
>为什么消息认证码（MAC）不能提供不可否认性：
密钥共享：MAC 使用对称密钥，发送方和接收方共享同一个密钥。
双方都能生成：因为密钥是双方共享的，接收方也有能力生成与发送方相同的 MAC。
无法证明来源：如果发生纠纷，发送方可以声称："这个 MAC 不是我计算的，是接收方自己伪造的！"由于双方都有相同的密钥，第三方无法判断 MAC 到底是谁生成的。
>
---
...
> 答：
>场景一：软件发布（软件签名）
场景描述：用户从官方网站下载软件安装包，如 Firefox、VSCode、Windows 更新等。
解决的问题：用户下载软件后，需要确认软件确实来自官方开发者，而不是被黑客篡改或植入了恶意代码。
实现方式：开发者对软件安装包进行数字签名，用户在安装时操作系统会自动验证签名。如果签名无效，系统会发出警告。
示例：微软对 Windows 更新文件进行数字签名，Android 应用需要开发者签名才能发布。

>场景二：代码签名（Git 提交签名）
场景描述：开源项目开发者使用 Git 提交代码时，对每次提交进行 GPG 签名。
解决的问题：确保代码提交确实来自声称的开发者，防止有人冒用他人身份提交恶意代码。
实现方式：开发者使用 GPG 私钥对每次 Git 提交进行签名，提交记录中会显示"Verified"标识。
示例：Linux 内核开发要求所有提交都必须有 GPG 签名，以建立完整的代码追溯链。GitHub 和 GitLab 都支持显示验证状态。

>场景三：数字证书与 HTTPS（SSL/TLS 证书）
场景描述：用户访问网站（如银行网站、购物网站），浏览器显示绿色锁标志。
解决的问题：确认网站的身份是真实的，不是钓鱼网站，同时建立加密连接。
实现方式：数字证书由 CA（证书颁发机构）签名，浏览器验证证书签名后，确认网站的身份。证书中包含网站的公钥，用于建立加密连接。
示例：当你访问某银行网站时，浏览器验证该网站的 SSL 证书，确保你连接的是真正的银行网站。
> 
...
消息确实来自持有私钥的发送者（身份认证）
消息自签名后未被篡改（完整性保护）
但签名不包含时间信息。如果接收者一年后验证签名，只能知道签名是有效的，但无法知道签名是今天签的还是去年签的。
解决方案：可信时间戳服务（TSA - Time Stamping Authority）
可信时间戳服务的核心流程：
发送方将文件的哈希值发送给 TSA。
TSA 将哈希值与当前精确时间（来自受信任的时间源，如原子钟）一起签名，生成时间戳凭证。
发送方将文件、数字签名和时间戳凭证一起发送给接收方。
接收方验证：
文件签名有效（证明身份和完整性）
时间戳签名有效（证明 TSA 认可的时间）
时间戳服务的价值：
证明"存在性"：证明某个文件在某个时间点之前已经存在
防止"事后伪造"：发送方无法声称文件是后来才创建的
法律效力：TSA 是受信任的第三方，其签名基于标准协议（RFC 3161），可作为法律证据
> 
...
> 答：
>为什么使用 2048 位：
安全性：1024 位 RSA 已被认为不安全。2010 年，研究人员用 1 年时间和 2000 台计算机成功分解了一个 1024 位的 RSA 模数。目前 1024 位 RSA 的破解成本已大幅降低，不再适用于安全场景。
业界标准：2048 位是 NIST（美国国家标准与技术研究院）、CA/Browser Forum 等标准组织推荐的最小密钥长度。大多数 TLS 证书和软件签名都使用 2048 位。
性能与安全的平衡：2048 位在提供足够安全性的同时，性能开销可接受。签名验证速度较快（因为公钥指数 e=65537 的运算经过优化），适合日常使用。
> 
>4096 位是否有必要？
大多数场景：2048 位已足够
2048 位 RSA 的破解需要大约 2^112 次运算
在可预见的未来（20-30 年）难以实现
NIST 建议 2030 年之前使用 2048 位
>
---
...
> 答：
>"签名的签名"是有意义的
这种"签名的签名"在密码学中称为签名链（Signature Chain）或嵌套签名（Nested Signature）。
签名的签名的意义：
增加信任层级：Bob 对 Alice 的签名进行签名，表示 Bob 证实"Alice 确实签署了这份文件"。这在信任链传递中非常有用。
提供额外的身份确认：如果 Alice 的签名有争议，Bob 的签名可以作为额外的证据，证明 Bob（可能是 Alice 的上司或组织负责人）确认了这份文件。
时间戳功能：如果 Bob 在 Alice 签名之后一段时间再签名，可以证明"至少在 Bob 签名的时间点之前，Alice 的签名已经存在"。
实际应用场景：
>1. 证书链（X.509 数字证书）
这是签名链最典型、最广泛的应用：
根 CA（证书颁发机构）对中级 CA 的证书签名
中级 CA 对下级 CA 或终端用户证书签名
形成从根 CA → 中级 CA → 用户证书的信任链
当浏览器访问 HTTPS 网站时，会沿着证书链逐级验证，最终信任根 CA。
>2. 区块链 / 加密货币多重签名
多重签名要求多个私钥持有者共同签署一笔交易才能执行：
例如，公司财务需要 CEO 和 CFO 共同签名才能完成大额转账（2-of-2 或 2-of-3 多重签名）
每个签名都基于同一份交易数据，但由不同私钥签署
增强了资产安全性，防止单点风险
>3. 文档审批流程
合同需要各级主管逐级签署
每个签名都在前一个签名的基础上添加，形成审批链
最终版本包含了所有审批人的签名，不可否认
常见于企业 OA 系统、政府采购流程
>4. 公证服务
公证人对用户已经签名的文档进行再签名
证明"在公证人见证下，用户确实签署了这份文档"
如果将来用户否认，公证人的签名可以作证
具有法律效力
> 
