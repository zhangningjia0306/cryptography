...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | 29:A3:8F:89:18:7A:E4:FF:CE:34:BC:78:04:CE:C2:3F:EF:C8:04:2C:8E:EE:10:AD:BF:1E:79:C5:C8:77:C2:55|
| SHA-1 指纹（完整） | 10:9A:30:FE:B0:31:DE:A4:38:67:32:20:C0:2B:0C:CE:D4:D8:2F:0C|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |ISRG Root X1（预装在浏览器/OS信任库中） | My Root CA（需用户手动安装信任）|
| 浏览器默认是否信任 |是 |否 |
| 证书签发者是否为第三方 CA | 是|否 |
| 适合的使用场景 |公网网站、电子商务、支付系统、需要公共信任的 HTTPS 服务 | 开发测试环境、企业内网、教学实验、内部 API 通信|
...
> 答：
数字证书的本质：数字证书是一个电子文档，它将公钥与持有者的身份信息绑定在一起，并由可信的第三方（证书颁发机构，CA）进行数字签名。本质上，数字证书是"身份信息 + 公钥 + CA 的数字签名"三者的组合体。

>解决的问题：数字证书解决了公钥分发中的信任问题。具体来说：
身份绑定：将公钥与具体的实体（个人、组织、网站）身份绑定，确保公钥确实属于声称的持有者
防篡改：证书内容被 CA 签名，任何篡改都会导致签名验证失败
可验证性：任何人都可以使用 CA 的公钥验证证书的有效性

>为什么单独的数字签名不够：
数字签名虽然能证明"某个消息确实来自持有某个私钥的人"，但它无法回答一个更根本的问题："这个公钥到底属于谁？"
在 Lab11 中，攻击者可以同时替换消息、签名和公钥，接收方用攻击者提供的假公钥验证假签名，仍然会得到"Verified OK"的结果。接收者根本无法知道自己用的是否是发送方的真实公钥。
数字证书通过引入可信第三方（CA）解决了这个"公钥归属"问题。CA 用自己的私钥对"公钥 + 身份信息"进行签名，任何人都可以用 CA 的公钥验证证书的合法性。只要信任 CA，就可以信任 CA 签发的所有证书。
---
...
> 答：
自签名证书是指由证书持有者自己签发给自己使用的证书，即证书的 Issuer（颁发者）和 Subject（主题）完全相同。根 CA 证书本质上就是自签名证书，只不过它被广泛接受为信任起点。

>浏览器警告自签名证书不安全的原因：
缺乏第三方担保：自签名证书没有经过任何可信第三方的审核和背书。任何人都可以声称自己是"Google"并签发一张自签名证书。
无法验证身份真实性：浏览器无法确认证书持有者是否真的拥有其声称的身份。
中间人攻击风险：攻击者可以自签名一张证书，冒充目标网站，拦截用户的通信。浏览器无法区分真假。

>可以使用自签名证书的场景：
开发测试环境：在本地开发、测试服务器上使用，不需要对外公开访问。
企业内网：在企业内部网络中使用，由 IT 部门统一管理和分发。
教学实验：学习和理解证书原理，如本实验。
加密传输：只需要加密而不需要身份认证的场景（如内部 API 通信）。
---
...
需要证书链的原因：
安全性（私钥保护）
根 CA 的私钥是整个 PKI 体系的核心，一旦泄露，所有信任该根的证书都失效。
通过使用中间 CA，根 CA 的私钥可以存放在离线的、物理安全的环境中，不参与日常签名操作。
即使中间 CA 的私钥泄露，也只需要吊销该中间 CA，不影响根 CA。
风险隔离
不同中间 CA 可以负责不同的业务领域、不同的组织，实现职责分离。
某个中间 CA 出问题，不会影响其他中间 CA 签发的证书。
可撤销性
中间 CA 的证书可以被吊销，而根 CA 的证书一旦吊销，所有下游证书全部失效，影响巨大。
运营灵活性
中间 CA 可以根据需要更新（密钥更换、算法升级），而不需要重新分发根 CA。
不同中间 CA 可以采用不同的签名算法、不同的安全策略。
商业和管理
CA 可以授权给不同的中间 CA 进行运营，实现业务扩展和分层管理。

>不让根 CA 直接签发终端证书的原因：
根 CA 私钥直接暴露给日常操作会极大增加风险
一旦根 CA 私钥泄露，整个 PKI 体系崩溃
根 CA 证书更新成本极高（需要所有客户端更新信任列表）
---
...
> 答：
>根 CA 证书的信任机制：
根 CA 证书是自签名的，意味着没有任何上级机构为它担保。我们信任它，是因为：
人工审核和选择：操作系统和浏览器厂商经过严格的审核流程，选择值得信任的 CA 机构。
预装机制：经过审核的根 CA 证书被预装到操作系统（Windows、macOS、Linux）和浏览器（Chrome、Firefox）的信任存储中。
利益约束：CA 机构为了维护自己的声誉和商业利益，会严格遵守行业规范（如 CA/Browser Forum 的要求）。
监管和审计：CA 机构需要定期接受第三方审计，违反规则会被移除信任列表。

>操作系统和浏览器决定信任根 CA 的流程：
申请审核：CA 机构向浏览器厂商（如 Mozilla、Apple、Microsoft）提交申请
技术审查：审查 CA 的技术能力、安全措施、运营流程
合规检查：检查是否符合 CA/Browser Forum 的 Baseline Requirements
审计要求：要求 CA 通过 WebTrust 或 ETSI 等外部审计
投票和批准：在浏览器社区中进行投票，决定是否加入信任列表
监控和更新：持续监控 CA 的行为，发现问题时移除信任
---
...
> 答：
pathlen 的含义：pathlen（Path Length Constraint）是 X.509 证书扩展中 basicConstraints 字段的一个参数，用于限制证书链中在此证书之下的最大 CA 层级数。

>pathlen:0 的含义：
表示该证书可以签发证书，但只能签发终端实体证书（如服务器证书、客户端证书）
不能签发下一层 CA 证书
这是中间 CA 证书的典型设置

>pathlen:1 的含义：
表示该证书可以签发下一层 CA 证书
并且那一层 CA 还可以再签发一层终端实体证书
即证书链深度允许再扩展 1 层

>区别：
pathlen:0 阻止了证书链的进一步延伸，即使中间 CA 私钥泄露，攻击者也无法用它创建假 CA。本实验中根 CA 设为 pathlen:1（允许一层中间 CA），中间 CA 设为 pathlen:0（只能签终端证书），遵循最小权限原则。
---
...
证书过期后发生的情况：
当浏览器访问一个证书已过期的网站时：
浏览器检查证书有效期，发现当前时间不在 Not Before 到 Not After 之间
浏览器显示安全警告页面，提示"证书已过期"
用户需要手动确认是否继续访问（风险自担）
如果是 API 调用，客户端代码会拒绝连接并报错

>证书需要有效期限制的原因：
密钥轮换：强制定期更换密钥，降低密钥泄露造成的损失
技术演进：允许签名算法、密钥长度等技术在合理时间内升级
减少漏洞窗口：如果发现某个 CA 被攻击，过期的证书自然失效
运营规范：定期审核和更新，确保证书持有者仍然存在并具有合法身份

>Let's Encrypt 证书 90 天有效期的原因：
安全：缩短有效期意味着即使私钥泄露，攻击者只有 90 天的时间窗口
自动化：Let's Encrypt 鼓励通过 ACME 协议实现全自动续期，不需要人工干预
减少吊销成本：如果证书出现问题，等待过期即可，不需要复杂的吊销处理
快速更新：可以快速响应安全威胁和算法升级
验证身份最新性：定期验证域名控制权，确保证书持有者仍然拥有该域名
---
...
> 答：
>需要吊销证书的情况：
私钥泄露、身份或域名变更、CA 误签发证书、业务终止、加密算法被攻破。

>CRL（证书吊销列表） 是 CA 定期发布的包含所有被吊销证书序列号的列表，客户端下载完整列表查询。优点是可离线使用，缺点是列表可能很大、存在更新延迟。

>OCSP（在线证书状态协议） 是客户端实时向 CA 查询单个证书状态，优点是实时性好、数据量小，缺点是需要 CA 提供在线服务、会暴露用户访问的域名（隐私问题）。

>OCSP Stapling 是改进方案，由服务器在 TLS 握手时附带 OCSP 响应，减少客户端独立查询，提高了性能和隐私保护。
---
...
> 答：
SAN 扩展使一张证书可以同时覆盖多个域名和 IP 地址，支持通配符匹配。更关键的是，现代浏览器强制要求证书包含 SAN，在 TLS 验证时只看 SAN，忽略 CN。如果证书没有 SAN，浏览器会报错"此证书对此域名无效"。

>CN 是证书主题中的传统字段，只支持一个域名；SAN 是 X.509 v3 扩展字段，支持 DNS、IP、Email、URI 等多种类型，可包含多个域名。CN 已被浏览器弃用，仅作为兼容性后备。所有现代服务器证书都必须包含 SAN 扩展。
...
> 答：
>证书指纹是对整个证书的二进制数据（DER 编码）进行哈希计算得到的摘要值。SHA-256 指纹长度为 64 个十六进制字符，SHA-1 为 40 个字符。证书中任意一个字节的变化都会导致指纹完全不同。

>证书钉扎（Certificate Pinning） 是安全机制：客户端在首次连接时记录服务器证书指纹，后续连接只接受指纹匹配的证书，拒绝其他任何证书（即使是由可信 CA 签发的）。这能有效防止中间人攻击——即使攻击者获得了 CA 签发的伪造证书，指纹对不上连接也会被拒绝。

>移动 App（银行、支付应用）广泛使用证书钉扎来保护用户通信安全。局限是证书更新时需要同步更新 App，否则用户无法连接。
---
...
> 答：
>通配符证书（如 *.example.com）可以匹配所有一级子域名：www.example.com、api.example.com、mail.example.com 等都匹配。

>但它不能匹配根域名 example.com 本身，也不能匹配多级子域名如 a.b.example.com。此外，通配符不能匹配通配符本身。

>安全风险是：一旦通配符证书私钥泄露，攻击者可以冒充该域名下的所有子域名（包括 pay.example.com、admin.example.com 等关键服务），危害极大。因此通配符证书应谨慎使用，关键服务建议使用具体域名证书。
---
...
> 答：
>证书透明度（Certificate Transparency, CT） 是一个公开审计框架：所有 CA 签发的证书必须提交到公共的、只可追加的日志服务器中，任何人都可以查询和监控。

>CT 解决了以下问题：CA 被入侵后为域名签发伪造证书，会被记录在公开日志中，域名所有者可监控发现；CA 误签发证书可被追溯审计；攻击者想秘密签发证书进行中间人攻击，但证书必须公开提交，无法保持秘密；所有证书签发记录被永久保存。

>Chrome、Safari 等浏览器强制要求证书必须在 CT 日志中记录，否则拒绝信任。CT 让 CA 的证书签发行为完全透明，便于社区监督，是 PKI 体系的重要安全增强。
---
...
> 答：
Let's Encrypt 使用 ACME 协议（自动证书管理环境）自动签发证书。

>ACME 验证域名控制权的方式：
HTTP-01 验证（最常用）：Let's Encrypt 生成随机令牌，客户端在 http://域名/.well-known/acme-challenge/令牌 路径放置验证文件，Let's Encrypt 通过 HTTP 访问验证。适用于有 Web 服务器的域名。
DNS-01 验证：客户端在 DNS 中添加 _acme-challenge.域名 的 TXT 记录，Let's Encrypt 通过 DNS 查询验证。适用于通配符证书或没有 HTTP 服务的场景。
TLS-ALPN-01 验证：在 TLS 握手阶段验证，适用于云服务和 CDN。

>90 天有效期和自动续期的原因：缩短私钥泄露窗口期；推动运维自动化；定期重新验证域名控制权；证书到期自动失效降低吊销成本；快速响应安全威胁。
---
...
> 答：
两者在证书格式和证书链结构上完全相同，都是 X.509 v3 标准的三层结构。核心差异在于信任根：Let's Encrypt 的根 ISRG Root X1 经过了 Mozilla、Google、Apple 等厂商的严格审核，被预装在操作系统和浏览器的信任存储中；本实验的 My Root CA 是自己创建的，没有经过任何外部审核，未预装在任何信任存储中。

>因此浏览器对 Let's Encrypt 证书的验证路径是：服务器证书 → R11 → ISRG Root X1（在信任库中）→ 验证通过。对私有证书的验证路径是：服务器证书 → 中间 CA → My Root CA（不在信任库中）→ 验证失败。

>信任的本质：真实 PKI 的信任是基于"群体审核和预装"的集体信任决策；私有 PKI 的信任需要用户手动安装根证书并做出个体信任决策。如果用户将 My Root CA 安装到系统信任库，私有证书也能被浏览器信任——这正是企业内网 PKI 的做法。
---
