...
> 答：三大保证：身份认证（用私钥签名、公钥验签）、完整性（哈希对比）、不可否认性（私钥唯一持有）。
...
> 答： 先哈希后签名的原因：效率高、突破RSA长度限制、增强安全性。
---
...
> 答： 2048位RSA签名长256字节，4096位则为512字节。
...
> 答：哈希三性质：抗原像性（单向）、抗第二原像性（弱抗碰撞）、抗碰撞性（强抗碰撞），任一性质缺失都会破坏签名安全。
...
> 答：签名私钥生成签名、签名公钥验证签名；加密私钥解密、加密公钥加密；攻击者获公钥可验签、加密消息，不可伪造签名和解密消息。
...
> 答：签名用私钥签、公钥验，解决身份/完整性/不可否认性；加密用公钥加、私钥解，解决保密性；加密无法证明发送者身份，不能代替签名。
...
> 答：不可否认性指发送方无法否认发送过消息；数字签名用非对称密钥，私钥唯一；MAC用对称密钥，双方共享密钥，无法向第三方证明。
...
> 答：软件发布（防篡改/伪造）、电子邮件（防伪造/钓鱼）、代码签名（防恶意代码）。
...
> 答：签名本身无法验证时间；用权威时间戳服务（TSA）绑定签名与时间，验证时间戳即可确认签名时间
...
> 答：2048位RSA兼顾安全与性能；1024位已不安全；4096位更安全但性能开销大，仅高安全场景需要。
...
> 答：多重签名有意义，可用于证书链、审批流程、跨机构认证等场景。
