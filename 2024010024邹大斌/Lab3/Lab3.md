...
```
 \boldsymbol{\mathrm{Adv}_{\mathrm{PRG}}[A, G] = \left|\Pr[A(G(k))=1] - \Pr[A(r)=1]\right|}（k \xleftarrow{\} K,\ r \xleftarrow{$} {0,1}^n$）
```
...
```
接近1：算法A可有效区分伪随机序列与真随机序列，G不安全；接近0：算法A无法区分，G对抗A安全
...
```
\begin{align*}
\mathrm{Adv}_{\mathrm{PRG}}[A, G] &= \left|\frac{2}{3} - \frac{1}{2}\right| \\
&= \left|\frac{4}{6} - \frac{3}{6}\right| \\
&= \left|\frac{1}{6}\right| \\
&= \frac{1}{6}
\end{align*}
```
...
原因：
PRG安全的定义要求，对任意有效的统计测试A，\mathrm{Adv}_{\mathrm{PRG}}[A, G]都必须是可忽略的（即随着安全参数n增大，优势趋近于0）。
 
而这里计算得到的优势是\boldsymbol{\frac{1}{6}}，这是一个常数级别的非可忽略优势，说明存在有效的统计测试A，能够有效区分G生成的伪随机序列和真正的随机序列，因此该PRG不安全
```
...
```
完美保密性（Perfect Secrecy）的定义是：
对于任意明文m \in \mathcal{M}和密文c \in \mathcal{C}，满足
\Pr[E(k, m) = c] = \Pr[E(k, m') = c]
对所有的m, m' \in \mathcal{M}成立
或者等价表述：
\Pr[M = m \mid C = c] = \Pr[M = m]
即密文c不会泄露任何关于明文m的信息，后验概率等于先验概率。
...
```
一次性密码本（OTP）的加密操作是：c = m \oplus k，其中\oplus是按位异或。
对于给定的m和c，解这个方程得到：
k = m \oplus c
这个k是唯一确定的，所以恰好存在\boldsymbol{1}个密钥k使得E(k, m) = c
```
...
```
由(b)可知：对任意m, m' \in \mathcal{M}和c \in \mathcal{C}，
\Pr[E(k, m) = c] = \frac{\text{满足条件的密钥数}}{|\mathcal{K}|} = \frac{1}{|\mathcal{K}|} 
同理
\Pr[E(k, m') = c] = \frac{1}{|\mathcal{K}|}
因此
\Pr[E(k, m) = c] = \Pr[E(k, m') = c]
满足(a)中完美保密性的定义，所以OTP具有完美保密性
```
...
```
证明加密方案\mathbb{E}=(E,D)不是语义安全的
语义安全的核心要求：攻击者无法从密文获得任何关于明文的非平凡信息。
这里存在高效算法A，能从密文c推断出明文m的最低有效位\mathrm{LSB}(m)，这说明密文直接泄露了明文的部分信息，违背了语义安全的定义，因此\mathbb{E}不是语义安全的
```
...
```
- 选择两条消息：
m_0（最低有效位\mathrm{LSB}(m_0)=0），m_1（最低有效位\mathrm{LSB}(m_1)=1）
- 收到密文c后：
调用子算法A，输入c得到\mathrm{LSB}(m_b)：- 如果A(c)=0，则猜测b=0
- 如果A(c)=1，则猜测b=1
```
...
语义安全优势的定义：
\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}] = \left|\Pr[B \text{ 获胜}] - \frac{1}{2}\right|
在这个构造中，B的获胜概率是1（因为A总能正确得到\mathrm{LSB}(m_b)，从而准确判断b）：
\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}] = \left|1 - \frac{1}{2}\right| = \boldsymbol{\frac{1}{2}}
```
...
```
EXP(0) 中，k 是从密钥空间均匀随机选取的，c = k \oplus m_0。
 
对任意固定的m_0，映射k \mapsto k \oplus m_0是\{0,1\}^n上的双射（异或运算可逆）。
因此c的分布与k完全相同，即c在\{0,1\}^n上均匀分布
```
...
```
EXP(1) 中，c = k \oplus m_1，同理：
k均匀随机，k \mapsto k \oplus m_1是双射，因此c在\{0,1\}^n上均匀分布
```
...
```
语义安全优势定义：
\mathrm{Adv}_{\mathrm{SS}}[A, \text{OTP}] = \left|\Pr[A \text{ 在 EXP(0) 中获胜}] - \Pr[A \text{ 在 EXP(1) 中获胜}]\right| 
由(c)，EXP(0)和EXP(1)中密文分布完全相同，攻击者A面对的密文分布无差别，因此：
\Pr[A(k \oplus m_0)=1] = \Pr[A(k \oplus m_1)=1]
代入优势公式：
\mathrm{Adv}_{\mathrm{SS}}[A, \text{OTP}] = \left|\Pr[A(k \oplus m_0)=1] - \Pr[A(k \oplus m_1)=1]\right| = 0
```
