...
| 签名公钥模数 N 的前 16 位十六进制 | 00:ab:23:1b:99:de:f5:5b:4c:32:19:83:b7:87:d3:
 |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护。发送者：何燃，学号：2023010120 |
| 密文文件 encrypted_message.bin 大小（字节） |256 |
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |522707d6404aef6802c2c091d7e8991db636fa5aaaa37fa25ca4ca9d52b7508a |
...
> 答：身份认证（Authentication）：证明消息确实是由持有对应私钥的发送者所签发的。在实验中，接收方使用 signature_public_key.pem 验证签名成功（Verified OK），由于公钥只能与配对的私钥对应，这证明该签名是由持有 signature_private_key.pem 的发送方生成的。
完整性保护（Integrity）：确保消息在传输过程中未被任何未经授权的篡改。在实验中，当密文被追加一个字节（tampered_encrypted_message.bin）后，重新验证签名会直接提示 Verification failure，说明数字签名严格绑定了密文的内容，任何微小的改动都能被检测出来。
不可否认性（Non-repudiation）：发送方无法否认自己曾发送过该消息。因为生成签名的私钥只有发送方（理论上）独自掌握，其他人无法伪造该签名，所以签名者无法抵赖。
...
> 答：效率与性能问题：非对称加密算法（如 RSA）的数学运算复杂度高、速度慢，直接对大文件或长密文进行签名计算开销极大；而哈希函数的计算速度极快，将大文件压缩为固定短摘要后再签名，可实现毫秒级的快速处理。

输入长度限制：RSA 等公钥密码体制对单次处理的数据块长度有严格限制，无法直接对任意长度的明文或密文进行模幂运算签名，必须通过哈希将其转化为固定长度（如 SHA-256 的 32 字节）的数据块。

标准化与接口统一：哈希输出长度固定，这使得签名算法的输入格式能够标准化，无论被签名的密文或文件大小如何，签名接口处理的数据长度始终保持一致。
...
> 答：RSA 签名的本质是用私钥对哈希摘要进行数学运算，其运算结果（即签名）的长度等于 RSA 模数 $N$ 的字节长度。2048 位除以 8 等于 256 字节，因此签名的长度固定为 256 字节。如果使用 4096 位的 RSA 密钥，模数长度为 4096 位，即 $4096 \div 8 = 512$ 字节，因此签名长度会是 512 字节。
...
> 答：抗原像性（Preimage Resistance）：给定一个哈希值，在计算上无法反推出对应的原始消息。若不满足，攻击者可以通过截获签名和公钥反推出哈希值，进而破解出明文或密文内容。抗第二原像性（Second Preimage Resistance）：给定消息 $m_1$，在计算上无法找到另一个消息 $m_2$（且 $m_2 \neq m_1$），使得它们的哈希值相同 $H(m_1) = H(m_2)$。若不满足，攻击者可以伪造一个与原消息哈希值冲突的恶意消息，使得针对原消息的签名对恶意消息同样有效，从而重用签名进行欺诈。抗碰撞性（Collision Resistance）：在计算上无法找到任意两个不同的消息 $m_1$ 和 $m_2$，使得它们的哈希值相同。若不满足，攻击者可以事先准备好内容完全不同的正常文件 A（用于让别人签名）和恶意文件 B（攻击者真正想替换的内容），使得两者的哈希值碰撞，从而用对文件 A 的签名来合法化文件 B。
...
> 答：因为 SHA-1 的抗碰撞性已经被破解，计算碰撞的代价在当前计算能力下已变得可行。

碰撞攻击意味着攻击者可以构造两个哈希值相同但内容截然不同的文件：文件 A（一份表面合理、请求签名的文档或代码）和文件 B（包含恶意攻击逻辑的代码或合同）。

举例影响：签名者审查并同意为良性的文件 A 加上数字签名。由于文件 A 和文件 B 的 SHA-1 哈希值完全一致，攻击者可以截获该签名，并将其附在恶意文件 B 上。当验证者使用公钥验证时，会误以为该签名确实是对恶意文件 B 签署的，从而破坏了数字签名的防篡改与身份认证机制。
...
> 答：签名私钥用于生成数字签名（对哈希值进行加密/私钥运算）；签名公钥用于验证数字签名的有效性。

加密私钥用于对密文进行解密操作；加密公钥用于将明文加密为密文。

公钥持有者的权限：攻击者获得公钥后，可以验证签名者的签名，或者使用加密公钥给通信方加密发送保密消息；无法做到的事情：无法通过公钥推导出私钥，也无法伪造私钥所有者的数字签名，更无法解密发送给私钥所有者的加密消息。
...
消息加密是为了保证数据的机密性（Confidentiality），防止信息在传输中被窃听，只有持有对应解密私钥的接收方才能看懂内容。

数字签名是为了保证数据的真实性、完整性与不可否认性，证明消息确实来源于特定主体且未被修改。

为什么不能替代：加密仅能保证“除了持有私钥的人，没人能看懂内容”，但无法证明是谁加密的（任何人拿到加密公钥都可以发送密文）；而且密文在传输途中依然可能被攻击者篡改或替换，接收方解密后无法判断内容是否被第三方破坏。因此，保密性不能等同于身份认证和完整性保护。
...
> 答：不可否认性是指发送方不能否认自己曾经发送过某条消息，接收方也不能虚构或篡改消息内容并假称是对方发送的。

原因：数字签名基于非对称密码体制，私钥由签名者单方秘密持有，公钥公开。签名是用私钥生成的，任何人用公钥验证通过，都证明该签名必然只能由私钥持有者发出；而消息认证码（MAC）基于对称密钥体制，通信双方共享同一个密钥。如果接收方能够生成合法的 MAC，他就无法向第三方证明这个 MAC 是自己生成的还是发送方生成的，因为接收方自己也持有相同的密钥，从而不具备不可否认性。
...
> 答：软件发布/系统更新：防止黑客攻陷下载服务器并篡改安装包。数字签名确保用户下载的安装包确实来自于官方开发者，且在下载过程中未被植入木马或病毒，保证了软件的完整性与真实性。

代码版本控制（如 Git 提交签名）：开源协作中，开发人员使用私钥对提交的代码（Commit）或合并请求（Pull Request）进行签名。这解决了身份冒充问题，确保代码确实是由声明的贡献者编写的，防止恶意代码被注入关键软件项目中。

电子邮件安全（S/MIME）：发送方使用私钥对邮件内容签名，接收方通过公钥验证发件人身份。这解决了电子邮件极易伪造发件人地址的钓鱼攻击痛点，确信邮件确实来自于声称的公司或个人，且内容在传输时没有被中途拦截篡改。
...
> 答：数字签名本身不能证明签名的时间。签名只证明“文件内容在被哈希并用私钥加密那一刻”的状态，但无法记录具体的操作绝对时间（Clock Time）。

解决方法：引入可信时间戳服务（Time-Stamp Authority, TSA）。在签名时，将文件哈希值与当前时间一起发送给权威时间戳服务器，由时间戳服务器对“哈希值 + 时间”的数据进行数字签名。这样既利用了签名防篡改，又通过可信第三方绑定了不可伪造的时间戳，从而证明文件签署的绝对时间。
...
> 答：1024 位：目前已被认为计算上不够安全，随着因式分解算法和计算能力的提升（如通用数域筛法 GNFS），在合理时间内分解 1024 位模数的成本已显著降低，容易被攻破。

2048 位：当前工业界和学术界的通用安全标准，在较长时间（如未来十几年内）内能够提供足够的抗攻击安全性，且加解密/签名验证的性能开销适中。

4096 位：理论上提供了超高等级的安全性，但在特定低功耗场景或高并发服务器中，密钥生成、加密和签名的计算耗时将大幅上升（性能开销呈多倍增长）。对于一般商业及保密需求，2048 位已然足够，4096 位通常用于对安全性要求极高（如根证书颁发机构 CA）的场景，是一种安全与性能的权衡选择。
...
> 答：这种做法是有实际意义的，它构成了一种信任链或多重授权机制。

应用场景：

公钥基础设施（PKI）中的证书链：下级证书颁发机构（Sub-CA）的公钥及其身份信息由根证书颁发机构（Root CA）用其私钥进行数字签名，形成层级信任关系。

企业财务审批或重要文档流转：一份重要合同需要多名高管（如部门经理、财务总监、总经理）依次进行审批签名。多重签名确保了文件经过了多方严格的层层授权与确认，缺少任何一方的签名都不具备法律效力。
