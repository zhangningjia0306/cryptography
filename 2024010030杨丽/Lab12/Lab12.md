...
| 证书有效期（起始-结束） | 026-05-22 12:16:05 GMT - 2026-08-20 12:16:04 GMT |
| 公钥算法 | id-ecPublicKey（P-256 椭圆曲线）|
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | 5A:4C:8A:95:3D:B3:40:FD:62:9F:00:81:EA:C8:77:84:2B:4A:36:72:AF:94:BF:FB:88:E9:FE:99:76:BE:65:09 |
| SHA-1 指纹（完整） | A0:91:0B:58:A9:DF:71:D3:49:6F:1E:64:57:9E:81:19:1B:F5:E0:A4 |

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（3n7er.space）| 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 | ISRG Root X1（系统预装全球可信根 CA）| 自建 My Root CA（本地私有根证书）| 
| 浏览器默认是否信任 |默认信任，无需手动导入证书  | 默认不信任，需要手动导入根 CA 到系统信任库 |
| 证书签发者是否为第三方 CA | 是，Let's Encrypt 合规第三方商业 CA | 否，自主搭建内部 CA，无第三方机构背书 |
| 适合的使用场景 | 公网对外公开网站、面向全网用户的 HTTPS 服务 | 企业内网、实验室测试、内部私有业务系统，不对外公网开放 |
...
>1. **数字证书本质**
数字证书是由可信第三方 CA 对「用户公钥 + 身份信息」做数字签名后的标准化文件（X.509），相当于公钥的可信身份证，绑定实体身份与对应公钥。
>
>2. **解决的核心问题**
解决公钥分发过程中的中间人攻击：防止攻击者伪造公钥冒充目标实体，保证通信方拿到的公钥确实属于对方本人。
>
>3. **仅数字签名存在的缺陷**
数字签名只能证明 “某份数据由对应私钥签名”，但无法证明公钥归属：
攻击者可伪造一对公私钥，用私钥签名数据，再把伪造公钥发给你；
你只能校验签名合法，但无法确认这个公钥真的属于目标网站 / 用户；
数字证书借助 CA 可信签名，对公钥身份做权威背书，补齐身份可信校验环节。
---
...
>1. **自签名证书定义**
自签名证书是实体使用自身私钥为自身签发的 X.509 证书，证书颁发者 Issuer 与证书主体 Subject 完全一致，不存在第三方 CA 进行签名背书。
>
>2. **浏览器弹出安全警告的原因**
浏览器内置全球可信第三方根 CA 列表，自签名证书的签发机构不在系统默认信任库内，浏览器无法验证证书身份合法性，存在中间人劫持窃听流量的风险，因此主动弹出不安全警告拦截访问。
>
>3. **自签名证书适用场景**
仅适用于访问人群完全可控的封闭环境：企业内网业务系统、本地开发 HTTPS 调试、实验室私有 PKI 测试平台；所有使用者可手动导入根证书完成信任，严禁用于面向全网普通用户的公网网站。
...
>1. **使用多级证书链的原因**
通过根 CA、中间 CA 分层架构拆分签发权限，实现分级管理、风险隔离；根 CA 私钥极少离线使用，大幅降低根密钥泄露风险；可按业务、地域划分多个中间 CA，适配大规模组织的证书管理需求。 
>
>2. **不直接用根 CA 签发终端证书的原因**
根 CA 为信任锚，安全等级最高，私钥通常离线封存。若频繁使用根 CA 签发大量终端证书，会大幅提升根私钥暴露、被盗的安全风险；一旦根密钥泄露，整个信任体系全部失效，代价极高。
...
>1. **信任自签名根 CA 的核心逻辑**
公网根 CA 提前预装在系统 / 浏览器，属于人工预置可信锚；私有根 CA 需要用户手动导入信任库，人为赋予信任关系，不存在上级 CA 为其背书。
>
>2. **系统与浏览器信任根 CA 的判定规则**
厂商会严格审核合规第三方 CA，将其根证书内置到系统、浏览器信任列表；仅列表内的根证书会被默认信任；未内置的自签名根证书默认不受信任，需手动添加。
---
...
>1. **pathlen:0 含义**
该中间 CA 仅能签发终端实体证书，不允许再签发任何下级中间 CA，限制证书链深度，缩小安全攻击面。 
>
>2. **pathlen:1 的区别**
允许该 CA 向下签发 1 层中间 CA，下级中间 CA 无法再创建子 CA；相比 pathlen:0，证书链层级更深，风险扩散范围更大。
...
> 答：
>1. **证书过期后的现象**
浏览器校验证书有效期失败，弹出安全警告，阻断 HTTPS 访问；客户端拒绝建立加密连接，网站无法正常打开。
>
>2. **设置有效期的原因**
① 缩小密钥泄露风险窗口，密钥泄露后仅在有效期内有效；
② 强制定期更新证书，同步更新域名、企业信息等身份内容；
③ 限制长期失效证书持续流通，降低中间人攻击长期利用的风险。
>
>3. **Let's Encrypt 仅 90 天有效期的原因**
Let's Encrypt 为免费自动化证书，短有效期倒逼站点部署自动续期工具；缩短证书生命周期可减少泄露证书的可利用时长，提升整体安全强度，同时降低证书吊销列表的维护压力。
...
>
>1. **需要吊销证书的场景**
服务器私钥泄露、域名所有权变更、企业业务注销、证书信息填写错误、证书被恶意冒用。 
>
>2. **CRL 与 OCSP 的区别**
  • CRL：证书吊销列表，CA 定期下发完整吊销证书清单，客户端本地下载完整列表校验，占用带宽、存在更新延迟； 
  • OCSP：在线证书状态查询，客户端单次向服务器查询单张证书状态，实时性更高、流量更小，是现代主流方案。
...
>1. **现代证书必须包含 SAN 扩展的原因**
单张证书可绑定多个域名，支持多站点共用一套证书；现代浏览器已不再信任仅依靠 CN 匹配域名，强制校验 SAN 字段，无 SAN 会判定证书无效。
>
>2. **SAN 与 CN 的区别**
 • CN（通用名）：早期仅支持填写一个主域名，仅作兼容遗留标准；
 • SAN（主题备用名）：标准扩展字段，可存放多个域名、通配符域名，是浏览器校验域名匹配的唯一有效字段。
...
>1. **证书指纹计算方式**
将完整证书原始 DER 二进制数据，使用 SHA-1/SHA-256 等哈希算法做单向哈希运算，输出分段十六进制字符串，即为证书指纹；指纹仅由证书完整内容决定，证书内容不变则指纹固定。 
>
>2. **证书钉扎（Certificate Pinning）定义**
客户端预先保存目标服务器证书 / 公钥的指纹，建立连接时强制比对线上证书指纹与本地预存指纹；即便攻击者拿到其他合法 CA 签发的伪造证书，指纹不匹配则直接拒绝连接，抵御 CA 被入侵伪造证书的攻击。
---
...
>1. **通配符证书可匹配域名**
一级子域名：www.example.com、mail.example.com、api.example.com；无法匹配根域名 example.com、二级子域名 dev.www.example.com。
>
 >2. **通配符证书局限性**
 ① 仅匹配一级下级域名，不支持多层子域名；② 私钥泄露后，该域名下所有子站点全部受影响，风险范围大；③ 无法同时绑定多个无关主域名，多域名场景需搭配 SAN 扩展。
...
>1. **证书透明度 CT 定义**
一套公开、不可篡改的证书日志系统，所有公网可信 CA 签发证书时，必须将证书写入公开 CT 日志，生成可审计的日志签名；浏览器会校验站点证书存在于 CT 日志中。 
>
>2. **解决的核心问题**
解决 CA 被入侵、违规滥发伪造证书却无法被及时发现的漏洞；任何人可检索 CT 日志，快速发现针对自身域名的非法证书，及时吊销止损，监督 CA 合规签发证书。                          证书透明度的作用是什么？ 如何查看证书的指纹？ 自签名证书有哪些应用场景？
---
...
>1. **免费签发证书的原理（结合 3n7er.space 证书链）**
Let's Encrypt 属于非营利公共 CA，依托 ISRG 公益机构运营，资金由企业捐赠覆盖，因此面向全网免费提供证书。3n7er.space 证书链结构：终端站点证书 → Let's Encrypt 中间 CA (E7) → 根 CA ISRG Root X1；根证书预装在主流浏览器 / 系统，无需用户手动信任，实现全网通用免费 HTTPS。 
>
>2.  **ACME 协议验证域名控制权两种主流方式**   
• HTTP-01：在域名根目录.well-known/acme-challenge下放 CA 指定的临时验证文件，CA 通过公网 HTTP 访问该文件，证明申请者拥有网站服务器读写权限； • DNS-01：在域名 DNS 解析添加_acme-challenge前缀的 TXT 记录，CA 查询 DNS 记录匹配随机字符串，证明申请者拥有域名 DNS 管理权限，支持通配符证书申请。  
>
>3. **短有效期 + 依赖自动续期的原因**
  • 90 天短有效期缩小密钥泄露、违规证书的可利用时间窗口，降低安全风险； • 强制使用 ACME 自动化工具续期，杜绝人工长期维护证书导致的过期、遗忘更新问题； • 减轻证书吊销列表 (CRL) 存储与查询压力，减少失效证书长期留存带来的安全隐患。
...
>1. 信任差异原因
3n7er.space证书由 Let's Encrypt 签发，其根证书 ISRG Root X1 预装在操作系统、浏览器内置信任列表，客户端默认信任整条证书链；实验私有 PKI 的根 CA 为自建自签名证书，未被系统信任库收录，浏览器无法校验签名合法性，因此默认告警不信任。 
>
>2.  相同点
   • 证书格式：均遵循 X.509 标准 PEM 格式，包含 Subject、Issuer、有效期、公钥、扩展字段等统一结构； • 证书链结构：都采用「根 CA→中间 CA→终端服务器证书」三级分层架构，依靠逐级签名完成信任校验；
    • 密码机制：均使用 RSA/ECC 非对称加密、SHA 哈希算法完成签名与身份绑定。
>
>3. 不同点
| 对比项 | 真实公网 PKI (Let's Encrypt)| 实验私有 PKI|| ---- | ---- | ---- |
| 信任根 | 全球预装的合规商业根 CA (ISRG Root X1)| 本地自建自签名根 CA，无系统默认信任 |
| 签发主体 | 第三方合规公益 CA，受行业监管 | 个人 / 企业自建内部 CA，无第三方监管 |
| 适用范围 | 公网所有互联网用户访问 | 仅内网、实验室可控环境使用 |
| 证书有效期 | 90 天 | 自定义（根 CA10 年、中间 CA5 年、服务器 1 年）|
| 信任方式 | 开箱即用，无需手动导入证书 | 必须手动导入根 CA 至系统信任库才可消除警告 |                          
