...
| :--- | :------- |
| 签名 RSA 密钥长度（位） | 2048 |
| 签名私钥 `signature_private_key.pem` 文件大小（字节） | 1740 |
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | 460 |
| 签名公钥指数 e 的值 | 65537（0x1001）|
| 签名公钥模数 N 的前 16 位十六进制 | 截图中未见 |
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） | 1740 |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：杨丽，学号：2024010030 |
| 密文文件 encrypted_message.bin 大小（字节） | 256 |
| 密文 SHA-256 哈希值（完整的 64 位十六进制） | ae58621f3e225ee1a071f722fd630234fb255bcc2cb58498a7553e0dc2d44a5c |
...
> 数字签名提供完整性、身份认证（不可伪造）、不可抵赖三大安全保证。
> 1. **数据完整性**
> 含义：接收方可确认数据在传输途中没有被篡改、增删。
> 实验体现：本次对密文 `encrypted_message.bin` 做 SHA-256 哈希再签名；篡改密文后验签直接输出`Verification failure`，只要密文改动一字节，哈希完全改变，签名校验失败，证明数据被篡改。
>
> 2. **身份认证（不可伪造）**
> 含义：能确认签名一定由对应签名私钥持有者生成，第三方无法伪造合法签名。
> 实验体现：签名使用独立私钥 `signature_private_key.pem` 生成，验签只能用配套公钥 `signature_public_key.pem`；其他人没有私钥，无法生成能通过公钥校验的合法签名。
>
> 3. **不可抵赖**
> 含义：签名者无法否认自己发送过该数据。
> 实验体现：私钥仅发送方持有，签名唯一绑定发送方身份；接收方持有公开签名公钥，可向第三方出示签名文件完成举证，发送方无法否认签名行为。
...
>1. **突破 RSA 签名长度限制**
2048 位 RSA 仅能直接签名不超过 245 字节的数据，本次密文虽短，但若文件较大则无法直接签名；SHA-256 固定输出 32 字节哈希摘要，长度永远满足 RSA 签名输入要求，适配任意大小文件。
>
>2. **大幅提升签名运算速度**
RSA 大数模幂运算开销很高，直接对完整密文签名耗时极大；哈希是快速单向运算，先把长文件压缩为短摘要再签名，能显著降低计算量，提升执行效率。
>
>3. **增强安全特性，规避签名攻击**
若直接对原文签名，存在构造伪造消息的安全漏洞；哈希算法具备雪崩效应，微小改动就会完全改变摘要，配合签名可杜绝攻击者构造相似伪造数据，同时保证完整性校验效果更强。
...
> 答：
>
> 1. **换算关系**：1字节=8比特，2048 bit ÷ 8 = 256 Byte。RSA签名输出的二进制长度固定等于密钥模数N的比特长度，因此2048位RSA生成的签名文件`signature.bin`大小为256字节。
>
> 2. **4096位RSA密钥对应的签名长度计算**：4096 ÷ 8 = 512 字节。若使用4096位RSA密钥，签名文件长度为512字节。
...
> 答：
>
> 哈希函数需要同时满足抗原像性、抗第二原像性、抗碰撞性三大安全性质。
> 1. **抗原像性（单向性）**
> 含义：给定哈希输出值h，无法在可行时间内反向计算出原始消息m，使得 Hash(m)=h。
> 安全隐患：若不满足，攻击者拿到签名对应的哈希值就能反推出原始密文/消息，破坏数据保密性，也能伪造匹配哈希值的消息。
>
> 2. **抗第二原像性（弱抗碰撞）**
> 含义：给定一条已知消息m₁，无法找到另一条不同消息m₂（m₂≠m₁），使得 Hash(m₁)=Hash(m₂)。
> 安全隐患：若不满足，攻击者拿到合法消息与签名后，可以构造另一条内容不同但哈希一致的恶意消息，替换原文后签名依旧校验通过，篡改数据。
>
> 3. **抗碰撞性（强抗碰撞）**
> 含义：无法找到任意两条完全不同的消息m₁、m₂，满足 Hash(m₁)=Hash(m₂)。
> 安全隐患：若不满足，攻击者可提前生成一对哈希相同的善恶消息，诱导用户对善意消息签名，再替换成恶意消息完成伪造，彻底摧毁数字签名完整性与身份认证能力。
...
> 答：
>
> 1. **弃用原因**：SHA-1 已被证明不满足强抗碰撞性，攻击者可以以可接受成本构造两条不同消息，产生完全相同的SHA-1哈希摘要，无法保障数字签名完整性，因此行业不再推荐。
> 2. **碰撞攻击对签名的影响**：数字签名是对消息哈希值签名，若两条消息哈希一致，一份合法签名会同时对两条消息生效，攻击者可篡改内容却通过验签，破坏完整性、身份认证、不可抵赖三大安全保证。
> 3. **攻击举例**：
> 攻击者提前构造两份文档：一份无害合同A、一份恶意转账合同B，二者SHA-1哈希完全相同。
> 攻击者先让用户对无害合同A签名；拿到签名文件后，将原文替换为恶意合同B。接收方验签时，计算B的哈希与签名内哈希匹配，判定签名合法，最终被恶意合同欺骗。
...
> 答：
>> 1. **签名密钥对分工**
> - 签名私钥 `signature_private_key.pem`：仅发送方持有，用于**生成数字签名**（对密文哈希做签名）。
> - 签名公钥 `signature_public_key.pem`：对外公开，用于**验证数字签名**，确认签名合法性。
>
> 2. **加密密钥对分工**
> - 加密公钥 `encryption_public_key.pem`：公开分发，用于**加密明文消息**，对应实验中加密`message.txt`生成密文。
> - 加密私钥 `encryption_private_key.pem`：仅接收方持有，用于**解密密文**，还原原始明文。
>
> 3. **攻击者获取公钥后的权限限制**
> 能做：① 使用加密公钥加密消息发给持有者；② 使用签名公钥校验已有签名的真伪。
> 不能做：① 伪造合法数字签名（无签名私钥）；② 解任何使用该公钥加密的密文（无加密私钥）；③ 推导、还原出配套私钥。
---
...
>
> 1. **本质区别与各自解决的问题**
> - 消息加密：使用接收方公钥加密，核心目标是**保密性**，防止第三方窃听读取明文；实验中用`encryption_public_key.pem`加密`message.txt`，只有加密私钥能解密。
> - 数字签名：使用发送方私钥签名，核心目标是**完整性、身份认证、不可抵赖**；实验中对密文哈希用`signature_private_key.pem`签名，所有人可用公钥校验数据未篡改、确实由发送方发出。
>
> 2. **不能用加密代替签名的三点原因**
> ① 操作主体相反：加密是公钥加密、私钥解密；签名是私钥签名、公钥验签。若用加密替代签名，需要所有人持有发送方私钥才能校验，私钥泄露后安全完全失效。
> ② 无法提供不可抵赖：加密的私钥仅接收方持有，无法向第三方证明原始发送者；签名私钥唯一归属发送方，可对外举证。
> ③ 安全目标完全不同：加密只管保密，无法校验数据是否被篡改；签名专门校验完整性，不隐藏原文内容。
...
>
> 1. **不可否认性定义**
>消息发送方无法否认自己发送、签署过该消息；接收方可向中立第三方出示凭证证明发送行为。
>
> 2. **数字签名具备不可否认性的原因**
> 数字签名基于非对称密钥：签名私钥仅发送方独立持有，只有持有者能生成合法签名；验证公钥公开，任何第三方都可独立校验签名归属，无法伪造、无法抵赖。
>
> 3. **MAC无法提供不可否认性的原因**
> MAC 采用对称密钥，收发双方共用同一密钥；接收方也能使用密钥伪造MAC值，当发生纠纷时，无法区分MAC是发送方生成还是接收方伪造，缺少中立第三方可采信的举证依据。
...
>
> 1. **软件安装包签名（程序发布场景）**
> 解决问题：防止安装包被植入病毒、木马篡改；用户下载后可校验发布厂商身份，确认程序未被中间人修改，保障终端安全。
>
> 2. **电子合同、电子发票（政务/金融场景）**
> 解决问题：提供法律效力上的不可否认性，确认签署人身份，合同内容不可篡改，替代手写签名，满足线上存证、维权举证需求。
>
> 3. **邮件S/MIME签名（办公通信场景）**
> 解决问题：验证邮件发送人真实身份，防止钓鱼仿冒邮件；同时保证邮件正文传输中未被篡改，防止中间人篡改邮件内容实施诈骗。
---
...
>
> 1. **数字签名本身无法证明签名时间**
> 数字签名仅对文件内容做私钥签名，内部不包含权威、不可篡改的标准时间信息。签名者可以提前签好文件，隔很久再对外分发，事后谎称是近期签署，无法通过签名本身辨别真实签署时刻。
>
> 2. **接收者验证签名时间的解决方案**
>使用可信时间戳服务TSA
> 3. **完整流程**：
> ① 用户完成文件数字签名后，将签名摘要发送给第三方权威可信时间戳服务器TSA；
> ② TSA获取国际标准可信时间，将时间与签名摘要绑定，用TSA私钥生成时间戳并签名；
> ③ 用户将原文件、数字签名、TSA时间戳一并保存；
> ④ 接收方校验时，同时验证文件签名合法性与TSA时间戳；
> ⑤ TSA是中立权威第三方，时间无法伪造篡改，可作为法律有效凭证，证明该签名在TSA标注的真实时间生成，戳破签名者谎称晚签署的抵赖行为。
---
...
>
> 1. **实验选用2048位RSA密钥的原因**
> 2048位是当前行业通用、安全与性能平衡最优的标准密钥长度，能满足教学实验安全要求，运算速度适中，同时符合国密、国际标准的中长期安全规范。
>
> 2. **1024位RSA安全性判断**
> 1024位已不再足够安全。现代分布式计算、数论算法可在可接受成本下分解1024位模数，各类安全标准早已淘汰1024位，存在被破解风险，禁止用于正式签名、加密业务。
>
> 3. **4096位RSA是否有必要**
> 4096位安全性更高，抗破解年限更长，但存在明显性能损耗：签名、验签、加密解密运算耗时是2048位的3~4倍，占用更多CPU与存储。
> - 有必要场景：长期归档文件、高密级政务/金融核心凭证、需要数十年有效期的签名；
> - 无必要场景：普通教学实验、高频次轻量业务（日常网页、普通邮件），2048位完全够用，4096位只会拖慢运行效率。
>
> 4. **安全性与性能权衡总结**
> - 1024位：性能最快，安全失效，禁止使用；
> - 2048位：安全达标、运算均衡，通用场景首选；
> - 4096位：安全余量极大，性能开销高，仅高安全等级场景使用。
...
> 答：
>
> 1. **该嵌套签名（签名的签名）具备实际意义。** Alice签名证明文件本身真实未篡改、由Alice发出；Bob对Alice的签名再签名，代表Bob核验认可Alice的签名与文件内容，形成两级权责背书，分层追溯身份。
>
> 2. **需要多重/嵌套签名的典型场景**
> ① 多级审批流程：企业合同、财务单据，经办人先签名，部门负责人、财务总监依次对前一级签名二次签名，每一级都留存审批凭证，权责分离；
> ② 证书签发体系（PKI）：终端用户证书由中间CA签名，中间CA根证书由根CA签名，形成多层签名链，逐级验证证书可信；
> ③ 跨境文件合规校验：本国机构签名后，第三方公证机构二次签名背书，满足跨地区法律举证要求；
> ④ 代码分层审核：开发人员签名程序包，安全审计团队对开发签名再签名，证明已完成安全审计。
