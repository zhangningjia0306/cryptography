...
| 签名公钥指数 e 的值 | 65537|
| 签名公钥模数 N 的前 16 位十六进制 | 00fd6d04785844f11b8d1798921302ac|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） | 1672|
...
| 密文 SHA-256 哈希值（完整的 64 位十六进制） | 9a29b6ed899a406f09de4058c814c10284c3376eed75bf511f89a1f0cba3b32f|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 解密后的文件是否与原文件一致 | 是，完全一致，终端输出 “解密结果一致”|
...
> 答：数字签名提供身份认证、完整性、不可否认性三大核心保证：
1.身份认证
含义：接收方可确认消息确实由对应私钥持有者发送，不存在伪造发送方。
实验体现：使用signature_private_key.pem私钥签名，公钥signature_public_key.pem验证输出Verified OK，证明密文只能由持有该私钥的用户生成。
2.数据完整性
含义：消息从发送到接收全程未被篡改、增删、修改，任意改动都会校验失败。
实验体现：篡改encrypted_message.bin后验证签名输出Verification failure，证明文件一旦修改签名校验失效。
3.不可否认性
含义：签名生成者无法否认自己发送过该消息，第三方可核验证据。
实验体现：私钥唯一归属发送人，全网仅本人持有，他人无法伪造有效签名，发送方不能抵赖本次传输行为。
...
> 答：
1.运算效率大幅提升
RSA 非对称加密运算速度极慢，直接对完整密文签名耗时巨大；SHA256 快速生成固定 32 字节短摘要，仅对摘要签名，运算速度提升数百倍。
2.解决 RSA 输入长度限制
2048 位 RSA 单次可处理明文上限约 245 字节，本实验密文刚好 256 字节，超出直接签名限制；哈希统一输出固定长度摘要，适配 RSA 输入约束。
3.标准化统一接口
无论原始文件大小，签名输入统一为 256bit 哈希值，算法实现标准化，兼容所有文件、数据流场景。
...
> 答：
1.2048 位 RSA 签名为 256 字节原因：
RSA 模数单位为比特，字节换算公式：密钥比特数 ÷ 8 = 签名字节长度
2048 ÷ 8 = 256，因此signature.bin固定 256 字节。
2.4096 位 RSA 签名长度：
4096 ÷ 8 = 512字节，使用 4096 位密钥生成的签名文件固定为 512 字节。
...
> 答：
1.抗原像性（单向性）
含义：给定哈希值 h，无法反向推导出原始消息 m。
风险：不满足则攻击者可通过签名哈希还原原始密文，泄露传输内容。
2.抗第二原像性
含义：给定消息 m1，找不到另一条不同消息 m2，使 H (m1)=H (m2)。
风险：不满足攻击者可篡改密文，生成哈希一致的伪造文件，绕过签名校验。
3.抗碰撞性
含义：无法找到两条完全不同的消息 m1、m2，二者哈希值相等。
风险：不满足攻击者可提前构造两份文件共用同一签名，伪造合法业务凭证。
...
> 答：
1.淘汰原因：2017 年谷歌完成 SHA-1 碰撞攻击，可人为构造两份内容完全不同、SHA1 哈希完全一致的文件，破坏哈希抗碰撞性，数字签名彻底失效。
2.攻击影响：攻击者可替换原始文件，签名校验仍显示合法，实现文件伪造。
3.攻击举例：
攻击者制作正规软件安装包 A、捆绑病毒的恶意安装包 B，使两者 SHA-1 哈希相同；厂商对 A 签名发布后，攻击者将下载链接替换为 B，用户校验签名时哈希匹配，会误认为恶意文件是官方原版。
...
> 答：
1.签名密钥对分工
签名私钥signature_private_key.pem：发送方生成数字签名
签名公钥signature_public_key.pem：所有人校验签名真伪

2.加密密钥对分工
加密公钥encryption_public_key.pem：发送方加密明文，生成密文
加密私钥encryption_private_key.pem：接收方解密密文，还原原始消息

3.攻击者获取公钥后的权限
可执行：加密消息、验证他人数字签名；
不可执行：伪造有效签名、解密密文、逆向推导私钥。
...
> 答：
核心区别
1.加密：目标保密性，公钥加密、私钥解密，仅隐藏消息内容，无法确认发送者、无法防篡改、无抵赖约束。
2.数字签名：目标身份认证 + 完整性 + 不可否认，私钥签名、公钥验证，不隐藏内容，可校验来源与文件完整性。
实验流程解释
本次流程加密→哈希→签名：先用加密保证消息不被窃听，再用签名保证消息未被篡改、来源可信。
不能用加密替代签名的原因
仅加密只能防止第三方读取内容，但任何人持有对应私钥均可解密修改后重新加密，无法证明消息原始发送人，也无法防止中间人篡改，缺少完整性与身份校验能力。
...
> 答：
1.不可否认性定义：消息发送者无法否认自己生成并发送过该消息，第三方可通过公开密钥完成公证核验。
2.数字签名具备、MAC 不具备的原因：
数字签名基于非对称密钥：签名私钥仅发送方独自持有，全网唯一，只有本人能生成合法签名，他人无法伪造，可作为法律证据。
MAC 消息认证码基于对称密钥：收发双方共用同一密钥，发送、接收方都能生成合法 MAC 值，出现纠纷时无法判定是谁生成的校验码，存在抵赖漏洞。
...
> 答：
1.软件安装包发布签名
厂商用私钥对安装包签名，用户通过官方公钥校验，防止下载渠道被劫持，拦截带病毒的篡改安装程序。
2.电子邮件 S/MIME 签名
发送方签名邮件正文，接收方验证签名，识别钓鱼伪造邮件，确认邮件发件人真实身份。
3.Git 代码提交签名
开发者私钥签名代码提交记录，仓库管理员验证签名，防止账号被盗后他人提交恶意代码，追溯代码责任人。
...
> 答：
1.数字签名本身不携带任何时间信息，无法自动证明签名生成的真实时间。
2.验证签名时间的解决方案：可信时间戳服务 TSA
操作流程：文件完成签名后，将签名哈希发送给权威第三方 TSA 机构；TSA 使用自身私钥对「哈希 + 标准世界时间」二次签名生成时间戳。
核验时同时校验文件原始签名与 TSA 时间戳，即可证明该文件在时间戳标注时刻前已完成签名。
...
> 答：
1.实验选用 2048 位 RSA 的原因：安全与性能均衡，当前主流商用、教学标准，短期算力无法暴力分解模数，加解密、签名速度适中。
1.1024 位 RSA：不安全，现代集群算力可完成模数分解，存在被破解风险，行业已全面淘汰，禁止用于正式业务。
3.4096 位 RSA：安全性更高，但密钥、签名文件体积翻倍，加解密、签名验证运算速度大幅下降，仅高安全场景（金融根证书、政务根 CA）使用，普通业务、实验无需选用。
...
> 答：
是否有意义
有实际业务意义，多层签名可构建信任链路、流程审批凭证。典型应用场景
1.数字证书信任链
根 CA 机构签名二级 CA 证书，二级 CA 再签名用户个人证书；客户端逐层校验多级签名，确认证书可信。
2.多级审批流程
员工 Alice 起草文件并签名，部门主管 Bob 审批后对 Alice 的签名二次签名，证明文件经过两级审核，留存完整审批证据。
3.区块链多签交易
多节点依次对交易签名，只有全部节点完成背书签名后，交易才会上链，实现多方共同授权。
