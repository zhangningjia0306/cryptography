...
| :--- | :------- |
| 签名 RSA 密钥长度（位） |2048|
| 签名私钥 `signature_private_key.pem` 文件大小（字节） |1.7KB|
| 签名公钥 `signature_public_key.pem` 文件大小（字节） |451|
| 签名公钥指数 e 的值 |65537|
| 签名公钥模数 N 的前 16 位十六进制 |00455c0b0064da05|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） |1.7KB|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护。发送者：杨连盛，学号：2024010033|
| 密文文件 encrypted_message.bin 大小（字节） |256|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |9405892d95644c438dd42d49244a1a979cf98c04228e042d7a18f05d7b4f1e2|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 解密后的文件是否与原文件一致 |cmp message.txt decrypted_message.txt && echo "解密结果一致"|
...
> 答：身份认证（Authentication）：证明消息确实来自持有私钥的发送者。本实验中，用发送方的签名私钥对密文签名，接收方用对应的签名公钥验证，只有私钥持有者才能生成通过验证的签名。

完整性保护（Integrity）：确保消息未被篡改。任何对密文的修改都会导致哈希值改变，签名验证失败（如篡改实验所示）。

不可否认性（Non-repudiation）：发送方无法否认自己发送过该消息，因为私钥是唯一的，只有发送方本人可生成有效签名。
...
> 答：效率：非对称签名运算慢，直接对任意长度文件签名不可行；哈希函数可将任意长度数据压缩为固定长度（如 256 位），签名只对哈希值进行，速度快。

输入长度限制：RSA 等算法对输入数据长度有限制（≤ 密钥长度），哈希后能保证输入在允许范围内。

标准化：哈希值是固定长度，使签名接口统一，便于实现和互操作。
...
> 答：2048 位 RSA 密钥生成的签名长度为 256 字节（2048 位 = 256 字节），因为签名结果就是模数 N 的完整长度。
若使用 4096 位密钥，签名长度将为 512 字节（4096 位 = 512 字节）。
...
> 答：抗原像性：给定哈希值，难以找到原像。若缺失，攻击者可从签名反推出消息或构造伪造签名。

抗第二原像性：给定消息，难以找到另一条消息具有相同哈希。若缺失，攻击者可让签名者签署一个“合法”消息，而后用该签名绑定另一个恶意消息。

抗碰撞性：难以找到任意两条不同消息哈希相同。若缺失，攻击者可伪造两个不同消息的签名冲突，实现欺骗。
...
> 答：SHA-1 已被证明存在碰撞攻击（Google 与 CWI 于 2017 年实现）。攻击者可构造两个内容不同但 SHA-1 哈希相同的文件（如一份正常合同和一份恶意合同），让签名者签署其中一个，之后将签名“搬运”到另一个文件上，从而欺骗验证者。因此 SHA-1 不再被推荐用于数字签名。
...
攻击者获得公钥后，可以验证签名或加密消息，但不能伪造签名或解密，因为这两者需要对应的私钥。
...
> 答：加密（使用公钥）保证机密性，确保只有持有私钥的接收方能阅读消息。

签名（使用私钥）保证身份认证和完整性，证明消息来源且未被篡改。

加密不能代替签名，因为加密只能防止窃听，不能证明发送者身份；而且任何人只要拥有公钥都能加密，无法确认真实来源。
...
> 答：不可否认性指发送方不能否认自己发送过某消息。数字签名使用非对称密钥，只有发送方持有私钥，因此签名绑定其身份，可提供不可否认性。
而消息认证码（MAC）使用对称密钥，收发双方共享同一密钥，接收方也可伪造 MAC，因此无法向第三方证明来源，不能提供不可否认性。
...
> 答：软件发布：开发者对软件包签名，用户验证签名以确保软件未被篡改且来自官方。

电子邮件签名（如 S/MIME、PGP）：验证邮件发件人身份并防止邮件内容被篡改。

代码提交签名（如 Git 签名）：确保提交确实来自声称的开发者，防止账号冒用。
...
> 答：数字签名本身不包含时间信息，无法证明签名生成的确切时间。
解决方案是引入可信时间戳服务（TSA）：将签名数据的哈希值发送给 TSA，TSA 返回包含当前时间且用其私钥签名的凭证。接收方可验证该凭证，从而确认签名时间。
...
> 答：1024 位 RSA 已被认为不够安全，目前可被强力破解（建议抛弃）。

2048 位是当前广泛接受的安全基线，兼顾安全性与性能。

4096 位提供更高安全余量，但加解密和签名速度明显变慢，常用于对长期安全性要求极高的场景（如根证书）。
一般应用选择 2048 位即可满足需求。
...
在证书链中，上级 CA 对下级证书签名，形成信任链。

在多方审批流程中，多个签名者依次签署，表示共同认可。

在区块链中，多重签名可提高资金安全性（如 2-of-3 多签）。
