...
| 是否为公开 CA / 第三方 CA 签发 |`是，C=US, O=Let's Encrypt, CN=E7`|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） | `C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Certificate Authority, CN=Liu Jianing Root CA`|
| 证书颁发者（Issuer） |` C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Certificate Authority, CN=Liu Jianing Root CA`|
| 主题和颁发者是否相同 |`是`|
| 证书有效期（天数） |`3650天`|
| 公钥长度（位） |`4096 bit`|
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |`C4:9D:D9:6B:D4:80:03:10:EA:6C:E9:F9:0A:6A:56:C4:AB:89:0D:3D:24:65:98:35:47:F5:7E:99:96:8B:54:1B`|
| SHA-1 指纹（完整） |`37:0D:4E:83:46:CE:F2:A2:D6:83:CD:B5:78:36:61:3B:28:6C:F3:68`|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |`操作系统 / 浏览器内置的全球可信第三方根 CA（如 DigiCert、Let’s Encrypt 根证书）`|`操作系统 / 浏览器内置的全球可信第三方根 CA（如 DigiCert、Let’s Encrypt 根证书）`|
| 浏览器默认是否信任 |`是，系统预装根证书，打开 HTTPS 网站无安全警告`|`否，浏览器默认不信任，需手动导入根证书到系统信任列表才不会报风险`|
| 证书签发者是否为第三方 CA |`是，由独立第三方商业 / 免费 CA 机构签发`|`否，完全自建：根 CA、中间 CA 均由自己生成、自主签发`|
| 适合的使用场景 |`面向互联网所有公众访问的公开网站、线上服务，所有用户无需额外配置即可正常访问`|`企业 / 实验室内网系统、内部管理平台、测试环境、仅内部人员使用的服务，不对外公网公开`|
...
> 答：
本质：数字证书是由可信 CA 对「用户公钥、身份信息、有效期、用途约束」做数字签名的结构化文件，相当于公钥的电子身份证，把公钥和实体身份绑定。
解决问题：解决公钥分发时的中间人攻击，证明某公钥确实属于对应的个人 / 网站 / 设备。
仅数字签名不足的原因：
数字签名只能证明 “这份数据没被篡改、签名者持有对应私钥”，但无法证明签名人的公钥真实身份；中间人可以伪造公钥并自行签名，接收方无法辨别公钥归属，必须依靠 CA 背书的证书完成身份绑定。
---
...
> 答：
自签名证书：实体用自身私钥给自己的公钥签发证书，没有第三方 CA 背书，证书签发者 = 证书主体。
浏览器警告原因：没有全球公认可信第三方验证身份，无法排除中间人伪造证书劫持流量，存在数据窃听、篡改风险。
适用场景：内网测试、实验室环境、企业内部系统，所有使用设备可手动导入根证书建立信任；禁止公网对外服务使用。
---
...
> 答：
证书链作用：分层分担签发压力、隔离安全风险、灵活分级管理信任，形成根 CA→中间 CA→终端证书信任传递链条。
根 CA 不直接签发终端证书的原因：
安全风险：根私钥是整个信任体系核心，频繁离线签发会提升泄露风险；分层后根 CA 仅离线签发少量中间 CA，根私钥极少使用。
管理规模：公网海量网站，根 CA 无法承载全部签发任务；中间 CA 可分布式负责业务签发、吊销、续期。
权限隔离：可通过pathlen限制中间 CA 签发层级，防止单一 CA 失控无限签发下级证书。
---
...
> 答：
根 CA 虽自签名，但信任并非来自签名，而是人为预装信任。
系统 / 浏览器信任逻辑：操作系统、Chrome、Firefox 等浏览器厂商会人工审核机构资质，将合规根 CA 证书内置到系统信任库；用户打开网站时，只要证书链最终追溯到内置根证书，就默认信任。
私有自建根 CA 不会预装，因此浏览器默认不信任，必须手动导入信任库。
---
...
> 答：
pathlen 在 basicConstraints 中，用于限制 CA 可向下签发的中间证书层级，防止证书无限分层扩散风险。
pathlen:0：该中间 CA不能再签发任何下级 CA 证书，仅能签发网站、设备等终端实体证书（本次实验中间 CA 配置）。
pathlen:1：该 CA 最多再签发 1 层中间 CA，下层中间 CA 无法继续签发 CA 证书；允许二级分层，适合大型企业多级内网 CA 架构。
---
...
> 答：
证书过期后果：浏览器 / 客户端直接抛出安全警告，阻断 HTTPS 连接，判定证书无效，无法建立加密通道。
设置有效期的原因：
① 私钥存在泄露风险，限时强制更换密钥降低长期泄露危害；
② 身份信息（企业、域名）会变更，到期强制更新实体信息；
③ 简化证书吊销机制，过期证书自动失效，减少 CRL/OCSP 维护压力。
Let’s Encrypt 仅 90 天：鼓励自动化续期（ACME 协议），缩短有效期能快速淘汰泄露、废弃证书；同时降低证书被盗用后的攻击窗口期，规避长期未吊销带来的安全隐患。
---
...
> 答：
需要吊销的场景：私钥泄露、域名转让、企业注销、证书申请信息造假、服务器被入侵。
CRL 与 OCSP 区别：
CRL（证书吊销列表）：CA 定期生成完整吊销证书清单，客户端下载完整列表本地校验；缺点：文件体积大、更新延迟，占用带宽。
OCSP（在线证书状态协议）：客户端向 OCSP 服务器单独查询单张证书状态，实时获取吊销结果；轻量化、实时性强，是现代浏览器主流方案。
---
...
> 答：
现代证书必须使用 SAN：旧标准仅依靠 CN 字段匹配域名，浏览器已废弃仅校验 CN 的逻辑，只以 SAN 列表为准，无 SAN 会判定证书无效。
SAN 与 CN 区别：
CN：仅单一通用名字段，早期用于填写主域名，现已不作为域名校验依据；
SAN：扩展字段，可写入多条 DNS 域名、IP，支持多域名共用一张证书，包含主域名、备用域名、通配符域名。
...
> 答：
计算方式：对完整证书二进制内容做 SHA-1/SHA256 哈希，输出冒号分隔的十六进制字符串，即指纹。
证书钉扎（Certificate Pinning）：客户端 / 服务端预先保存合法证书指纹，建立连接时校验服务器证书指纹是否匹配；防止可信根 CA 被劫持、恶意 CA 签发伪造证书劫持流量，额外增加一层校验屏障。
---
...
> 答：
匹配规则：*.example.com 仅匹配一级子域名：www.example.com、mail.example.com；不匹配根域名example.com、不匹配多级子域名a.www.example.com。
局限性：
① 仅覆盖单一层级子域名，无法跨多级子域；
② 若通配符私钥泄露，旗下所有子域名服务全部受影响；
③ 部分机构、内网隔离场景出于风险管控，禁止使用通配符证书。
---
...
> 答：
CT（Certificate Transparency）：一套公开、可审计的证书日志系统，所有公网可信 CA 签发证书后必须同步写入公共 CT 日志。
解决核心问题：防止恶意 CA 在未告知域名所有者的情况下，私自签发伪造域名证书；域名所有者、监管机构可公开检索日志，及时发现非法证书并吊销；浏览器会校验证书是否录入 CT 日志，未录入直接拦截。
---
...
> 答：
**证书链**：3n7er.space站点证书 ← R3中间CA ← ISRG Root X1根CA
运营主体是非营利组织 ISRG，各大科技企业出资承担运维成本，不靠证书收费盈利；
依靠 ACME 协议全自动化流程，无人工审核，大幅削减人力开销；
分层 CA 架构：离线根仅签发中间 CA，由 R3 承载海量网站签发，扩容成本低。
**ACME 域名所有权验证两种方式**
HTTP-01：在网站/.well-known/acme-challenge/下放校验文件，CA 公网访问核验，仅支持单域名；
DNS-01：给域名添加_acme-challenge TXT 解析记录，CA 查询 DNS 核验，支持通配符证书。
**90 天短有效期 + 强制自动续期的原因**
短有效期：缩短私钥泄露后的攻击窗口期，强制定期轮换密钥，废弃证书会自动失效，减轻吊销系统压力；
依赖自动续期：90 天周期太短，手动续期极易遗忘导致网站证书过期报错；ACME 自带自动续期工具，可提前自动重签部署证书。
---
...
> 答：
**相同点**
证书格式标准一致：均遵循 X.509 v3 标准，包含 Subject、Issuer、有效期、SAN、Basic Constraints 等扩展；
证书链结构一致：都采用「根 CA→中间 CA→终端服务器证书」分层签发模型，依靠信任链传递合法性；
密码算法体系一致：均使用 RSA/ECC 非对称加密、SHA256 哈希签名。
**不同点**
信任根不同：
真实 PKI（Let’s Encrypt）根证书预装在浏览器 / 操作系统，全局默认信任；
实验私有 PKI 自建根 CA，系统无预装，浏览器默认不信任，必须手动导入。
签发主体：
真实 PKI 由第三方合规 CA 机构签发，受行业规范监管；私有 PKI 完全自主签发，无外部第三方审计。
适用范围：
真实 PKI 面向全球公网服务，所有互联网用户无需配置即可信任；私有 PKI 仅适用于内网、实验室，信任范围仅限手动导入根证书的设备。
吊销与审计：
真实 PKI 强制接入 CT 日志、OCSP/CRL 吊销体系，公开可审计；私有 PKI 无强制日志，吊销机制需要自行搭建。
**浏览器信任差异原因**
浏览器校验证书链能否追溯到系统内置可信根列表：Let’s Encrypt 根证书在预装列表内，信任链合法；你的自建根 CA 不在预装列表，信任链断裂，因此浏览器弹出不安全警告。
---
