...
| :--- | :------- |
| 签名 RSA 密钥长度（位） |2048|
| 签名私钥 `signature_private_key.pem` 文件大小（字节） |1740.8|
| 签名公钥 `signature_public_key.pem` 文件大小（字节） |460|
| 签名公钥指数 e 的值 |65537|
| 签名公钥模数 N 的前 16 位十六进制 |   `00:a7:cc:0f:c9:2a:48:cb:cd:63:71:03:54:2d:12:15:`|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） | 1740.8|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护。发送者：刘珈宁，学号：2024010005 |
| 密文文件 encrypted_message.bin 大小（字节） | 256|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |d4fa97924551d8a76e197a7e492e2207ea0211a7b0a039e3f6a56fe54cbb80c8 |
...
> 答：三个关键保证：身份认证（Authentication）、完整性保护（Integrity）、不可否认性（Non-repudiation）
-身份认证（Authentication）
含义：能够确认消息的发送方身份，证明该消息一定是持有对应签名私钥的用户发出，杜绝他人伪造发送者身份，等同于现实中的手写签名，可确认签署人本人。
实验实现说明：
本次实验使用 signature_private_key.pem（签名私钥）对消息 message.txt 生成数字签名；接收方仅能通过配套公钥 signature_public_key.pem 完成验签。
私钥仅由发送者刘珈宁持有，其他人无法伪造有效签名；只要验签通过，就能确定消息一定由私钥持有者发出，完成身份认证。
-完整性保护（Integrity）
含义：保障消息在传输、存储全程没有被篡改，只要原文发生任意微小修改，数字签名的验证操作就会直接失败，立刻发现数据改动。
实验实现说明：
签名本质是对消息哈希值使用私钥加密。本次实验中，只要修改 message.txt 内任意一个字符、增删文字，消息 SHA-256 哈希值就会完全改变；使用原签名 + 公钥验签时会校验失败。
哪怕仅改动学号末尾数字，也能直接检测出篡改，实现消息完整性保护。
-不可否认性（Non-repudiation）
含义：消息发送者事后无法否认自己发送过这条消息；因为签名生成唯一依赖发送者独有的私钥，第三方可通过公钥核验签名归属，具备举证效力。
实验实现说明：
本次实验签名由仅发送者掌握的 signature_private_key.pem 生成，公钥对外公开，任何第三方都能用 signature_public_key.pem 核验签名合法性。
私钥不存在共享、泄露前提下，发送者刘珈宁无法否认这条带签名消息是自己发出的，完成不可否认性保障。
...
> 答：
-运算效率原因
RSA 属于非对称加密算法，大数模幂运算速度极慢，仅适合处理少量短数据。
本次实验密文encrypted_message.bin固定 256 字节，如果直接对完整密文执行 RSA 签名，运算开销更大；而 SHA-256 哈希运算速度极快，无论原始数据长短，都能快速压缩为固定 32 字节（256 位）哈希摘要，仅对 32 字节短摘要做 RSA 签名，耗时仅毫秒级，大幅提升签名、验签效率。
-RSA 输入长度限制问题
2048 位 RSA 模数单次最多只能处理小于 256 字节的原始数据块，且 RSA 签名标准并非设计用于直接处理任意长度完整文件。
本实验密文刚好 256 字节，若换成更大文件会直接超出 RSA 单次处理上限；先哈希可把任意大小数据统一压缩为 256 位固定长度摘要，完美适配 RSA 输入长度限制，不受原始密文 / 明文体积约束。
-算法标准化、接口统一
哈希输出长度固定为 256 位，让 RSA 签名的输入接口完全统一。
在本次实验中，不管原始message.txt长短、加密后密文体积变化，签名阶段的输入永远是固定长度 SHA-256 哈希值；程序、OpenSSL 命令不需要区分不同长度文件做适配，签名逻辑简洁通用，符合密码学标准规范。
...
> 答：
-单位换算规则
存储文件单位为字节(Byte)，密钥长度单位为比特(bit)，换算公式：`1 Byte = 8 bit`
\[2048 \div 8 = 256\]
-RSA签名数学约束
RSA签名运算的结果是小于模数 \(N\) 的整数，模数的比特长度与密钥比特长度完全相等。
因此二进制签名的最大存储长度固定等于密钥比特长度，转换成字节即为 `密钥比特数 ÷ 8`。
本次实验 `signature.bin` 文件大小为256字节，与计算结果完全吻合，和原始待签名数据长度无关。
-4096位RSA密钥对应的签名长度计算
按照相同换算逻辑：
\[4096 \div 8 = 512\]
若使用4096 bit RSA密钥，生成的二进制签名文件固定为 **512字节**。
...
> 答：## 一、哈希函数保障签名安全的三个核心性质
-抗原像性（Preimage Resistance）
定义：给定一段哈希输出值 \(h\)，攻击者在计算上不存在可行方法，逆向求出满足 \(H(m)=h\) 的原始消息 \(m\)。
签名场景作用：数字签名本质是对消息哈希值进行私钥运算 \(\sigma = H(m)^d \bmod N\)；攻击者仅拿到签名与哈希值，无法反推原始明文。
不满足的安全危害：若抗原像性失效，攻击者拿到密文哈希与签名后，可以逆向还原出完整原始消息，泄露传输明文内容，失去数据保密能力。
-抗第二原像性（Second Preimage Resistance）
定义：给定已知消息 \(m_1\)，无法计算出另一条完全不同的消息 \(m_2(m_2 \neq m_1)\)，使两者哈希满足 \(H(m_1)=H(m_2)\)。
签名场景作用：确保不存在第二条替换消息，能够复用原有合法签名。
不满足的安全危害：攻击者拿到合法消息 \(m_1\) 与对应签名后，可以构造恶意替换消息 \(m_2\)；两条数据哈希一致，原有签名对恶意消息同样验签通过，实现消息篡改、伪造欺骗接收方。
-抗碰撞性（Collision Resistance）
定义：无法主动找到任意一对互不相同的消息 \(m_1 \neq m_2\)，满足哈希等式 \(H(m_1)=H(m_2)\)。
签名场景作用：防止攻击者提前预制一对哈希相同、内容完全不同的消息，完成签名欺诈。
不满足的安全危害：攻击者提前构造「正常消息+恶意消息」一对碰撞数据，使用私钥对正常消息完成合法签名；后续将传输内容替换为恶意消息，公钥验签依然判定有效，伪造虚假业务数据、伪造凭证，破坏完整性与身份认证。
-三者安全层级关系
抗碰撞性要求 > 抗第二原像性 > 抗原像性；
若哈希不满足抗碰撞性，则一定同时不满足抗第二原像性，签名体系会彻底失去完整性保护能力。
---
...
> 答：
-SHA-1 不再推荐用于数字签名的原因：
SHA-1 已经被攻破，不满足哈希函数必需的抗碰撞性安全要求。2017 年 Google 与阿姆斯特丹 CWI 研究所公开了 SHA-1 碰撞攻击方案，攻击者可以构造两份内容完全不同、但 SHA-1 哈希值完全相等的文件。而数字签名依赖哈希的抗碰撞性保证消息完整性，一旦存在哈希碰撞，签名校验逻辑会被绕过，无法区分合法内容与恶意篡改内容，因此现代数字签名场景不再使用 SHA-1，本实验选用安全性更强的 SHA-256。
-2017 年 SHA-1 碰撞攻击对数字签名的影响：
数字签名的核心流程是先计算文件哈希，再对哈希值使用私钥签名；验签时会重新计算文件哈希，与签名中保存的哈希比对。
若两份文件哈希完全一致，原本针对正常文件 A 生成的合法签名，对恶意文件 B 验签时同样会判定有效。签名者仅认可文件 A，但攻击者替换成恶意文件 B 后，接收方验签依然通过，直接破坏数字签名提供的完整性、身份认证、不可否认性三大安全保障，造成伪造、篡改欺诈风险。
-攻击者利用漏洞的实例说明
攻击者预先制作一组 SHA-1 碰撞文件：
文件 A（无害文件）：正常合规的合同、单据，是签名方愿意签署的内容；
文件 B（恶意文件）：修改金额、篡改条款的虚假版本，二者内容完全不同，但 SHA-1 哈希值相同。
攻击者将无害文件 A 交给签名人，签名人使用私钥对 A 生成合法数字签名，并对外发布该签名。
攻击者在传输、交付环节，把文件替换为恶意文件 B，同时附上原本对 A 生成的签名。
接收方校验时，计算恶意文件 B 的 SHA-1 哈希，与签名内哈希完全匹配，验签成功，误判这份恶意文件是签名人认可的合法文件，最终遭受损失。
...
> 答：
-签名密钥对用途
**签名私钥（signature_private_key.pem）**用于**生成数字签名**：发送方对密文计算SHA-256哈希后，使用该私钥对哈希值进行RSA运算，生成signature.bin数字签名，证明消息由本人发出。私钥仅发送方持有，绝不对外公开。
**签名公钥（signature_public_key.pem）**用于**验证数字签名**：公钥可以公开分发，接收方拿到密文、签名后，使用该公钥校验签名合法性，判断密文是否未被篡改、是否为对应私钥持有者发送。
-加密密钥对用途
**加密公钥（encryption_public_key.pem）**用于**加密明文消息**：任何人都可以使用该公钥对message.txt明文加密，生成encrypted_message.bin密文；只有配套私钥能解密。
 **加密私钥（encryption_private_key.pem）**用于**解密密文**：仅接收方持有，收到密文后，使用该私钥执行RSA解密，还原出原始明文message.txt。
-攻击者获取公钥后的能力限制
**攻击者可以做的操作**：
用加密公钥加密任意明文，生成密文发给接收方；
用签名公钥验证任意已存在的数字签名，校验签名是否合法、核对数据完整性；
查看公钥内的模数、指数等公开参数。
-**攻击者无法做到的操作**：
无法解密任何密文：解密必须加密私钥，仅公钥不具备解密能力；
无法伪造有效数字签名：生成合法签名必须签名私钥，仅有公钥不能生成可通过校验的签名；
无法推导、逆向算出对应的私钥：2048位RSA大数分解计算量极大，仅凭公钥无法破解私钥；
无法篡改已有签名并通过验签：修改数据后哈希改变，仅靠公钥无法重新生成匹配的合法签名。
---
...
> 答：结合实验流程区分二者本质
**实验完整流程**：明文message.txt → 加密公钥加密得到密文encrypted_message.bin → 计算密文 SHA-256 哈希 → 签名私钥对哈希签名得到signature.bin → 接收方先用签名公钥验证签名 → 验签通过后用加密私钥解密密文。
消息加密：作用对象是原始明文，使用加密公钥运算，输出密文；仅负责隐藏消息内容。
数字签名：作用对象是密文的哈希摘要，使用签名私钥运算，输出签名文件；仅负责校验发送者身份与数据完整性。
**二者各自解决的安全问题**：
-消息加密（RSA 加密）
解决数据保密性问题：
明文经过公钥加密后变为无意义二进制密文，传输过程中即使被截获，攻击者没有加密私钥就无法还原原始文字，防止消息内容泄露。
本实验中加密操作保证只有持有加密私钥的接收方可以读取消息内容。
-数字签名（RSA 签名）
同时解决三类安全问题：
身份认证：只有发送者持有签名私钥，合法签名可证明消息由该发送方发出；
完整性保护：数据任意修改都会改变哈希值，签名验证直接失败，检测篡改；
不可否认性：私钥唯一归属发送方，事后无法否认自己发送过这条消息。
本实验接收方先验签，能提前拦截篡改、伪造的密文，避免无效解密操作。
**不能用加密简单替代签名的三大核心原因**:
-密钥使用逻辑完全相反，安全目标冲突
加密：公钥加密、私钥解密，任何人可加密，仅持有者能解密，目标是保密；
签名：私钥签名、公钥验签，仅持有者能生成合法签名，任何人可核验，目标是身份与完整性；
如果用私钥 “加密” 充当签名：公钥解密人人都能读取原始数据，完全失去保密能力；如果用公钥加密充当签名：任何人都能生成密文，无法证明发送者身份，不存在身份认证、不可否认性。
-无法提供不可否认性
单纯加密仅能保证内容不泄露，任何人拿到公钥都能生成密文；无法证明密文是谁发送的。
而签名依赖独有的私钥生成，第三方可通过公钥核验归属，发送者无法抵赖，加密做不到这一点。
-无法高效校验完整性，且存在长度限制
加密直接作用完整数据，效率极低；签名仅对固定长度哈希摘要运算，速度更快；
加密不会自动校验数据是否被篡改：密文被篡改后解密只会得到乱码，无法提前拦截；签名验签阶段就能直接识别篡改，提前丢弃恶意数据；
RSA 单次处理长度有限，长消息加密有分段开销；签名依靠哈希压缩，适配任意长度数据。
---
...
> 答：# 答：
**不可否认性（Non-repudiation）定义**
不可否认性是密码学安全特性之一：**消息发送方无法事后否认自己发送过这条消息**，接收方或第三方仲裁者可以拿出客观、可信的证据证明发送行为，具备举证效力。
**数字签名可以提供不可否认性的原因**
数字签名基于**非对称RSA密钥体系**，密钥分为唯一私钥、公开公钥：
-签名仅由发送方独有的**签名私钥**生成，私钥只有发送者本人持有，其他人无法伪造合法签名；
-配套签名公钥完全公开，任意第三方都能使用公钥核验签名；
-验签通过即可客观证明：该签名只能由对应私钥持有者产生。
一旦完成验签，发送者无法否认自己生成过这条消息的签名，实现不可否认性。
结合本次实验：仅发送者拥有`signature_private_key.pem`，任何人都能用`signature_public_key.pem`验证签名，可作为发送行为的有效证据。
**消息认证码MAC无法提供不可否认性**
MAC依靠**单一对称共享密钥**完成生成与校验，存在致命缺陷：
-对称密钥由通信双方（发送方、接收方）共同持有，双方都可以用密钥生成合法MAC值；
-若出现纠纷，第三方无法区分MAC到底是发送方生成，还是接收方自己伪造的；
-发送方可以抵赖：声称MAC是接收方私自伪造，没有中立证据判定责任，无法实现不可否认。
**核心对比总结**
-RSA数字签名（非对称）：一私一公，私钥唯一归属发送人 → 可举证，支持不可否认；
-MAC对称认证码：双方共用同一密钥 → 双方均可伪造，无法区分责任，不支持不可否认。
---
...
> 答：
-场景 1：软件 / 安装包发布
应用方式：软件开发者使用自己专属签名私钥对安装包生成数字签名，对外公开配套签名公钥；用户下载软件后，使用公钥校验签名。
**解决的问题**
完整性：防止安装包被黑客植入病毒、木马，文件任意篡改都会导致签名验证失败；
身份认证：即便官网被入侵、哈希值被篡改，攻击者没有开发者私钥就无法伪造合法签名，用户能确认安装包确实来自官方开发者；
不可否认性：若软件存在恶意后门，可凭借签名证明该安装包由对应开发者发布。
-场景 2：企业电子邮件签名（商务转账、公文邮件）
应用方式：发件人用个人签名私钥对邮件正文生成数字签名，接收方使用发件人公开公钥完成验签。
**解决的问题**
身份防伪造：杜绝仿冒老板、客户邮箱的钓鱼邮件，接收方可确认邮件确实是声明发送人发出；
完整性校验：传输途中邮件内容被篡改（修改转账金额、收款账户）时，验签直接失败；
责任追溯：涉及资金、合同类邮件，签名可作为凭证，发件人无法否认发送过该指令。
-场景 3：开源代码提交 / Git 代码签名
应用方式：程序员本地用私钥对代码提交记录（Commit、PR）签名，仓库管理员使用开发者公钥校验提交签名。
**解决的问题**
账号冒用防护：即使攻击者盗用 GitHub 账号，没有开发者私钥也无法生成有效签名，管理员可识别伪造提交；
代码完整性：代码提交记录被恶意篡改、插入漏洞代码时，签名校验不通过；
代码归属确权：每一行带签名的提交都能锁定真实开发者，出现安全漏洞时可精准追溯责任人。
...
> 答：
-单纯数字签名本身无法证明签名真实时间
RSA 数字签名仅包含文件哈希与私钥运算结果，签名数据内部不携带任何可信时间信息：
你今天用私钥签名一份文件，一年后完全可以拿出一模一样的文件，重新生成一份新签名；两份签名格式完全一致，无法区分是一年前还是昨天生成，签名本身不存在可信时间凭证，无法反驳 “昨天才签名” 的抵赖说法。
-接收方仅靠签名无法核验时间，必须依赖可信时间戳服务（TSA）
**完整流程解决办法：**
**签名方操作**
① 对文件哈希生成数字签名；
② 将该签名的哈希值发送给官方可信时间戳机构 TSA；
③ TSA 使用自身可信时钟读取标准全球时间，对签名哈希加盖权威时间戳（TSA 用自身私钥签名时间 + 哈希）；
④ 签名方把「原始文件 + RSA 签名 + TSA 时间戳」一并交付接收方。
**接收方核验步骤**
① 先用签名公钥验证文件 RSA 签名合法，确认文件未篡改、由发送方签署；
② 使用 TSA 公开根公钥验证时间戳，时间戳内带有 TSA 权威记录的精确签署日期；
③ 时间戳由中立第三方机构背书，发送者无法篡改、伪造时间，以此锁定这份签名在 TSA 记录的当日就已存在。
-如何反驳 “一年后声称昨天才签名”
可信时间戳由独立中立第三方机构生成，机构拥有不可篡改的标准时钟：
时间戳证据可以客观证明：该签名对应的文件哈希，早在一年前就提交给 TSA 完成存证，不可能是昨天才生成签署；发送者无法篡改 TSA 出具的时间凭证，法院、第三方仲裁均可采信该时间作为有效证据，彻底解决时间抵赖问题。
...
> 答：
> -**本实验选择 2048 位 RSA 密钥的原因**：
**安全符合行业规范**
NIST、国内商用密码标准均将 2048 位 RSA 定为通用业务最低安全基线，能够抵御当前已知的大数分解、暴力破解攻击，完全满足本次加密、数字签名实验的安全需求，可实现身份认证、完整性、不可否认性三大安全保障。
**性能均衡适配实验环境**
RSA 模幂运算开销随密钥长度平方上升，2048 位密钥在 OpenSSL、Git Bash 中签名、加密、解密操作均为毫秒级，运行流畅，不会出现长时间等待，适合课堂快速复现完整实验流程。
**贴合真实工业场景**
2048 位是当前软件签名、邮件加密、普通电子凭证最通用的密钥规格，实验选用该长度能还原真实线上业务的使用逻辑。
**-1024 位 RSA 是否足够安全**：
1024 位 RSA 已不安全，正式场景完全不推荐使用
**安全层面**：1024 位模数存在成熟的优化分解算法，大型算力集群可在数周内破解公钥并还原私钥，攻击者可伪造合法签名、解密传输密文；NIST 早在 2015 年就废止 1024 位 RSA，禁止用于签名、敏感数据加密等业务。
**仅有的适用场景**：无任何隐私、敏感数据的临时本地调试，无法用于本次带身份认证与存证要求的实验。
-**4096 位 RSA 是否有必要**
**优势**：
安全强度远高于 2048 位，在经典计算机算力持续提升的背景下，可提供数十年长期安全防护，适合高密级、高价值数据场景，例如金融核心交易系统、长期司法存证电子合同、国家级涉密文件。
**性能缺陷**：
运算效率大幅降低：4096 位签名、加解密耗时约为 2048 位的 3~4 倍；
存储与传输成本翻倍：密钥、签名文件体积是 2048 位的 2 倍，占用更多磁盘与网络带宽；
老旧嵌入式、低性能设备易出现运算超时、兼容报错。
...
> 答：# 答：
-**Alice签名后Bob再对该签名二次签名有意义**
-Alice签名：证明原始文件由Alice出具，保障文件完整性与Alice身份；
-Bob二次签名：证明Bob核验并认可Alice的签名凭证，锁定Alice的签名未被篡改。
仅保留单层签名无法完整追溯双方权责，嵌套签名可分层留存每一方的确认证据。
**需要链式多重签名的场景**
-**多级审批单据**
经办人签名后，各级主管依次对前序签名二次签名，出现问题可沿签名链定位对应审核责任人。
-**CA证书信任链**
根CA签名二级CA证书，二级CA再签名用户证书，客户端依靠根公钥逐层核验所有下游证书合法性。
-**电子公文/合同流转存证**
起草人、合作方、公证处逐层嵌套签名，完整留存全部参与方确认记录，具备司法举证效力。
-**软件多级分发校验**
源码开发者、打包方、分发平台逐层嵌套签名，可逐层排查代码篡改环节。
---
