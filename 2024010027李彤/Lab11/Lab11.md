...
![计算密文哈希值]![alt text](image.png)

![生成密文签名]![alt text](image.png)
...
![验证密文签名失败]![alt text](verify_fail.png)
...
| :--- | :------- |
| 签名 RSA 密钥长度（位） |2048 |
| 签名私钥 `signature_private_key.pem` 文件大小（字节） | 1741|
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | 460|
| 签名公钥指数 e 的值 | 65537|
| 签名公钥模数 N 的前 16 位十六进制 | 00:bd:c0:48:21:83:9d:c6:05:6a:45:55:18:b0:74:40|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） |1741|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：李彤，学号：2024010027|
| 密文文件 encrypted_message.bin 大小（字节） | 256|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |cc58af67474c88c345d7c53efa8f633441ae7ad21a0353bd1cef31a61f1c9636 |
...
> 答：
> 1.身份认证
> 含义：能够确认消息的发送者身份，证明数据一定由持有对应签名私钥的主体生成，无法伪造发送人。
> 实验实现：本次使signature_private_key.pem私钥生成签名，接收方依靠配套公钥signature_public_key.pem验签；私钥仅本人持有，只有私钥持有者能生成Verified OK的合法签名，由此确认密文发送者是私钥持有人。
>  2. 完整性保护
> 含义：保证数据传输、存储全程未被篡改，文件任意字节改动都会导致签名验证失败。
> 实验实现：签名基于密文 SHA256 哈希值生成；实验中复制密文追加字符得到tampered_encrypted_message.bin，使用原签名验证直接输出Verification failure，证明密文篡改可被立刻检测。
>  3. 不可否认性
> 含义：发送者事后无法否认自己发送过该消息；私钥唯一归属发送人，第三方可使用公开公钥核验签名，具备举证效力。
> 实验实现：签名仅能由专属私钥生成，公钥完全公开，任何人都可核验签名；发送者无法否认自己生成过这份密文的签名。
...
> 答：
1.提升运算效率RSA 
非对称模幂运算速度极慢，只适合短数据；SHA256 哈希可将任意长度文件压缩为固定 32 字节摘要，仅对短摘要签名，耗时大幅降低。 
2. 突破 RSA 输入长度限制
2048 位 RSA 单次仅能处理小于 256 字节的数据，长文件无法直接签名；哈希统一输出固定 256 位摘要，适配算法输入上限，不受文件大小约束。
 3. 标准化统一接口
无论原始数据长短，签名输入固定为 256bit 哈希值，算法逻辑统一，无需针对不同长度文件做适配，符合密码学标准规范。
---
...
> 答：换算规则：1 字节 (Byte)=8 比特 (bit)，2048÷8=256，因此 2048 位 RSA 密钥生成的二进制签名固定 256 字节，与本次signature.bin文件大小一致。RSA 签名运算结果是小于模数 N 的整数，模数比特长度等于密钥比特长度，签名存储长度等于密钥比特长度除以 8。
4096 位 RSA 密钥签名长度：4096÷8=512 字节。
...
> 答：1.抗原像性
> 定义：给定哈希值 h，无法逆向求出满足 H (m)=h 的原始消息 m。
> 安全作用：攻击者拿到哈希与签名，不能反推原始数据，防止明文泄露；失效后攻击者可逆向还原消息，泄露传输内容。
>  2. 抗第二原像性
> 定义：给定消息 m₁，无法构造另一不同消息 m₂，使 H (m₁)=H (m₂)。
> 安全作用：无法复用原有合法签名篡改消息；失效后攻击者可构造替换文件，复用签名欺骗接收方。
>  3. 抗碰撞性
> 定义：无法找到任意两组不同消息 m₁≠m₂，哈希值完全相等。
> 安全作用：防止攻击者预制一对哈希相同的文件实施签名欺诈；失效后攻击者可提前准备正常 / 恶意碰撞文件，替换内容后签名依然验签通过
...
> 答：1.淘汰原因：2017 年 Google 公开 SHA-1 碰撞攻击，可构造两份内容完全不同、哈希值一致的文件，破坏哈希抗碰撞性，无法保障签名完整性，因此现代签名场景禁用 SHA-1，本实验采用 SHA256。 2. 对数字签名的影响：签名流程依赖文件哈希比对，两份文件哈希一致时，原本针对正常文件的合法签名，对恶意文件验签同样有效，直接绕过完整性校验，伪造虚假数据。 3. 攻击示例：攻击者制作碰撞文件 A（合规合同）、文件 B（篡改金额的恶意合同），二者 SHA-1 哈希相同；让签名人对 A 生成合法签名，交付时替换为 B，接收方验签通过，误认恶意文件为签名人认可的内容。
...
> 答：签名私钥signature_private_key.pem：生成数字签名，仅发送方持有，保密；
> 签名公钥signature_public_key.pem：验证数字签名，可对外公开。  
> 加密公钥encryption_public_key.pem：加密明文，任何人可使用；
> 加密私钥encryption_private_key.pem：解密密文，仅接收方持有，保密。 
> 攻击者获取公钥后的能力 可做：加密任意明文、验证已有签名、读取公钥模数与指数；不可做：解密密文、伪造合法签名、逆向破解私钥、篡改已有签名并通过验签
...
> 答：RSA 加密：解决保密性，明文加密为乱码，无加密私钥无法读取内容，防止传输窃听；
数字签名：解决身份认证、完整性、不可否认性，确认发送人、检测篡改、发送者无法抵赖。
不能用加密替代签名
1.密钥逻辑相反：加密是公钥加密私钥解密；签名是私钥签名公钥验签，安全目标完全冲突；
2.无不可否认性：任何人都能用公钥加密，无法证明数据发送者，无法追责；
3.无法提前拦截篡改：密文篡改后解密只会输出乱码，不能提前识别恶意数据；签名验签阶段直接拦截篡改文件；
4.性能缺陷：加密直接处理完整文件效率极低，签名仅对哈希摘要运算，速度更快。
...
> 答：1.不可否认性定义：消息发送者无法事后否认发送过该消息，第三方可凭客观证据判定发送行为，具备司法举证效力。
>  2. 数字签名支持不可否认性：基于非对称密钥，签名私钥唯一归属发送人，配套公钥完全公开，任意第三方均可核验签名；私钥仅发送者持有，无法伪造合法签名，发送人无法抵赖。
>  3. MAC 无法提供：MAC 依靠对称共享密钥，发送、接收双方共用同一密钥，双方均可生成合法 MAC；出现纠纷时第三方无法区分 MAC 是谁生成，发送者可抵赖，无有效举证依据。
...
> 答：1.软件安装包发布
> 开发者私钥签名安装包，用户用官方公钥验签；防止安装包被植入病毒、官网被篡改，确认安装包来自官方开发者。
>  2. 商务电子邮件（资金、合同指令）
> 发件人私钥签名邮件正文，接收方公钥验签；抵御仿冒老板钓鱼邮件，防止转账金额、收款账户被篡改，邮件可作为责任凭证。
>  3. Git 开源代码提交签名
> 程序员私钥对代码提交记录签名，仓库管理员使用开发者公钥校验；防止账号被盗用伪造代码提交，精准追溯漏洞代码责任人。
...
> 答：1.数字签名本身无法证明签署时间，签名数据不含可信时间，一年后可重新生成一模一样的签名，无法区分签署时间。 2. 验证签名真实时间方案：使用可信时间戳服务 TSA完整流程：① 发送方对文件生成 RSA 签名；② 将签名哈希发送给中立第三方 TSA 机构；③ TSA 使用权威标准时钟记录精确时间，对哈希加盖 TSA 数字时间戳；④ 交付文件时附带原始签名 + TSA 时间戳。接收方核验：先验文件 RSA 签名，再核验 TSA 时间戳，第三方机构出具的时间不可篡改，可证明签名早在 TSA 记录日期已存在，反驳 “昨天才签名” 的抵赖
...
> 答：
> 1.本实验选用 2048 位 RSA：行业通用安全基线，NIST 商用标准最低要求；安全强度足够抵御现有算力攻击；运算性能均衡，签名、加密毫秒级完成，适配课堂实验与普通业务。 
> 2. 1024 位 RSA 不安全：现有集群算力可在短时间内分解模数破解私钥，攻击者可伪造签名、解密密文；官方早已淘汰，禁止用于签名、敏感数据加密。
>  3. 4096 位 RSA 仅特定场景有必要：优势：安全强度更高，可长期抵御未来算力提升，适合金融核心交易、长期司法电子存证等高密级场景；缺陷：运算速度是 2048 位的 3~4 倍，密钥、签名文件体积翻倍，占用更多存储与带宽，普通业务使用会造成性能浪费，无强制需求无需选用。
...
> 答：多层签名有实际意义，每层签名留存对应主体确认凭证，分层追溯责任。
> 1.流程含义：Alice 对原始文件签名，证明文件由 Alice 出具；Bob 对 Alice 的签名二次签名，证明 Bob 核验并认可 Alice 的签名，锁定 Alice 的签名未被篡改。 
> 2. 需要多重链式签名的场景① 多级审批电子单据：经办人签名后，各级主管逐层嵌套签名，出现问题沿签名链定位对应审核责任人；② CA 证书信任链：根 CA 签名二级 CA 证书，二级 CA 再签名用户证书，客户端逐层核验完成信任校验；③ 电子合同多方存证：起草人、合作方、公证处逐层签名，完整留存全部参与方确认记录，具备司法效力；④ 软件多级分发校验：源码作者、打包方、分发平台依次嵌套签名，快速定位篡改环节。
