...
| 证书有效期（起始-结束） |2026-05-22 20:16:05 GMT — 2026-08-20 20:16:04 GMT |
| 公钥算法 |id-ecPublicKey（椭圆曲线） |
...
![浏览器证书查看](browser_cert.png)[alt text]

![OpenSSL 查看并验证证书]![alt text](openssl_cert.png)
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |1B:F1:0F:64:27:32:06:CE:8B:BC:95:6B:17:01:18:7D:56:FB:6A:BF:BA:53:17:C7:9B:22:5F:2B:16:91:CF:08 |
| SHA-1 指纹（完整） |5C:EF:3B:5F:3A:13:93:2A:13:D2:2F:8F:C2:F1:42:6A:57:13:3A:74 |

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任 | ISRG Root X1（系统预装公开根证书）|My Root CA（自建自签名根证书） |
| 浏览器默认是否信任 | 是，操作系统内置信任库| 否，需手动导入根证书才可信任|
| 证书签发者是否为第三方 CA |是（Let's Encrypt 公开合规 CA） | 否，完全自建私有 CA 体系|
| 适合的使用场景 |公网对外网站、面向互联网的公开服务 |企业内网、本地开发测试、内部隔离业务系统 |
...
> 答：数字证书是由可信 CA 签发的、绑定实体身份与公钥的 X.509 结构化文档，包含实体 DN、公钥、有效期、CA 数字签名。仅靠数字签名只能证明消息未篡改、发送方持有私钥，但无法证明公钥归属，攻击者可伪造虚假公钥签名实施中间人攻击；数字证书依靠权威 CA 对公钥做背书，解决公钥可信性问题，弥补纯数字签名的缺陷。
...
> 答：自签名证书是持有者使用自身私钥签名自身公钥，无第三方权威机构背书。
浏览器默认不信任，因为根证书未预装在系统信任库，无法校验合法性，存在中间人劫持风险。
适用场景：本地开发调试、物理隔离的企业内网、无公网访问的内部系统；禁止用于公网对外服务。
...
> 答：根 CA 私钥安全等级极高，通常离线冷存储，不适合频繁签发海量终端证书；分层 CA 架构拆分签发压力、隔离安全风险。
若根 CA 直接签发所有终端证书：根私钥高频使用，泄露风险大幅上升；一旦根私钥泄露，全网所有证书全部失效；无法单独吊销某一批网站证书，运维成本极高。分层结构实现权限隔离、风险拆分、批量吊销管控。
...
> 答：根证书自签名，不存在密码学手段证明可信，依靠厂商人工信任机制：操作系统、浏览器厂商人工审核 CA 机构资质，将其根证书预装至本地信任库；厂商定期审计、移除发生安全事故的根 CA
...
> 答：pathlen:0：该 CA 仅能签发终端实体证书，禁止再创建下级 CA；
pathlen:1：允许再签发一层中间 CA，整条证书链最多包含两层子 CA。
pathlen 限制证书链最大层级，防止攻击者盗用中间 CA 私钥构建多层伪造 CA，阻断大规模中间人攻击。
...
> 答：证书过期后浏览器直接拦截网站，提示连接不安全，拒绝建立 HTTPS 会话。
有效期强制定期轮换密钥，降低私钥长期泄露带来的攻击窗口期；限制证书有效周期，缩小泄露后的影响范围。
Let’s Encrypt 采用 90 天短证书：配合 ACME 自动化续期脚本，减少长期密钥泄露隐患，全程自动化无需人工操作，规避人工遗忘续期导致业务中断。
...
> 答：现代浏览器仅识别 SAN 扩展，完全忽略 CN 通用名字段，因此必须配置 SAN。
CN 仅为历史兼容字段，仅能填写单个域名；SAN 可同时存放多个域名、通配符域名，一张证书适配多个站点。
...
> 答：指纹是对整张证书完整 DER 二进制数据做哈希运算得到的唯一标识，不属于证书内置字段，实时计算生成。
证书钉扎：客户端本地存储服务器证书指纹，每次连接比对指纹；即便攻击者持有合法 CA 签发的伪造证书，指纹不匹配会直接断开连接，抵御 CA 被入侵后的中间人攻击。
...
> 答：*.myserver.com 仅匹配一级子域名（api.myserver.com、mail.myserver.com）；无法匹配多级域名（a.api.myserver.com）。
局限性：任意子域名私钥泄露，整张证书全部失效；仅覆盖同一后缀一级域名，不支持多级子域。
...
> 答：证书透明度是一套公开日志系统，所有公网 HTTPS 证书必须录入公共 CT 日志。
任何人可查询某域名全部已签发证书，快速发现 CA 违规签发的伪造域名证书，及时监控中间人攻击证书。
...
> 答：Let’s Encrypt 是非盈利免费公开 CA，依托 ACME 自动化协议签发证书。
域名验证方式：HTTP-01（站点放置验证文件）、DNS-01（添加 TXT 解析记录），证明申请者拥有域名控制权。
90 天短证书 + 自动续期：全程自动化，无需人工审核，短有效期减少密钥泄露风险，脚本自动续期避免过期
...
> 答：相同点：均遵循 X.509 v3 证书标准，都采用「根 CA - 中间 CA - 终端证书」三层信任链结构，签名、验证密码学流程完全一致。
不同点：信任根不同，真实 PKI 根证书预装在系统，浏览器默认信任；私有根证书需要手动导入信任库；真实 CA 受第三方合规审计，私有 CA 无公开监管；真实证书面向公网，私有仅用于内网测试。
