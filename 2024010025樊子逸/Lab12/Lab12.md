...
| :--- | :------- |
| 访问的网站 | www.baidu.com |
| 证书主题（Subject） | C=CN, ST=beijing, L=beijing, O=Beijing Baidu Netcom Science Technology Co., Ltd, CN=baidu.com|
| 证书颁发者（Issuer） |C=BE, O=GlobalSign nv-sa, CN=GlobalSign RSA OV SSL CA 2018 |
| 证书有效期（起始-结束） |Jul 9 07:01:02 2025 GMT ~ Aug 10 07:01:01 2026 GMT |
| 公钥算法 |rsaEncryption |
| 签名算法 |sha256WithRSAEncryption |
| 证书链层级数 | 3|
| 根 CA 名称 |GlobalSign Root CA - R3 |
| OpenSSL 验证结果（Verify return code） |0 (ok) |
| 是否为公开 CA / 第三方 CA 签发 |是（GlobalSign） |
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | 08:E6:B5:B5:C6:53:F3:CD:AE:1C:83:C5:28:1E:14:DF:7F:7D:5F:45:BE:4F:77:E7:5C:5B:42:AA:FF:D6:5E:74|
| SHA-1 指纹（完整） | 7F:20:53:CA:A3:19:82:81:15:D5:38:1B:D5:6E:94:9C:0E:11:71:61|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |GlobalSign Root CA - R3 | My Root CA（自签名）|
| 浏览器默认是否信任 |是 | 否|
| 证书签发者是否为第三方 CA | 是| 否|
| 适合的使用场景 |公网 HTTPS 服务 |内网开发、测试、教学 |
...
> 答：数字证书的本质是一个由可信第三方（CA）签名的数据结构，它将一个公钥与一个实体的身份信息绑定在一起。它解决了公钥分发过程中的信任问题——即如何确认拿到的公钥确实属于声称的实体。单独的数字签名只能保证消息的完整性和来源认证，但无法确保公钥本身的真实性，数字证书通过引入 CA 作为信任锚点来解决这个问题。
...
> 答：自签名证书是由证书所有者自己签名生成，Issuer = Subject，没有第三方 CA 为其背书。浏览器会警告自签名证书不安全，是因为无法验证其身份的真实性——任何人都可以生成一张声称是任意域名的自签名证书。适用场景：内部测试、开发环境、个人加密、企业内部系统（可手动信任）。
...
> 答：需要证书链的原因：①安全性：根 CA 私钥离线保存，不直接签发终端证书，降低泄露风险；②可撤销性：中间 CA 可被吊销而不影响根 CA；③管理灵活性：不同中间 CA 可负责不同业务。如果根 CA 直接签发所有终端证书，私钥使用频繁易暴露，且吊销管理复杂。
...
> 答：根 CA 证书是自签名的，我们信任它基于非技术的社会工程——操作系统和浏览器厂商通过严格的审核流程（合规审计、安全控制、财务稳定等）决定将哪些 CA 根证书预装进信任存储库。用户依赖厂商的决策，默认信任预装的根证书。信任是人决策的结果，而非数学证明。
...
> 答：pathlen:0 表示该 CA 证书下不能再有其他 CA 证书，只能签发终端实体证书。若为 pathlen:1，则允许该 CA 再签发一层子 CA。pathlen 限制了证书链的最大深度，防止无限延伸，增强安全性。
...
> 答：证书过期后浏览器会警告用户连接不安全，并可能阻止访问。有效期限制的必要性：①减少私钥泄露风险；②强制更新证书以应用新的加密算法；③便于撤销（过期证书自然失效）。Let's Encrypt 的 90 天有效期是为了鼓励自动化续期，缩短安全风险窗口，减少吊销列表大小。
...
> 答：需要吊销证书的情况：私钥泄露、证书错误颁发、域名变更、组织变更等。CRL（证书吊销列表）是 CA 定期发布的被吊销证书列表，客户端下载并查询，实时性差但离线可用。OCSP（在线证书状态协议）是实时查询单个证书状态，响应快但需在线支持。
...
> 答：SAN 扩展允许在一个证书中列出多个域名。现代浏览器强制要求证书包含 SAN，并忽略 CN 字段中的域名。区别：CN 只能填一个域名，SAN 可以包含多个；CN 是旧标准，SAN 是 v3 扩展，支持更灵活的域名表示。
...
> 答：证书指纹是对证书整个二进制数据的哈希值（如 SHA-256）。证书钉扎（Certificate Pinning）是一种安全机制：应用在首次连接时记录服务器证书指纹，后续只接受指纹匹配的证书，即使证书链验证通过，指纹不匹配也拒绝连接，用于防范 CA 被入侵后签发伪造证书的中间人攻击。
...
> 答：通配符证书如 *.example.com 可匹配所有一级子域名（如 mail.example.com, api.example.com），但不能匹配二级及更深（如 a.b.example.com）。局限性：泄露后影响所有子域名；某些安全策略对通配符有额外要求；不支持多级子域名。
...
> 答：证书透明度（CT）是一个公开日志系统，所有 CA 必须将签发的证书提交到多个 CT 日志中，日志公开可查。它解决了欺诈性证书问题：如果 CA 被入侵或违规签发了未经授权的域名证书，域名所有者或公众可查询 CT 日志发现异常，从而及时吊销。
...
> 答：Let's Encrypt 通过 ACME 协议自动化证书签发。验证域名控制权的方式：HTTP-01（在域名特定路径下放置 token 文件，通过 HTTP 访问验证）、DNS-01（在 DNS TXT 记录中放入指定值，适合通配符证书）、TLS-ALPN-01（在 TLS 握手时验证）。证书有效期短（90 天）以鼓励自动化续期，降低未续期风险，减少私钥泄露窗口。
...
> 答：相同点：证书格式（X.509）、证书链结构（根-中间-终端）、签名验证过程完全一致。不同点：真实 PKI 的根证书预装在系统信任库，浏览器默认信任；私有 PKI 的根证书未预装，需手动信任。真实 PKI 由公开可信的 CA 签发，私有 PKI 由自己创建。浏览器不信任私有证书是因为无法验证其根证书的合法性。
