...
| 项目 | 你的结果 |
| :--- | :------- |
| 签名 RSA 密钥长度（位） |2048bit|
| 签名私钥 `signature_private_key.pem` 文件大小（字节） |约 1740（1.7K）|
| 签名公钥 `signature_public_key.pem` 文件大小（字节） |460|
| 签名公钥指数 e 的值 |65537（0x10001）|
| 签名公钥模数 N 的前 16 位十六进制 |00:c5:2d:1d:77:64:6d:b7|
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） |约 1740（1.7K）|
...
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |637524ebefc9f9455ce0b61a936f9e1f6c211fa465040a20ba0a105ae75ea72b|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 解密后的文件是否与原文件一致 |完全一致，终端输出「解密结果一致」|
...
> 答：数字签名具备身份认证、完整性、不可否认三大特性：
1.身份认证：仅私钥持有者可生成合法签名，本次实验使用专属签名私钥生成 signature.bin，他人无对应私钥无法伪造，公钥核验可确认密文由本人发出。
2.完整性：签名绑定密文 SHA256 哈希，本次追加字符篡改 tampered_encrypted_message.bin 后，哈希改变，验证直接失败，证明文件中途未被修改。
3.不可否认性：私钥唯一归属本人，不存在第三方拥有，事后无法否认发送过该密文与消息。
...
> 答：1.效率限制：2048 位 RSA 仅适合短数据，直接加密大文件速度极慢；SHA256 快速压缩为 32 字节摘要，签名运算毫秒完成。
2.长度约束：RSA 运算输入不能超过模数长度，任意长度密文经哈希统一为固定 256bit，满足算法输入规范。
3.标准化：无论密文长短，签名输入统一为哈希摘要，工具、算法接口统一通用。
...
> 答：2048 位 RSA 模数 = 2048 bit = 256 字节，因此 signature.bin 大小为 256 字节；若使用 4096 位 RSA 密钥，模数 4096bit，签名文件大小为 512 字节。
...
> 答：1.抗原像：给定哈希无法反推密文，攻击者拿到摘要不能还原 encrypted_message.bin；
2.抗第二原像：找不到另一份密文拥有相同 SHA256，无法篡改后复用原有签名；
3.抗碰撞：不存在两份不同密文哈希完全相同，杜绝替换文件绕过校验。
任一性质失效，攻击者可构造恶意文件，签名校验失效。
...
> 答：2017 年公开 SHA1 碰撞攻击，可制作两份内容不同但哈希一致的文件；攻击者正常文件签名后替换恶意文件，验证依旧通过。本实验采用 SHA256，无公开碰撞漏洞，安全性更高。
...
> 答：1.签名密钥：私钥生成数字签名，公钥公开用于校验；
2.加密密钥：公钥加密明文，私钥解密密文；
3.攻击者获取公钥：仅能加密、验证签名；无法伪造签名、解密密文、推导私钥。
...
> 答：RSA 加密：仅保障消息保密性，外人无法读取，但无法验证发送人、无法检测篡改；
RSA 签名：保障身份 + 完整性 + 不可否认，任何人可读取密文，但能确认来源、发现篡改；
二者不可互相替代，本实验流程先加密保密、再签名验真，同时实现两种安全目标。
...
> 答：不可否认：发送者无法否认发送消息。数字签名依托唯一私钥，第三方可独立核验，具备凭证效力；MAC 使用对称密钥，收发双方共用密钥，任意一方均可伪造，无法追责，无不可否认能力。
...
> 答：1.软件安装包校验：厂商私钥签名安装包，用户公钥核验，防止劫持病毒包；
2.Git 代码签名：开发者签名 commit，防止他人冒用账号提交恶意代码；
3.企业加密邮件 S/MIME：私钥签名邮件，抵御钓鱼仿冒邮件。
...
> 答：数字签名本身不含时间，无法证明签署时间；解决方案：可信时间戳 TSA，将文件哈希提交权威机构，机构带标准时间签名，可证明签名发生在指定时间前。
...
> 答：1024 位：算力可分解模数，现已淘汰；
2048 位：安全与性能平衡，本实验选用，短期无破解风险；
4096 位：安全更强，但密钥、签名体积翻倍，生成 / 验证速度大幅下降，仅金融等高密场景使用。
...
> 答：Alice 签名后 Bob 再对签名二次嵌套无实际业务价值；多重签名有效场景：
1.证书信任链：根 CA 签名二级 CA，二级 CA 签发用户证书；
2.门限签名钱包：多人密钥分片，达到指定人数签名才能转账。
