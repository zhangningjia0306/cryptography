...
| 是否为公开 CA / 第三方 CA 签发 |是（Let's Encrypt 免费公共 CA）|
...
| 证书颁发者（Issuer） |C=CN, ST=Yuxi, O=My Root CA, OU=Certificate Authority, CN=My Root CA|
| pathlen 值 |0|
| 证书有效期（天数） |1825 天（5 年）|
...
| 证书颁发者（Issuer） |C=CN, ST=Yuxi, O=My Organization, OU=IT Department, CN=My Intermediate CA|
| CA 字段的值 |CA:FALSE|
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |SHA256 Fingerprint=XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX|
| SHA-1 指纹（完整） |SHA1 Fingerprint=XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX:XX|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |ISRG Root X1（全球通用预装根 CA）|自建 My Root CA（仅本机手动信任）|
| 浏览器默认是否信任 |是（系统内置信任库）|否，需手动导入根证书信任|
| 证书签发者是否为第三方 CA |是（Let's Encrypt 公共权威 CA）|否，自建私有 CA，无官方公信力|
| 适合的使用场景 |公网对外网站、线上服务|内网测试、教学实验、企业内部系统|
...
> 答：1、数字证书本质：一套由可信 CA 签名的 X.509 标准数据包，内容包含实体身份信息、持有者公钥、证书有效期、各类扩展字段以及 CA 的数字签名。
2、解决核心问题：数字签名只能保证消息完整性与发送方身份可信，但无法证明公钥本身没有被攻击者篡改 / 替换；数字证书依靠第三方可信 CA 担保公钥归属，解决公钥可信性、中间人公钥劫持问题。
3、仅数字签名的缺陷：攻击者可以替换整套公钥、签名、文件，用户用伪造公钥校验依然显示通过；证书引入权威 CA 作为信任锚，公钥被 CA 签名保护，攻击者无法伪造合法证书公钥，弥补数字签名的信任短板。
...
> 答：1、自签名证书：证书持有者使用自身私钥对自己的身份与公钥签名，Issuer 和 Subject 完全一致，不存在上级 CA。
2、浏览器警告原因：操作系统 / 浏览器内置信任库没有收录该自签名证书的根公钥，无法验证证书合法性，存在中间人劫持风险，因此弹出安全告警。
3、适用场景：内网开发调试、校园 / 企业内部测试环境、本地 HTTPS 实验教学；不允许用于公网对外服务。
...
> 答：1、安全隔离：根 CA 私钥离线严格保管，极少参与签名；若直接签发海量网站证书，根私钥频繁联网，泄露风险大幅提升；分层后仅中间 CA 日常签发证书，根私钥长期离线。
2、风险可控：中间 CA 私钥泄露时，仅需吊销该中间 CA，不会波及整条体系；根 CA 一旦泄露，所有网站证书全部失效，修复成本极高。
3、业务分层：不同中间 CA 可划分不同业务板块（政企、互联网），便于分类管理、分级管控证书签发权限。
4、路径长度限制：通过pathlen约束链条长度，防止攻击者利用被盗中间 CA 无限生成下级伪造 CA。
...
> 答：1、信任逻辑：根证书无上级机构担保，依靠浏览器、操作系统厂商人工审核后，提前内置到系统信任库，厂商完成安全背书，因此用户设备默认信任。
2、筛选规则：系统厂商（微软、苹果、Linux 发行方）会严格审核 CA 机构资质、安全运维规范、吊销机制；只有合规、安全记录良好的 CA，才会纳入预装信任列表；出现安全事故的 CA 会被移除信任库。
...
> 答：1、pathlen:0：该 CA 仅能签发终端实体证书（服务器、客户端证书），不允许再签发任何下级 CA 证书，限制证书链只能两层（根→中间→网站）。
2、若改为pathlen:1：允许该中间 CA 再创建一层次级 CA，证书链条可以延长至四层，会扩大安全攻击面，一旦中间 CA 私钥泄露，攻击者可伪造多层 CA 批量签发虚假域名证书。本实验限制 0，遵循最小权限安全原则。
...
> 答：1、过期后果：浏览器直接拦截网站，提示证书无效、禁止访问；程序、接口 HTTPS 连接直接报错，无法建立加密通道。
2、设置有效期目的：密钥长期使用存在泄露风险，有效期强制定期更换密钥，缩小安全风险窗口；同时便于定期清理废弃域名、失效机构证书，配套 CRL/OCSP 吊销机制。
3、Let’s Encrypt 仅 90 天：完全自动化免费证书，短有效期降低证书被盗用的危害；依托 ACME 自动续期脚本，无需人工操作，频繁更新不会增加运维成本。
...
> 答：1、吊销场景：服务器私钥泄露、域名转让、机构注销、证书申请信息造假、CA 判定证书存在安全漏洞。
2、CRL（证书吊销列表）：CA 定期发布完整吊销证书清单，客户端完整下载全列表比对，文件体积大、更新存在延迟。
3、OCSP（在线证书状态协议）：客户端向 CA 在线发送单条证书查询，实时获取该证书是否吊销，流量小、实时性更强，现代 HTTPS 主流方案。
...
> 答：1、传统仅依靠 CN 字段存储域名存在兼容性缺陷，现代浏览器不再读取 CN，仅识别 Subject Alternative Name（SAN）扩展，无 SAN 会判定域名不匹配，拦截网站。
区别：
2、CN 仅单一通用名称，只能填写一个域名；SAN 支持多条 DNS 域名，可配置多个普通域名 + 通配符域名，一张证书服务多个站点。
...
> 答：1、计算方式：将整张证书完整 DER 二进制数据做 SHA1/SHA256 哈希，输出十六进制指纹，每张证书指纹唯一。
2、证书钉扎：客户端（APP / 程序）提前存储合法服务器证书指纹，后续每次连接服务器时比对指纹；即使攻击者拿到其他合法 CA 签发的伪造证书，指纹不一致直接拒绝连接，抵御中间人劫持。
...
> 答：1、可匹配：一级子域名，如www.myserver.com、mail.myserver.com、api.myserver.com。
2、局限：无法匹配多级子域名，例如 dev.www.myserver.com；通配符仅覆盖域名前缀，不能匹配根域名myserver.com（需单独写入 SAN）；通配符证书泄露后，旗下所有子域名全部存在风险。
...
> 答：1、CT（Certificate Transparency）：一套公开日志系统，所有 CA 签发证书必须上传公共日志，全网可检索。
2、解决问题：防止 CA 违规秘密签发虚假域名证书（如当年 DigiNotar 事件）；域名所有者可定期检索日志，快速发现非法证书并吊销，及时拦截中间人攻击证书。
...
> 答：1、原理：Let’s Encrypt 是免费公共非盈利 CA，依靠 ACME 自动化协议验证域名所有权后自动签发证书。
2、ACME 协议：通过 HTTP 文件验证、DNS 解析验证两种方式，证明操作者拥有目标域名控制权，无需人工审核，全自动签发。
3、90 天短有效期：免费证书无人工运维，缩短风险周期；配合自动续期脚本，用户无需手动更新，大幅降低证书被盗用带来的安全危害。
...
> 答：相同点
都遵循 X.509 v3 标准，证书链结构一致（根→中间→终端），均使用 RSA/ECDSA+SHA256 签名，包含有效期、SAN、密钥用途扩展。
不同点
1、信任根：3n7er.space 根 ISRG Root X1 预装在系统；自建根 CA 需要手动导入信任。
2、签发机构：前者第三方合规公共 CA，经过厂商安全审核；后者私有自建 CA，无官方背书。
3、使用场景：公网线上业务可用自建仅内网测试。
4、浏览器行为：公网证书直接放行；自建证书弹出安全告警，阻断访问。
