...
| 证书有效期（起始-结束） | Not Before: May 22 12:16:05 2026 GMT
Not After : Aug 20 12:16:04 2026 GMT|
| 公钥算法 |id-ecPublicKey（椭圆曲线 P-256） |
| 签名算法 |ecdsa-with-SHA384 |
| 证书链层级数 | 3|
| 根 CA 名称 | C=US, O=Internet Security Research Group, CN=ISRG Root|
| OpenSSL 验证结果（Verify return code） | 0 (ok)|
| 是否为公开 CA / 第三方 CA 签发 |是（Let’s Encrypt 公开第三方 CA） |
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | 3C:3E:36:31:71:D9:37:8B:4F:CA:9F:B8:2D:95:86:AE:FD:10:DF:9F:CC:AF:AC:2B:26:0F:08:07:9C:71:9C:AC|
| SHA-1 指纹（完整） | DC:51:8D:16:B4:AD:B0:72:EA:BD:43:7A:70:99:BF:27:0B:1E:9A:9B|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 | ISRG Root X1（互联网全球公认根 CA）| 自建 My Root CA（个人私有根证书）|
| 浏览器默认是否信任 |是（浏览器预装该根证书，默认信任） |否，需要手动将根证书导入系统 / 浏览器信任库才会被信任 |
| 证书签发者是否为第三方 CA |是，Let’s Encrypt 国际公开第三方 CA |否，完全自建层级 CA 体系（根 CA→中间 CA→服务器证书） |
| 适合的使用场景 | 公网对外网站、面向全网公众访问的业务，全球通用可信| 企业内网系统、内部测试环境、局域网服务、内部加密通信，仅内部可信|
...
> 答：
数字证书本质是由可信 CA 签发、绑定实体身份与公钥的结构化电子凭证，采用数字签名防篡改，证明某个公钥确实归属对应个人 / 服务器。
解决的核心问题：公钥分发的中间人攻击问题。单纯数字签名只能证明数据未被篡改、签名者持有私钥，但无法确认对方公钥是否真实属于目标主体，攻击者可伪造公钥冒充他人；数字证书借助可信 CA 背书，对公钥 + 身份整体签名，为公钥建立可信绑定，补齐信任链条。
...
> 答：自签名证书：证书签发者（Issuer）与证书主体（Subject）完全一致，由实体自己用自身私钥给自己公钥签名，没有第三方 CA 背书。
浏览器告警原因：操作系统 / 浏览器预装可信根列表，自签名根不在内置信任库，浏览器无法验证合法性，判定存在中间人劫持风险，弹出安全警告。
适用场景：企业内网测试环境、局域网内部服务、开发调试环境、内部加密通信；不可用于公网对外网站。
...
> 答：必要性：采用三级证书链（根 CA→中间 CA→终端证书）实现权限隔离与风险隔离。根 CA 是整个信任体系基石，必须极致安全，极少离线使用；由中间 CA 承接日常签发业务，降低根私钥暴露、泄露概率。
根 CA 不直接签发终端证书原因：
安全风险：频繁使用根私钥签发证书，泄露概率大幅提升，一旦根私钥泄露，整个信任体系全部崩塌；
管理臃肿：海量网站直接由根签发，证书管理、吊销、运维无法落地；
分层分权：可给不同中间 CA 分配不同签发权限，便于机构分级管理。
...
> 答：
信任逻辑：根证书是自签名，信任并非密码学验证得来，属于预置信任。操作系统、浏览器厂商经过严格资质审核，将合规公共根 CA 证书预装到系统内置信任列表，人为预设信任。
判定规则：程序校验证书链时，若整条链条最终回溯到内置信任列表内的根证书，则整条证书链判定可信；不在列表内则默认不信任。
---
...
> 答：pathlen 用于限制证书链可嵌套的中间 CA 最大层数，写在中间 CA 证书 Basic Constraints 扩展中：
pathlen:0：该中间 CA不能再签发新的下级中间 CA，仅允许直接签发网站终端证书；
pathlen:1：该中间 CA 最多还能再签发一层中间 CA，下级中间 CA 不可再继续派生新 CA；
作用：限制证书链深度，防止无限嵌套 CA，缩小私钥泄露带来的连锁信任风险。
...
> 答：过期后果：浏览器拦截 HTTPS 访问，弹出证书过期安全警告，无法正常建立加密连接；程序校验证书有效期失败，SSL 握手中断。
设置有效期原因：
密钥存在长期泄露风险，定期强制更换公私钥；
应对哈希、加密算法破解演进，定期迭代证书算法；
规避域名所有权变更、企业主体变更带来的身份失效问题。
Let’s Encrypt 仅 90 天原因：免费自动化签发，短有效期倒逼站点部署自动续期脚本；减少证书长期被盗用、冒用的危害；降低批量吊销证书的运维压力。
...
> 答：吊销场景
域名控制权变更、私钥泄露、服务器被入侵、企业注销、证书申请信息造假、证书误签发，需要在到期前提前作废证书。
CRL 与 OCSP 区别
CRL（证书吊销列表）：CA 定期发布完整吊销名单，客户端下载全列表本地比对；缺点：列表体积越来越大、存在更新延迟、带宽开销高。
OCSP（在线证书状态协议）：客户端单独向 OCSP 服务器查询单张证书实时状态；优点：查询粒度小、实时性强、流量更小；缺点：依赖 OCSP 服务器可用性，存在隐私泄露隐患。
...
> 答：
现代证书普遍携带 SAN 扩展原因：早期仅依靠 CN 字段绑定域名，RFC 规范逐步限制 CN 校验优先级，浏览器不再优先采信 CN；SAN 可以容纳多个域名，一张证书匹配多个站点，是现代 HTTPS 域名匹配的标准实现方式。
SAN 与 CN 区别：
CN：旧规范单一主体字段，仅能填写一个域名，现已不作为域名校验依据；
SAN：专用扩展字段，支持多个普通域名、通配符域名，浏览器优先校验 SAN 列表是否匹配访问域名。
---
...
> 答：计算方式：对完整证书 DER 二进制内容做哈希运算（SHA1/SHA256），得到固定长度摘要，即证书指纹。
证书钉扎（Certificate Pinning）：客户端本地预置可信证书指纹，建立连接后比对服务器证书指纹；若不一致直接拒绝连接，抵御中间人替换合法证书劫持流量，强化移动端 / 客户端通信安全；弊端：证书更换后若未同步更新钉扎配置，会导致服务阻断。
...
> 答：
*.example.com 可匹配：a.example.com、www.example.com、mail.example.com 等一级子域名；不能匹配 example.com 本身、二级子域名 a.b.example.com。
局限性：
仅覆盖同级一级子域名，无法跨层级匹配更深子域名；
一旦私钥泄露，该域名下全部子域名站点同时面临风险；
部分 CA 对通配符证书审核更严格，不支持泛顶级域名申请。
---
...
> 答：CT 即证书透明度，核心是公开日志系统（CT Log），CA 签发证书后必须将证书提交公开日志存档、公开可查。
解决问题：遏制恶意、误签发证书滥用；域名所有者、第三方机构可以定期检索日志，发现针对自身域名的异常伪造证书，及时发现劫持风险；浏览器逐步强制要求 HTTPS 证书必须带有 CT 日志 SCT 凭证才会被信任。
...
> 答：1.免费签发模式：非营利机构运营，依靠企业赞助、公益资金覆盖成本，基于 ACME 自动化协议全自动申请、校验、签发、续期，人工成本极低，实现免费批量签发 DV 域名证书。
2.ACME 域名控制权验证两种主流方式：
HTTP-01：在网站根目录部署 ACME 指定临时文件，Let’s Encrypt 外网访问验证文件存在，证明申请者控制该站点；
DNS-01：给域名添加指定 TXT 解析记录，校验解析内容，验证域名所有权，适用于通配符证书申请。
3.90 天短有效期 + 自动续期原因：短周期降低私钥泄露、恶意证书长期滥用危害；强制站点部署自动续期程序，避免证书遗忘过期大面积故障；适配全自动 ACME 运维模式，适配免费证书运营体系。
结合3n7er.space证书链：终端站点证书→中间 E7 证书→ISRG Root X1 公可信根，构成完整可信任层级。
...
> 答：信任差异原因
3n7er.space证书由 Let’s Encrypt 公共 CA 签发，根证书 ISRG Root X1 预装在 Windows、各大浏览器信任列表，系统默认信任整条证书链；
本次实验自建私有根 CA 未预装进系统信任库，浏览器无法校验根合法性，默认判定不信任。
相同点
证书格式完全一致，均遵循 X.509 v3 标准，包含版本、序列号、公私钥、有效期、扩展字段；
都采用三级链式结构：根 CA→中间 CA→终端服务器证书，依靠外键约束 pathlen、CA 布尔值管控签发权限；
密码学原理一致，均使用非对称签名实现防篡改、身份绑定。
不同点
信任根：公网 PKI 根为全球公认预装根；私有 PKI 根为自建，需手动导入信任；
签发主体：公网第三方合规 CA 受行业监管；私有 CA 完全自主管控，无外部监管；
适用范围：公网证书全网可信；私有证书仅导入根的设备内部可信；
运营规则：公共 CA 有严格域名审核、吊销合规机制；私有 CA 规则自定义。
