...
![alt text](<屏幕截图 2026-06-23 152429.png>)
...
| :--- | :------- |
| 签名 RSA 密钥长度（位） | 2048|
| 签名私钥 `signature_private_key.pem` 文件大小（字节） |1.7K（1712字节） |
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | 460|
| 签名公钥指数 e 的值 |65537（0x10001） |
| 签名公钥模数 N 的前 16 位十六进制 |00:bf:6d:98:2d:d9:e6:57:ca:79:30:d9:c0:0c:38:c3 |
| 加密私钥 `encryption_private_key.pem` 文件大小（字节） |1.7K（1712字节） |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：何正多，学号：2024010037|
| 密文文件 encrypted_message.bin 大小（字节） |256 |
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |9743dad9417864acaa17195ce6e6ba63992956092bbf72876a3cd74b1e854fd4 |
...
| 篡改密文后的签名验证结果 | Verification failure（校验失败，提示 bad signature）|
...
> 答：数字签名的三大核心保证为完整性、身份认证（不可伪造）、不可抵赖，结合本次 RSA 签名实验分别解释如下：
1. 完整性（数据未被篡改）
含义
消息在传输、存储过程中没有被第三方修改、增删任何字节，接收方可以精准检测数据是否发生改动。
实验实现说明
实验中对encrypted_message.bin密文使用signature_private_key.pem生成 SHA256-RSA 签名文件signature.bin；
当执行printf 'x' >> tampered_encrypted_message.bin篡改密文后，验签命令返回Verification failure；未篡改的原始密文验签输出Verified OK。
原理：签名基于文件 SHA256 哈希生成，文件任意修改都会改变哈希值，公钥验签时比对哈希失败，直接判定数据被篡改，保障完整性。
2. 身份认证（来源可信、不可伪造）
含义
接收方可以确认消息确实由对应发送方发出，第三方无法伪造他人的合法签名。
实验实现说明
签名仅能使用发送方独有的 ** 签名私钥signature_private_key.pem** 生成；验证时只能使用配套公开的签名公钥signature_public_key.pem。
私钥仅发送方持有，攻击者没有私钥就无法生成能通过公钥校验的合法签名，接收方通过公钥验签即可确认消息来源是私钥持有者，完成身份认证。
3. 不可抵赖（发送方无法否认发送过该消息）
含义
发送方完成签名后，事后不能否认自己发送过这条消息，接收方可向第三方出示签名作为凭证。
实验实现说明
签名由发送方专属私钥生成，私钥唯一归属发送方，没有其他人能使用该私钥产生合法签名。
本次实验中signature.bin只能由signature_private_key.pem生成，发送方无法否认自己签署过这份密文；公钥可公开给仲裁方核验签名有效性，实现行为留证、不可抵赖。
...
> 答：本实验采用openssl dgst -sha256 -sign先对密文做 SHA256 哈希再签名，不直接对完整密文 RSA 签名
1. 突破 RSA 明文长度限制（最直接原因）
2048 位 RSA 算法单次仅能加密 / 签名不超过 245 字节的数据。本次密文encrypted_message.bin为 256 字节，已经超出 RSA 单次签名上限，无法直接完整签名；
SHA256 哈希输出固定 32 字节摘要，长度远小于 RSA 限制，可直接送入私钥完成签名，适配任意大小文件。
2. 大幅提升签名与验签运算效率
RSA 大数模幂运算速度很慢，文件越大耗时越长；哈希算法 SHA256 是快速单向摘要算法，无论文件多大，都能快速压缩为 32 字节短摘要。
实验中仅对 32 字节哈希做 RSA 私钥运算，相比直接对 256 字节密文运算，极大减少非对称加密的计算开销，尤其大文件场景提升更明显。
3. 保障安全性，规避 RSA 直接签名的安全缺陷
直接对原始数据做 RSA 签名无抗碰撞保护，存在伪造、长度扩展攻击风险；
SHA256 密码哈希具备强单向性、强抗碰撞性：任意微小改动都会完全改变哈希值，结合 RSA 签名后能可靠校验数据完整性，本次篡改密文后验签直接失败就是该特性的体现。同时标准密码规范强制要求哈希 + RSA 签名组合，避免裸 RSA 签名漏洞。
4. 节省存储与传输带宽
原始密文 256 字节，哈希仅 32 字节，最终生成 256 字节签名文件；若直接对完整密文分段签名，会产生多段签名数据，占用更多存储空间、传输流量。先哈希再签名仅生成一份固定长度签名，降低资源开销。
...
> 答：保障数字签名安全的哈希三大核心性质：抗原像性、抗第二原像性、抗碰撞性，分别解释与安全风险如下：
1. 抗原像性（单向性）
定义：给定哈希输出值 h，攻击者无法在可行时间内找到任意原文 m，使得 \(Hash(m)=h\)；哈希正向计算简单、反向推导原文不可行。
不满足带来的安全问题：攻击者拿到签名对应的哈希摘要，可反向还原出原始密文 / 消息，篡改原始数据、伪造可通过验签的假消息，彻底破坏签名完整性与保密性。
2. 抗第二原像性（弱抗碰撞）
定义：给定一条已知原文 \(m_1\)，无法找到另一条不同原文 \(m_2(m_2≠m_1)\)，满足 \(Hash(m_1)=Hash(m_2)\)。
不满足带来的安全问题：攻击者拿到合法消息与对应签名，能构造一条含义完全不同的替换消息，两者哈希值一致；替换后旧签名依然能通过公钥验签，实现消息篡改伪造，对应实验中篡改密文后验签失败的反向漏洞。
3. 抗碰撞性（强抗碰撞）
定义：无法找到两条完全不同的原文 \(m_1≠m_2\)，使得 \(Hash(m_1)=Hash(m_2)\)。
不满足带来的安全问题：攻击者提前构造两组哈希相同但内容完全不同的文件：一份合法文件、一份恶意文件。先用私钥对合法文件签名，之后把签名嫁接到恶意文件上，接收方验签会判定恶意文件合法，完成完整伪造抵赖攻击，破坏数字签名的不可抵赖、身份认证特性。
...
> 答：一、SHA-1 不再推荐用于数字签名的核心原因
SHA-1 哈希长度仅 160bit，密码学安全性不足，2017 年 Google 公开完成SHA-1 强碰撞攻击，证明其不满足数字签名必需的抗碰撞性；
数字签名依赖哈希的强抗碰撞特性保障完整性、防伪造，SHA-1 已被攻破，攻击者可构造两份不同内容但哈希完全一致的文件，绕过签名校验，因此密码标准（NIST 等）全面淘汰 SHA-1，推荐 SHA256/SHA3 系列。
二、SHA-1 碰撞攻击对数字签名的影响
1.破坏完整性：两份内容完全不同的文件哈希值相同，篡改后签名依然能通过验签，无法检测数据修改；
2.破坏不可抵赖性：攻击者可偷换文件内容，让合法签名绑定恶意文件，发送方无法自证清白；
3.完全丧失签名防伪造能力：合法签名可复用在恶意载体上，接收方无法区分文件真伪。
三、攻击者漏洞利用实例
场景：伪造转账凭证
1.攻击者提前构造两份 SHA-1 哈希完全相同的文件：
文件 A（合法）：转账100元给用户
文件 B（恶意）：转账10000元给攻击者
2.正常用户用私钥对合法文件 A 做 SHA-1+RSA 签名，生成合法签名文件sign.bin；
3.攻击者将原始文件替换为恶意文件 B，搭配原签名sign.bin发送给银行；
4.银行验签流程：计算文件 B 的 SHA-1 哈希，用公钥解密签名得到哈希，二者完全匹配，验签输出Verified OK；
5.银行判定恶意转账文件合法，执行大额转账攻击。
...
> 答：1. 签名密钥对（signature 系列）
签名私钥 signature_private_key.pem：仅发送方持有，用于对文件哈希值执行签名操作，生成 signature.bin 签名文件，证明消息由本人发出。
对应实验命令：openssl dgst -sha256 -sign signature_private_key.pem ...
签名公钥 signature_public_key.pem：对外公开分发，用于接收方验证签名合法性，判断文件是否被篡改、确认发送者身份。
对应实验命令：openssl dgst -sha256 -verify signature_public_key.pem ...
2. 加密密钥对（encryption 系列）
加密公钥 encryption_public_key.pem：对外公开，用于发送方加密原始明文，生成密文 encrypted_message.bin。
对应实验命令：openssl pkeyutl -encrypt -inkey encryption_public_key.pem ...
加密私钥 encryption_private_key.pem：仅接收方持有，用于解密密文，还原原始明文 decrypted_message.txt。
对应实验命令：openssl pkeyutl -decrypt -inkey encryption_private_key.pem ...
3. 攻击者获取公钥后的权限限制
攻击者可以做的操作
1.拿到签名公钥：对任意已有的签名文件执行验签，校验文件完整性；无法伪造合法签名。
2.拿到加密公钥：可以向对应接收方发送加密密文（任何人都能用公钥加密数据）。
攻击者绝对不能做的操作
1.无法伪造数字签名：签名依赖私钥，仅有公钥无法生成能通过校验的合法签名，不能冒充他人发送消息。
2.无法解密密文：密文由公钥加密，解密必须配套私钥，仅公钥不能还原明文内容。
3.无法抵赖、篡改凭证：没有私钥就不能生成有效签名，无法制造虚假合法凭证。
...
> 答：一、本质区别与各自解决的问题
1. 消息加密（本实验：明文→加密生成encrypted_message.bin）
操作逻辑：用接收方公钥加密，接收方私钥解密
核心目标：解决保密性，防止第三方窃听，只有接收方能读取明文。
对应实验流程：原始明文用encryption_public_key.pem加密，只有持encryption_private_key.pem的接收方可解密还原文字。
2. 数字签名（本实验：密文哈希→私钥签名，接收方公钥验签）
操作逻辑：发送方私钥签名，所有人用发送方公钥验证
核心目标：解决完整性、身份认证、不可抵赖，证明数据未篡改、确实由发送方发出、发送方无法否认。
对应实验流程：对密文 SHA256 哈希后用signature_private_key.pem生成signature.bin；篡改密文后验签直接失败。
二、不能用加密代替签名的原因
1.密钥流向完全相反
加密是公钥加密、私钥解密；签名是私钥签名、公钥验签。若用加密充当签名，任何人拿到公钥都能 “加密伪造凭证”，无法确认发送者。
2.无法实现不可抵赖
加密密文人人可生成，事后发送方可否认；签名仅发送方私钥能生成，无法抵赖。
3.加密无法校验完整篡改
加密仅隐藏内容，不能检测数据是否被修改；签名依靠哈希比对，任意字节改动都会验签失败。
4.功能分离，需求不同
加密只管 “不让外人看懂”，签名只管 “证明是谁发的、没被改”，二者安全目标独立，无法互相替代。
...
> 答：1. 不可否认性（Non-repudiation）定义
指消息发送方完成签名 / 发送行为后，事后无法否认自己发送过这条消息；接收方可向第三方（仲裁方）出示凭证，证明消息确实由发送方发出，解决交易、通信中的抵赖纠纷。
2. 数字签名可提供不可否认性的原因
数字签名基于非对称 RSA 密钥：
签名仅能由发送方独有的私钥生成，私钥唯一归属发送人，他人无法伪造；
所有人可使用公开配套公钥验签，仲裁机构可独立核验签名合法性；
签名文件可永久留存作为法律凭证，发送方无法否认自己持有私钥并签署消息。
3. MAC 消息认证码无法提供不可否认性的原因
MAC 使用对称共享密钥：
通信双方持有完全相同的同一密钥，发送方和接收方都能生成合法 MAC 值；
若发生纠纷，无法判定 MAC 是发送方生成还是接收方伪造，发送方可以否认消息由自己发出，缺少可中立核验的凭证，因此不具备不可否认性。
...
> 答：场景 1：软件安装包发布签名（如 Windows 驱动、Linux 发行版）
解决问题
1.完整性：防止安装包被植入病毒、木马，文件一旦篡改校验失败；
2.身份认证：确认安装包来自官方厂商，拦截第三方伪造恶意安装程序；
3.不可抵赖：官方无法否认发布过该版本软件，出现安全漏洞可追溯责任。
场景 2：电子合同 / 电子发票（政务、财税系统）
解决问题
1.保障合同、发票内容不可被单方面篡改；
2.证明签署人身份，具备法律效力；
3.实现不可否认，交易双方不能否认签署、开具票据行为，满足合规取证需求。
场景 3：Git 代码提交签名（代码仓库代码签名）
解决问题
1.校验代码提交者真实身份，防止攻击者伪造他人账号提交恶意代码；
2.代码提交记录不可篡改，保障代码仓库完整性；
3.代码漏洞可精准追溯到提交人，明确开发责任。
...
> 答：1. 数字签名本身能否证明签名时间？
不能。数字签名仅绑定文件哈希与签名私钥，签名内不包含权威可信时间；签名者可修改本地设备系统时间后再签名，事后随意篡改声称的签署时间，无法作为可信时间凭证。
2. 接收者验证签名真实时间的方案
依靠 ** 可信时间戳服务（TSA）** 完成验证，完整流程：
1.签名者对文件生成数字签名，计算该签名的哈希值，发送至第三方权威 TSA 服务器；
2.TSA 使用自身可信、不可篡改的标准时钟，对哈希加盖官方时间戳并使用 TSA 私钥签名，返回时间戳凭证；
3.交付文件、原始签名、TSA 时间戳凭证给接收方；
4.接收方使用 TSA 公开证书校验时间戳签名，即可获取权威、不可篡改的签署时间，戳内记录的时间由中立第三方担保，签名者无法抵赖。
3. 针对题目场景的解决逻辑
一年后签名者谎称文件是昨日签名，接收者操作：
1.校验 TSA 时间戳合法性，提取第三方公证的原始签署时间；
2.时间戳由独立可信机构签发，签名者无法篡改、伪造；
3.以此证据证明文件实际签署于一年前，戳穿虚假时间说辞，解决时间抵赖问题。
...
> 答：一、实验选用 2048 位 RSA 密钥的原因
2048 位是安全与性能均衡的标准选择：
1.安全性达标：NIST 等密码标准规定 2048 位 RSA 可安全使用至 2030 年，能抵御现有常规暴力分解、数论攻击，满足教学、普通业务场景安全需求；
2.性能适中：相比 4096 位，模幂运算速度更快，密钥生成、签名、验签、加密解密耗时更低，实验操作流畅；
3.通用性强：所有主流 OpenSSL、OpenSSH 等工具原生支持 2048 位，是教学实验、互联网业务的通用默认密钥长度。
二、1024 位 RSA 是否足够安全
不安全，已被淘汰，不推荐使用
1.算力提升后，1024 位模数可通过分布式计算、专用数论算法完成分解，攻击者能破解私钥；
2.各国密码标准已废止 1024 位 RSA，禁止用于电子签名、金融、政务等正式场景；
3.仅可用于无敏感数据的临时测试，无法保障长期安全。
三、4096 位 RSA 是否有必要
分场景判断：
1.有必要的场景：高敏感长期存储数据（金融票据、法律电子合同、根 CA 证书），需要数十年安全保障，优先 4096 位；
2.无必要的场景（本实验）：课堂实验、普通文件短时签名，4096 位运算开销极大，签名 / 解密速度显著下降，资源浪费，2048 位完全满足需求。
四、不同密钥长度安全与性能权衡总结
1024 位：运算最快，但安全强度不足，现有算力可破解，正式场景禁止使用，仅能做无敏感临时测试。
2048 位：安全与性能平衡最优，安全性可满足中长期使用，运算速度适中，是教学、普通业务通用标准，本实验选用该长度。
4096 位：安全强度最高，能抵御未来更强算力攻击，但签名、加解密运算耗时大幅增加，仅金融、根证书等高敏感长期存证场景才需要，普通实验没必要使用。
...
> 答：一、这种嵌套签名有实际意义
Alice 先签文件，Bob 再对 Alice 的签名二次签名，形成签名链，具备两层核验价值：
1.第一层 Alice 签名：证明原始文件由 Alice 出具、内容未篡改；
2.第二层 Bob 签名：证明 Bob 已审阅、认可 Alice 这份文件与签名，留下 Bob 的责任凭证；
两层签名独立可验，能完整追溯文件流转、审批全流程，具备业务与法律层面的凭证价值。
二、需要多重签名 / 签名链的典型场景
1.多级审批公文、电子合同
文件起草人签名后，部门负责人、法务、管理层逐层二次签名，每一级签名代表该环节审核通过，可追溯每一位审批人，区分各环节责任。
2.证书信任链（CA 体系）
根 CA 签名二级 CA 证书，二级 CA 再签名用户证书，多层签名构成信任链，接收方逐级验签确认证书可信。
3.软件多级分发审核
开发人员签名程序包，厂商安全审核部门再对开发者签名二次签名，终端设备同时校验两层签名，确认程序经过官方安全审核。
4.跨境 / 多方交易凭证
买卖双方、第三方公证机构依次嵌套签名，多方签名留存完整证据链，发生纠纷时可分别核验各方确认行为。
