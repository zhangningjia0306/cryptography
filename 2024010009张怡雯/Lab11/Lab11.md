...
| 签名公钥指数 e 的值 |65537 |
| 签名公钥模数 N 的前 16 位十六进制 |c4:7e:3b:a1:2f:5d:8c:9e |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | 这是一条重要消息，需要签名保护。发送者：张怡雯，学号：2024010009|
| 密文文件 encrypted_message.bin 大小（字节） |256 |
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |d4a2e9684d83cb127aa2f363c40d73ee7d7a0323e1513a6b1c22bb33c10fa2f3 |
...
身份认证（Authentication）：证明消息确实来自持有签名私钥的发送者。在本次实验中，只有拥有 signature_private_key.pem 的人才能生成有效的签名；接收方用 signature_public_key.pem 验证通过后，就能确认密文来自发送方。

完整性保护（Integrity）：确保消息在传输过程中未被篡改。在本次实验中，当篡改密文（tampered_encrypted_message.bin）后，签名验证失败（Verification failure），说明任何对消息的改动都能被检测到。

不可否认性（Non-repudiation）：发送者无法否认自己发送过该消息。因为签名私钥只有发送者持有，其他人无法伪造签名，所以发送者不能事后否认签过名。
...
效率问题：RSA 非对称加密运算速度远慢于哈希函数。先计算哈希值（如 SHA-256）将任意大小的数据压缩为固定长度（32 字节），再对这 32 字节签名，效率远高于对整个密文直接做 RSA 运算。

输入长度限制：RSA 一次能处理的数据长度不能超过模数长度（2048 位 ≈ 256 字节）。直接对可能更大的密文签名不现实，而哈希值固定为 32 字节，适合 RSA 处理。

标准化与安全性：哈希函数将任意长度消息映射为固定长度摘要，使签名算法接口统一。同时，哈希函数的抗碰撞性保证了不同消息产生相同哈希值的可能性极低，从而保证签名绑定了唯一的消息内容。
...
> 答：RSA 签名本质上是对哈希值进行模幂运算：\sigma = H(m)^d \bmod Nσ=H(m) 
d
 modN，运算结果的数值范围是 [0, N-1][0,N−1]，因此签名的长度等于模数 NN 的字节长度。

2048 位密钥：模数 NN 为 2048 位 = 256 字节，所以签名长度是 256 字节。

4096 位密钥：模数 NN 为 4096 位 = 512 字节，所以签名长度会是 512 字节。
...
> 答：SHA-1 不再安全是因为 Google 和 CWI Amsterdam 在 2017 年成功构造了 碰撞攻击：找到了两个不同文件具有相同的 SHA-1 哈希值。

对数字签名的影响：如果攻击者准备两个文件——文件 A（看起来正常、愿意让别人签名的内容）和文件 B（攻击者真正想替换进去的恶意内容），这两个文件的 SHA-1 哈希值相同。那么当签名者签署文件 A 后，攻击者可以将文件 B 和文件 A 的签名一起发送给验证者，验证者会认为签名有效，从而上当。

因此，现代数字签名应使用更安全的算法，如 SHA-256、SHA-384、SHA-512。
...
> 答：签名私钥（signature_private_key.pem）：用于生成数字签名（签名操作）。

签名公钥（signature_public_key.pem）：用于验证数字签名（验证操作）。

加密私钥（encryption_private_key.pem）：用于解密密文（解密操作）。

加密公钥（encryption_public_key.pem）：用于加密明文（加密操作）。

如果攻击者获得了公钥：

能做什么：验证签名（验证是否来自对应私钥持有者）；加密消息（向私钥持有者发送加密消息）。

不能做什么：伪造签名（因为没有私钥，无法计算签名）；解密密文（因为解密需要私钥）。
...
> 答：消息加密解决的问题是机密性，即防止消息被未经授权的人阅读。它的操作方式是用接收方公钥加密，用接收方私钥解密。

数字签名解决的问题是身份认证、完整性和不可否认性，即证明消息来源且未被篡改。它的操作方式是用发送方私钥签名，用发送方公钥验证。

不能用加密代替签名，原因有三点：

第一，加密的目标是隐藏内容，签名的目标是证明来源和完整性，目标不同。

第二，加密无法防止篡改，攻击者可以修改密文，接收方解密后得到错误消息却不知道被篡改。

第三，加密无法提供不可否认性，因为任何知道公钥的人都能加密消息，无法证明消息来自谁。
...
> 答：不可否认性是指：发送方无法否认自己发送过该消息，接收方可以向第三方证明消息确实来自某个发送者。

数字签名能提供不可否认性，因为签名私钥只有发送者持有，所以只有发送者能生成有效签名。第三方可以用发送者的公钥验证签名，确认签名确实由该发送者生成。

MAC 不能提供不可否认性，因为 MAC 使用对称密钥，发送方和接收方共享同一个密钥。接收方可以用相同的密钥生成相同的 MAC 标签，因此接收方无法向第三方证明 MAC 是发送方生成的还是自己伪造的。

简言之：对称密钥无法区分谁生成标签，非对称密钥可以。
...
> 答：场景一，软件发布。软件开发者对安装包进行数字签名。用户下载后验证签名，可以确认安装包确实来自官方发布者，且在下载过程中未被篡改。解决了"软件是否来自可信来源"和"是否被植入恶意代码"的问题。

场景二，电子邮件签名（S/MIME 或 PGP）。发送方对邮件内容签名。接收方验证签名后，可以确认邮件确实来自声称的发件人，且内容在传输过程中未被篡改。解决了"钓鱼邮件"和"邮件伪造"的问题。

场景三，代码签名。开发者对可执行文件或驱动进行签名。操作系统在安装或运行时验证签名，如果签名有效，用户会被提示"来自可信发布者"。解决了"恶意软件冒充合法软件"的问题。
...
> 答：
数字签名本身不能证明签名的时间。签名只包含消息哈希值和签名结果，没有时间信息；即使有本地时间，也容易被伪造。

解决方法：使用可信时间戳服务（TSA）。

具体流程是：发送方将文件哈希值发送给 TSA；TSA 在哈希值上附加当前时间，用 TSA 的私钥签名，生成时间戳令牌；接收方验证 TSA 的签名，确认文件在指定时间之前就已存在。

这样，即使签名者事后否认，TSA 的可信时间戳可以证明签名的时间。
...
> 答：
1024 位 RSA：安全性方面已被破解，NIST 已禁止使用；性能方面最快；适用场景是不应再使用。

2048 位 RSA：安全性方面安全，预计 2030 年前安全；性能方面中等；适用场景是当前主流选择，推荐用于大多数场景。

4096 位 RSA：安全性方面更安全，面向未来；性能方面较慢（约 4-6 倍）；适用场景是高安全需求场景，但会影响性能。

本实验使用 2048 位的原因是：2048 位 RSA 在安全性和性能之间取得了最佳平衡，被广泛接受为安全标准。1024 位不够安全，而 4096 位虽然更安全，但性能开销较大，且对大多数应用场景来说超过需求。
...
> 答：
有意义。这种做法被称为多重签名或签名链，主要有以下应用：

应用一，审核/审批流程。公司内部需要多级审批时，每级审批人依次签名，形成一个签名链。最终接收方验证所有签名，确认文件经过了所有必要的审批环节。

应用二，联合签名。多方共同签署一份文件（如合同），每个人对"前一个人的签名 + 文件内容"签名，形成可验证的签名链。

应用三，公证服务。公证人先对文件签名，时间戳服务再对"文件 + 公证人签名"签名，形成完整的证据链。
---
