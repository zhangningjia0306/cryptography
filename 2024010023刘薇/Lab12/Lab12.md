...
| 证书有效期（起始-结束） | 起始：May 22 12:16:05 2026 GMT结束：Aug 20 12:16:04 2026 GMT|
| 公钥算法 |id-ecPublicKey（椭圆曲线 EC 公钥） |
...
| 是否为公开 CA / 第三方 CA 签发 | 是（Let’s Encrypt 是免费公开第三方 CA）|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） |根 CA 自签名证书verify -CAfile ca.cert.pem ca.cert.pem输出 OK，证明自签发 |
| 证书颁发者（Issuer） | 和 Subject 完全相同，根证书自签发|
| 主题和颁发者是否相同 | 是|
| 证书有效期（天数） |3650天 |
| 公钥长度（位） |2048bit |
| Basic Constraints 中 CA 字段的值 |X509v3 Basic Constraints: critical CA:TRUE, pathlen:1 |

### C. 私有中间 CA 证书（任务三）

| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） | 自定义中间 CA 标识|
| 证书颁发者（Issuer） |与私有根 CA 证书 Subject 完全一致，由根 CA 签发 |
| pathlen 值 |0 |
| 证书有效期（天数） | 3650天|
...
| 证书主题（Subject） | CN=3n7er.space|
| 证书颁发者（Issuer） | C=US, O=Let's Encrypt, CN=E7|
| CA 字段的值 |X509v3 Basic Constraints: critical CA:FALSE |
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |77:36:4C:41:32:A1:C5:FB:5E:90:8C:BA:BC:AE:76:09:F1:B5:70:01:D7:E0:C4:A6:FA:A7:D3:20:7F:06:64:02 |
| SHA-1 指纹（完整） | 7B:B1:94:9B:C5:B0:DA:23:C9:E7:8F:CA:20:FC:BD:BF:7E:94:3C:0E|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |操作系统 / 浏览器内置的公开可信根 CA | 本地自建根 CA|
| 浏览器默认是否信任 |是，系统预置根证书，打开网站无安全警告 |否，浏览器默认不信任，访问会弹出证书风险告警，需手动导入根 CA 证书 |
| 证书签发者是否为第三方 CA |是，Let’s Encrypt 是中立第三方公共 CA 机构 | 否，完全自建，根 CA、中间 CA 均为本地私有，无第三方参与|
| 适合的使用场景 |公网对外提供 HTTPS 服务、互联网公开网站、面向全网用户访问业务 |企业内网系统、实验室测试环境、内部业务服务、局域网设备加密通信、开发调试 |
...
> 答：一、数字证书的本质
数字证书本质是由可信第三方 CA（证书颁发机构）签名的电子身份凭证，是把「用户身份信息、公钥、有效期、使用权限约束」绑定在一起的加密文件（标准 X.509 格式）。
CA 用自身私钥对这份绑定信息做数字签名，证明这份公钥确实属于证书标注的实体（网站、设备、个人）。
二、数字证书解决的核心问题
1.解决公钥分发的中间人攻击问题
无证书场景下公钥直接传输，攻击者可拦截替换公钥，实现中间人窃听篡改；证书由可信 CA 背书，公钥和身份不可被随意篡改替换。
2.对公钥持有者身份做权威背书
证明某公钥确实对应对应的域名 / 组织 / 个人，确认通信对方真实身份，防范伪装钓鱼。
3.管控公钥使用范围与生命周期
证书内置有效期、密钥用途扩展字段（仅允许网站加密、禁止签发子证书等），避免公钥被滥用；过期 / 泄露证书可通过 CRL/OCSP 吊销。
4.构建分层可信信任链
通过根 CA→中间 CA→终端证书的层级结构，实现大规模网络下统一信任体系，不用两两线下交换公钥。
三、仅靠数字签名不足，必须配套数字证书的原因
1.数字签名只能证明「数据没被篡改、签名者持有对应私钥」，无法证明签名者身份可信
任何人都能生成公私钥、对消息做数字签名，但接收方无法确认这个公钥到底属于谁，攻击者可以生成自己密钥签名伪装目标主体。
2.数字签名无法限制密钥的合法用途与有效期
单纯签名没有附加约束，一份密钥可被随意用于加密网站、签发其他证书等违规操作；证书通过扩展字段强制限定密钥用途，同时设置过期时间。
3.数字签名缺少统一可信锚点，无法规模化信任
若只有签名，所有通信双方必须线下核验对方公钥，互联网场景完全不可行；数字证书依托全球预置根 CA，实现跨设备、跨网络自动信任校验。
4.数字签名不支持证书吊销机制
若私钥泄露，仅靠签名无法快速全网作废对应公钥；证书配套 CRL/OCSP 吊销列表，可实时判定证书是否失效。
...
> 答：一、什么是自签名证书
自签名证书是实体使用自身私钥对自身身份、公钥信息签名生成的 X.509 证书，证书的颁发者 (Issuer) 与主题 (Subject) 完全一致，不存在第三方 CA 机构参与签发。
实验里的私有根 CA 证书就属于典型自签名证书；而 CA 签名证书是由可信第三方 CA 使用 CA 私钥签发，Issuer 与 Subject 不相同。
二、浏览器提示自签名证书不安全的原因
1.无全球可信第三方身份背书
浏览器内置了正规公网 CA 根证书信任列表，自签名证书签发主体不在内置信任库内，浏览器无法确认公钥对应的网站身份真实有效，无法抵御中间人攻击。
2.无法自动化校验证书合法性与吊销状态
正规 CA 证书配套 CRL/OCSP 吊销机制，密钥泄露可全网作废；自签名证书无统一吊销渠道，泄露后无法全局拦截风险。
3.缺少域名所有权校验流程
公网 CA 签发前会验证域名归属，防止他人冒用域名申请证书；自签名证书可随意填写任意域名，极易被钓鱼网站滥用。
4.默认信任域极小
仅手动导入根证书的单台设备会信任，其他设备访问都会弹出安全告警，公网环境会严重影响用户体验。
三、可合法使用自签名证书的场景
1.实验室 / 开发调试环境
本地后端、Web 服务、密码学实验测试（本次 PKI 实验），仅开发者个人访问。
2.企业纯内网隔离业务系统
局域网 OA、内部管理平台、服务器运维面板，所有访问设备可统一批量导入私有根证书。
3.无公网暴露的本地设备通信
智能家居、内网数据库、工业局域网加密通道，仅内部人员可控访问。
4.临时点对点加密通信
短期测试文件传输、本地 SSH/TLS 加密通道，无外部普通用户接入。
...
> 答：一、证书链分层（根 CA→中间 CA→终端）的必要性
1.保护根密钥：根 CA 离线封存，仅中间 CA 对外签发，降低根私钥泄露风险；
2.风险隔离：单个中间 CA 泄露仅影响其下属证书，不会摧毁整套信任体系；
3.分级分权：按业务划分中间 CA，精细化管控证书签发权限、有效期；
4.分担签发压力：多中间 CA 分布式处理海量证书请求，提升扩展性。
二、根 CA 不直接签发终端证书的原因
1.根私钥需频繁联网签名，泄露后所有证书全部失效，损失不可挽回；
2.无法按业务隔离风险，单张证书故障会牵连全网；
3.难以差异化管控不同场景证书规则，运维、算法更新成本极高。
...
> 答：一、自签名根 CA 仍可被信任的原因
根证书的信任不靠上级证书签名背书，依靠预先预置 + 人工可信导入建立信任锚：
1.公网根证书由系统 / 浏览器厂商预装，厂商已提前审核 CA 资质；
2.私有根证书由使用者手动导入信任库，人为确认其可信；
3.证书链校验终点是根证书，只要本地信任库存在该根证书，就认可整条链合法。
二、系统、浏览器筛选可信根 CA 的规则
1.严格资质审核：CA 机构需通过 WebTrust 等国际安全审计，满足密钥保管、域名核验、吊销机制等规范；
2.厂商人工收录：Windows、Chrome、Firefox 等厂商建立根证书计划，仅收录合规 CA，定期剔除违规根；
3.本地手动自定义信任：用户可自行导入私有根 CA，系统会将其加入本机信任列表；
4.动态淘汰机制：出现密钥泄露、违规签发证书的根 CA，会被新版系统 / 浏览器移除信任库。
...
> 答：pathlen 含义
1.pathlen 是 Basic Constraints 扩展里的路径长度限制，规定该 CA 证书下方最多允许存在几层下级 CA，用于限制证书链层级、防止证书层级无限延伸。
2.pathlen:0
该中间 CA不允许再签发任何下级 CA 证书，仅能签发终端网站 / 设备证书。
证书链只能是：根 CA → 此中间 CA → 终端证书。
3.pathlen:1
该 CA 最多可以再签发 1 层下级中间 CA，下级 CA 不能再继续签发 CA。
证书链最长为：根 CA → 此 CA → 次级中间 CA → 终端证书。
4.二者核心区别
pathlen 数值越大，允许向下延伸的 CA 层级越多；pathlen:0 禁止新增中间 CA，层级限制更严格。
...
> 答：1. 网站证书过期的后果
浏览器判定证书无效，弹出高危安全告警，默认阻止用户访问网站；HTTPS 加密链路失效，通信易被窃听、中间人篡改。
2. 证书设置有效期的原因
限制密钥风险窗口：密钥泄露后，过期证书会自动失效，降低长期危害；
强制更新密钥与算法：推动站点迭代更安全的加密套件；
强制核验域名所有权：定期确认域名仍归申请者持有，防止证书被冒用。
3. Let's Encrypt 仅 90 天有效期的原因
缩短风险周期：证书泄露、域名被盗用的影响时长被压缩；
推动自动化运维：倒逼站长部署自动续期工具（ACME），减少人工漏更新；
降低吊销压力：短期证书自然过期，无需频繁生成庞大的吊销列表；
契合免费自动化签发的设计定位，减少长期证书滥用风险。
...
> 答：一、需要吊销证书的场景
1.证书对应的私钥泄露、被盗；
2.域名所有权变更，原持有者不再拥有该域名；
3.证书申请者身份信息造假、违规使用证书；
4.服务停止运营，证书不再使用；
5.CA 发现证书签发存在错误。
二、CRL 与 OCSP 的区别
1.CRL（证书吊销列表）
CA 定期生成一份完整文件，包含所有已吊销证书序列号，客户端完整下载后本地比对。
缺点：文件随吊销证书增多持续变大，占用带宽；存在更新延迟。
优点：离线校验，一次下载长期可用。
2.OCSP（在线证书状态协议）
客户端向 OCSP 服务器单独查询单张证书的实时状态，仅返回该证书结果。
优点：请求体积小、实时性高、流量开销低；
缺点：必须联网，存在 OCSP 服务器单点故障、隐私泄露风险。
...
> 答：一、现代证书必须带 SAN 扩展的原因
1.浏览器已废弃 CN 域名校验：新版浏览器不再读取 Subject 里的 CN 来匹配网站域名，仅信任 SAN 字段内的域名，无 SAN 会判定证书无效、弹出安全警告。
2.支持多域名共用单张证书：一张证书可填入多个域名（主域名、子域名、通配符域名），降低多站点证书部署成本。
3.支持多种身份类型：除 DNS 域名外，还可存放 IP、邮箱、设备标识符等，适配服务器、客户端、硬件设备多类场景。
二、SAN 与 CN 的核心区别
1.CN（Common Name，通用名）
位于证书 Subject 主体字段，仅能填写单个域名；旧标准遗留字段，现代浏览器不再以此校验域名匹配，仅作备注展示。
2.SAN（Subject Alternative Name，主题备用名称）
独立 X.509 扩展字段，支持存储多条域名 / IP；是当前浏览器、系统校验站点身份的唯一标准字段，功能覆盖并替代 CN。
...
> 答：一、证书指纹计算方式
将完整 DER 二进制格式的证书全文，输入 SHA-1/SHA-256 等哈希算法运算，输出固定长度哈希摘要，再用冒号分隔十六进制字符，即为证书指纹。
二、证书钉扎（Certificate Pinning）
1.定义：客户端（浏览器 / App）预先内置网站证书 / 公钥的指纹，强制校验服务器返回证书指纹必须与本地预存值匹配。
2.作用：抵御中间人替换合法 CA 签发伪造证书的攻击，仅信任固定公钥，不信任其他任何 CA 颁发的证书。
3.缺点：证书更换后未同步更新钉扎信息会导致网站无法访问，现多采用 HPKP 动态钉扎或公钥钉扎缓解该问题。
...
> 答：一、*.example.com 可匹配域名
1.匹配一级子域名：a.example.com、blog.example.com、admin.example.com；
2.无法匹配：根域名 example.com、多级子域名 api.blog.example.com、其他主域名 test.com。
二、通配符证书的局限性
1.仅匹配一层子域名：不能覆盖二级及更深子域名；
2.安全风险集中：私钥泄露后，所有匹配子域名全部沦陷；
3.不匹配裸根域名：无法保护 example.com 本身，需额外添加到 SAN；
4.通配符仅允许最左侧：a.*.example.com、www.example.* 这类写法不被标准支持；
5.公开 CA 限制：Let's Encrypt 等免费 CA 对通配符证书有签发校验门槛，且无法搭配多个不同主域名。
...
> 答：一、证书透明度（CT）定义
CT 是一套公开日志系统（CT Logs），CA 签发证书前必须将预证书提交至公共 CT 日志服务器，日志会生成 SCT（签名证书时间戳）附加到正式证书中；浏览器会校验证书携带合法 SCT 才信任该证书。所有入库证书全网公开可查询、审计。
二、CT 解决的核心问题
1.监控违规证书签发
任何人可查询某域名下全部已签发证书，能快速发现 CA 违规签发的伪造域名证书（中间人钓鱼证书），及时上报吊销。
2.约束 CA 滥发证书行为
强制证书公开留痕，CA 无法秘密签发证书，大幅降低恶意 CA 违规操作空间。
3.辅助浏览器校验证书合法性
主流浏览器强制校验 SCT，不含有效 CT 日志记录的证书会被标记不安全，拦截非法证书。
4.提供全局证书审计能力
安全机构、域名所有者可持续检索自身域名所有证书，第一时间发现私钥泄露、未授权签发的异常证书。
...
> 答：一、Let's Encrypt 免费签发证书原理（结合 3n7er.space 证书链）
1.组织与成本模式：非营利机构，依靠企业捐赠、赞助覆盖运营开销，不向用户收取证书服务费。
2.分层证书链架构（3n7er.space 对应链路）
根 CA（ISRG Root X1）→ 中间 CA（E7）→ 终端网站证书（3n7er.space）。根证书预装在浏览器，中间 CA 负责批量签发终端证书，分担签名压力，无需人工介入审核。
3.全自动化 ACME 协议：全程机器交互，省去人工审核、人工签发人力成本，实现零人工成本批量出证。
4.标准化自动化运维：配合 CT 证书透明度日志强制归档，自动化吊销、自动化续期流程，降低人工运维开销。
二、ACME 协议域名控制权验证方式（HTTP-01 / DNS-01）
1.HTTP-01 验证（普通域名）
客户端在域名根目录.well-known/acme-challenge/下放指定随机验证文件，CA 发起 HTTP 访问该文件；能正常读取即证明申请者拥有网站服务器控制权。适配 3n7er.space 这类普通单域名。
2.DNS-01 验证（通配符域名）
客户端在域名 DNS 添加_acme-challenge前缀 TXT 记录，CA 查询该记录内容；匹配则证明申请者拥有域名 DNS 解析权限，用于签发*.3n7er.space通配符证书。
三、证书有效期 90 天、强制依赖自动续期的原因
1.缩短风险窗口
证书泄露、域名被盗用后，最长仅 90 天自动失效，缩小安全漏洞暴露时长。
2.倒逼自动化运维
短期证书无法人工手动批量更新，推动用户部署 Certbot 等 ACME 自动续期工具，避免过期断站。
3.减轻证书吊销负担
大量短期证书会自然过期失效，无需长期保存在庞大 CRL 吊销列表，降低服务器存储与查询压力。
4.定期重新核验域名所有权
每 90 天重新走一遍域名验证流程，防止域名转让后旧证书继续被冒用。
...
> 答：一、信任差异原因
1.浏览器默认信任3n7er.space证书：该证书的证书链顶端是 ISRG Root X1 公网根 CA，这份根证书预先内置在操作系统和浏览器的信任列表里，浏览器能完整校验整条证书链的合法性，因此默认信任。
2.不信任实验私有服务器证书：实验证书的信任锚是本地自建的私有根 CA，系统、浏览器默认信任库中不存在这份根证书，没有可信校验起点，证书链校验失败，浏览器判定不安全；只有手动把私有根证书导入本机信任库后，才会被信任。
二、两者相同点
1.证书格式标准统一：都遵循 X.509 v3 规范，包含主题、SAN 域名、有效期、基础约束、密钥用途等标准扩展，PEM、DER 文件格式互相兼容。
2.证书链分层结构一致：均采用「根 CA→中间 CA→终端服务器证书」三层架构；根证书 Basic Constraints 标记CA:TRUE，终端服务器证书标记CA:FALSE，依靠 pathlen 控制证书链最大层级。
3.底层密码学机制相同：都依靠非对称密钥完成证书签名，使用哈希算法校验证书完整性，都支持 CRL、OCSP 两种证书吊销校验方式。
三、两者不同点
1.信任根来源不同：Let's Encrypt 使用经过安全审计、系统预装的公共根 CA；私有 PKI 使用本地自行生成的自签名根 CA，不会预装在通用设备中。
2.签发主体与监管不同：公网证书由合规第三方 CA 机构签发，受国际安全规范审计约束；私有证书由个人本地搭建签发，无第三方监管审核。
3.域名校验规则不同：Let's Encrypt 通过 ACME 协议强制验证域名所有权，避免证书被冒用；私有 PKI 无强制域名校验，可随意填写任意域名。
4.信任覆盖范围不同：公网证书全网设备默认可信；私有证书仅在手动导入根证书的设备内有效，外部设备全部告警。
5.附加合规要求不同：Let's Encrypt 强制接入 CT 证书透明度日志，所有签发证书全网公开可审计；私有 PKI 无 CT 日志强制要求。
6.适用场景不同：公网证书面向互联网对外开放的公开站点；私有 PKI 仅适用于实验室、企业内网、内部测试系统等封闭环境。
